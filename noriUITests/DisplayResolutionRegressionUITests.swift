...
final class DisplayResolutionRegressionUITests: XCTestCase {
    private let defaultDisplayHarnessManifestPath = "/tmp/nori-ui-test-display-harness.json"
    ...
    override func setUp() {
        ...
        let tempPrefix = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-ui-test-display-\(token)")
            .path
        launchTag = "ui-tests-display-resolution-\(token.prefix(8))"
        diagnosticsPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-ui-test-display-churn-\(token).json")
            .path
    ...
    private let prelaunchManifestPath = "/tmp/nori-ui-test-prelaunch.json"
    ...
    private func loadPrebuiltHelperBinaryPath(_ env: [String: String]) -> String? {
        guard let helperBinaryPath = env["NORI_UI_TEST_DISPLAY_HELPER_BINARY_PATH"],
    ...
    private func loadExternalHarnessFromEnvironment(_ env: [String: String]) -> ExternalDisplayHarness? {
        guard let readyPath = env["NORI_UI_TEST_DISPLAY_READY_PATH"], !readyPath.isEmpty,
              let displayIDPath = env["NORI_UI_TEST_DISPLAY_ID_PATH"], !displayIDPath.isEmpty,
              let donePath = env["NORI_UI_TEST_DISPLAY_DONE_PATH"], !donePath.isEmpty else {
            return nil
        }
        ...
        return ExternalDisplayHarness(
            ...
            startPath: env["NORI_UI_TEST_DISPLAY_START_PATH"],
            donePath: donePath,
            logPath: env["NORI_UI_TEST_DISPLAY_LOG_PATH"],
            helperBinaryPath: nil
        )
    }

    private func loadExternalHarnessFromManifest(_ env: [String: String]) -> ExternalDisplayHarness? {
        let manifestPath = env["NORI_UI_TEST_DISPLAY_HARNESS_MANIFEST_PATH"] ?? defaultDisplayHarnessManifestPath
    ...
    private func launchAppProcess(targetDisplayID: String) throws {
        ...
        let logPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-ui-test-app-\(launchTag).log").path
    ...
    private func resolveAppBinaryPath() throws -> String {
        ...
        //   .../Build/Products/Debug/noriUITests-Runner.app/Contents/PlugIns/noriUITests.xctest
        ...
        let productsDir = testBundle.bundleURL
            ...
            .deletingLastPathComponent()  // -> .../noriUITests-Runner.app
            .deletingLastPathComponent()  // -> .../Debug
        let binaryPath = productsDir
            .appendingPathComponent("nori DEV.app")
            .appendingPathComponent("Contents/MacOS/nori DEV")
            .path
    ...
    private func launchEnvironment(targetDisplayID: String) -> [String: String] {
        [
            "NORI_UI_TEST_MODE": "1",
            "NORI_UI_TEST_DIAGNOSTICS_PATH": diagnosticsPath,
            "NORI_UI_TEST_DISPLAY_RENDER_STATS": "1",
            "NORI_UI_TEST_TARGET_DISPLAY_ID": targetDisplayID,
        ]
    }
