...
final class FeedbackComposerShortcutUITests: XCTestCase {
    ...
    func testCmdOptionFWorksFromSettingsWindow() {
        let app = XCUIApplication()
        app.launchEnvironment["NORI_UI_TEST_MODE"] = "1"
        app.launchEnvironment["NORI_UI_TEST_SHOW_SETTINGS"] = "1"
        app.launch()
        app.activate()
...
final class CommandPaletteAllSurfacesUITests: XCTestCase {
    ...
    private let hiddenSurfaceToken = "nori-command-palette-hidden-surface"
    private let visibleSurfaceToken = "nori-command-palette-visible-surface"
    private let noMatchWorkspaceQuery = "nori-command-palette-no-match"

    override func setUp() {
        super.setUp()
        continueAfterFailure = false
        socketPath = "/tmp/nori-ui-test-command-palette-\(UUID().uuidString).sock"
    ...
    func testMinimalModeToggleKeepsSettingsWindowFocused() throws {
        let app = XCUIApplication()
        let diagnosticsPath = "/tmp/nori-ui-test-settings-focus-\(UUID().uuidString).json"
        ...
        let diagnostics = waitForDiagnostics(
            ...
            data["keyWindowIdentifier"] == "nori.settings" && data["settingsWindowIsKey"] == "1"
        }
