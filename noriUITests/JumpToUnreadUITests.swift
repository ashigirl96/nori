...
final class JumpToUnreadUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        dataPath = "/tmp/nori-ui-test-jump-unread-\(UUID().uuidString).json"
        try? FileManager.default.removeItem(atPath: dataPath)
    }

    func testJumpToUnreadFocusesPanelAcrossTabs() {
        let app = XCUIApplication()
        app.launchEnvironment["NORI_UI_TEST_JUMP_UNREAD_SETUP"] = "1"
        app.launchEnvironment["NORI_UI_TEST_JUMP_UNREAD_PATH"] = dataPath
        app.launch()
        app.activate()
