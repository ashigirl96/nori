...
final class CloseWorkspaceCmdDUITests: XCTestCase {
    ...
    func testCmdDConfirmsCloseWhenClosingLastWorkspaceClosesWindow() {
        ...
        app.launchEnvironment["NORI_UI_TEST_FORCE_CONFIRM_CLOSE_WORKSPACE"] = "1"
        app.launch()
        app.activate()
    ...
    func testCmdWClosingLastTabKeepsWorkspaceWindowOpen() {
        let app = XCUIApplication()
        let keyequivPath = "/tmp/nori-ui-test-keyequiv-\(UUID().uuidString).json"
    ...
    func testChildExitInHorizontalSplitClosesOnlyExitedPane() {
        ...
        for attempt in 1...attempts {
            let app = XCUIApplication()
            let dataPath = "/tmp/nori-ui-test-child-exit-split-\(UUID().uuidString).json"
            try? FileManager.default.removeItem(atPath: dataPath)

            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] = "1"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"] = dataPath
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_LAYOUT"] = "lr"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_EXPECTED_PANELS_AFTER"] = "1"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_AUTO_TRIGGER"] = "1"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_STRICT"] = "1"
            app.launch()
            app.activate()
            defer { app.terminate() }
    ...
    func testCtrlDFromKeyboardInHorizontalSplitClosesOnlyFocusedPane() {
        let app = XCUIApplication()
        let dataPath = "/tmp/nori-ui-test-child-exit-keyboard-\(UUID().uuidString).json"
        try? FileManager.default.removeItem(atPath: dataPath)
        app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] = "1"
        app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"] = dataPath
        app.launch()
    ...
    func testCtrlDFromKeyboardInThreePaneLayoutClosesOnlyFocusedPane() {
        let app = XCUIApplication()
        let dataPath = "/tmp/nori-ui-test-child-exit-keyboard-tree-\(UUID().uuidString).json"
        try? FileManager.default.removeItem(atPath: dataPath)
        app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] = "1"
        app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"] = dataPath
        app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_LAYOUT"] = "lr_left_vertical"
        app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_EXPECTED_PANELS_AFTER"] = "2"
        app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_AUTO_TRIGGER"] = "1"
        app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_STRICT"] = "1"
        app.launch()
        app.activate()
    ...
    func testCtrlDAfterClosingRightColumnIn2x2KeepsWorkspaceOpen() {
        ...
        for attempt in 1...attempts {
            let app = XCUIApplication()
            let dataPath = "/tmp/nori-ui-test-child-exit-keyboard-2x2-\(UUID().uuidString).json"
            try? FileManager.default.removeItem(atPath: dataPath)
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] = "1"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"] = dataPath
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_LAYOUT"] = "lrtd_close_right_then_exit_top_left"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_EXPECTED_PANELS_AFTER"] = "1"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_AUTO_TRIGGER"] = "0"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_STRICT"] = "1"
            app.launch()
            app.activate()
            defer { app.terminate() }
    ...
    func testCtrlDAfterClosingBottomRowIn2x2KeepsWorkspaceOpen() {
        ...
        for attempt in 1...attempts {
            let app = XCUIApplication()
            let dataPath = "/tmp/nori-ui-test-child-exit-keyboard-2x2-bottom-\(UUID().uuidString).json"
            try? FileManager.default.removeItem(atPath: dataPath)
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] = "1"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"] = dataPath
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_LAYOUT"] = "tdlr_close_bottom_then_exit_top_left"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_EXPECTED_PANELS_AFTER"] = "1"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_AUTO_TRIGGER"] = "0"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_STRICT"] = "1"
            app.launch()
    ...
    func testCtrlDFromRealKeyboardAfterClosingRightColumnIn2x2KeepsWorkspaceOpen() {
        ...
        for attempt in 1...attempts {
            let app = XCUIApplication()
            let dataPath = "/tmp/nori-ui-test-child-exit-keyboard-2x2-realkey-\(UUID().uuidString).json"
            try? FileManager.default.removeItem(atPath: dataPath)
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] = "1"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"] = dataPath
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_LAYOUT"] = "lrtd_close_right_then_exit_top_left"
    ...
    func testCtrlDFromRealKeyboardInHorizontalSplitKeepsWindowOpen() {
        ...
        for attempt in 1...attempts {
            let app = XCUIApplication()
            let dataPath = "/tmp/nori-ui-test-child-exit-keyboard-lr-realkey-\(UUID().uuidString).json"
            try? FileManager.default.removeItem(atPath: dataPath)
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] = "1"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"] = dataPath
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_LAYOUT"] = "lr"
    ...
    func testCtrlDEarlyDuringSplitStartupKeepsWindowOpen() {
        ...
        for attempt in 1...attempts {
            let app = XCUIApplication()
            let dataPath = "/tmp/nori-ui-test-child-exit-keyboard-lr-early-ctrl-\(UUID().uuidString).json"
            try? FileManager.default.removeItem(atPath: dataPath)
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_SETUP"] = "1"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_PATH"] = dataPath
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_LAYOUT"] = "lr"
            ...
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_AUTO_TRIGGER"] = "1"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_STRICT"] = "1"
            app.launchEnvironment["NORI_UI_TEST_CHILD_EXIT_KEYBOARD_TRIGGER_MODE"] = "early_ctrl_d"
            app.launch()
            app.activate()
            defer { app.terminate() }
