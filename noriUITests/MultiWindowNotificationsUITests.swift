...
final class MultiWindowNotificationsUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        dataPath = "/tmp/nori-ui-test-multi-window-notifs-\(UUID().uuidString).json"
        socketPath = "/tmp/nori-ui-test-socket-\(UUID().uuidString).sock"
    ...
    func testNotificationsRouteToCorrectWindow() {
        ...
        app.launchEnvironment["NORI_UI_TEST_MULTI_WINDOW_NOTIF_SETUP"] = "1"
        app.launchEnvironment["NORI_UI_TEST_MULTI_WINDOW_NOTIF_PATH"] = dataPath
        app.launch()
        XCTAssertTrue(
    ...
    func testEmptyNotificationsPopoverBlocksTerminalTyping() throws {
        ...
        app.launchEnvironment["NORI_SOCKET_PATH"] = socketPath
        app.launchEnvironment["NORI_SOCKET_MODE"] = "allowAll"
        app.launchEnvironment["NORI_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["NORI_UI_TEST_SOCKET_SANITY"] = "1"
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
        ...
        let marker = "nori_notif_block_\(UUID().uuidString.replacingOccurrences(of: "-", with: "").prefix(8))"
    ...
    func testNotifyCLIDoesNotStealFocusAcrossWindows() throws {
        ...
        app.launchEnvironment["NORI_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["NORI_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["NORI_UI_TEST_NOTIFY_SOURCE_TERMINAL_READY"] = "1"
        app.launchEnvironment["NORI_UI_TEST_ENABLE_DUPLICATE_LAUNCH_OBSERVER"] = "1"
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
        ...
        let commandStatusPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-ui-test-notify-\(commandResultStem).status")
            .path
        let commandStdoutPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-ui-test-notify-\(commandResultStem).stdout")
            .path
        let commandStderrPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-ui-test-notify-\(commandResultStem).stderr")
            .path
        let commandScriptPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-ui-test-notify-\(commandResultStem).sh")
            .path
        defer {
        ...
        guard let bundledCLIPath = resolveNoriCLIPaths(strategy: .bundledOnly).first else {
            XCTFail("Failed to locate bundled nori CLI for notify regression test")
            return
        }
        ...
        do {
            ...
            XCTFail(
                "Failed to write delayed bundled `nori notify` script. " +
        ...
        XCTAssertTrue(
            ...
            "Expected nori to move to background before delayed notify command runs. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertTrue(
            ...
            "Expected delayed bundled `nori notify` command to finish without foregrounding nori. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertFalse(
            ...
            "Expected nori to remain in background after bundled `nori notify`. state=\(app.state.rawValue) stderr=\(notifyStderr)"
        )
        guard notifyExitStatus == "0" else {
            XCTFail(
                "Expected bundled `nori notify` launched from the in-app shell to succeed. " +
    ...
    private func waitForNoriPing(timeout: TimeInterval) -> (stdout: String?, stderr: String?) {
        var lastStdout: String?
        var lastStderr: String?
        let didSucceed = waitForCondition(timeout: timeout) {
            let result = self.runNoriCommand(
        ...
        let result = runNoriCommand(socketPath: socketPath, arguments: ["ping"], responseTimeoutSeconds: 2.0)
        let stdout = result.stdout.isEmpty ? nil : result.stdout
        let stderr = result.stderr.isEmpty ? nil : result.stderr
    ...
    private func runNoriCommand(
        socketPath: String,
        arguments: [String],
        responseTimeoutSeconds: Double = 3.0,
        cliStrategy: NoriCLIStrategy = .any
    ) -> (terminationStatus: Int32, stdout: String, stderr: String) {
        var args = ["--socket", socketPath]
        args.append(contentsOf: arguments)
        var environment = ProcessInfo.processInfo.environment
        environment["NORITERM_CLI_RESPONSE_TIMEOUT_SEC"] = String(responseTimeoutSeconds)

        let cliPaths = resolveNoriCLIPaths(strategy: cliStrategy)
        if cliPaths.isEmpty, cliStrategy == .bundledOnly {
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to locate bundled nori CLI"
            )
        }

        var lastPermissionFailure: (terminationStatus: Int32, stdout: String, stderr: String)?
        for cliPath in cliPaths {
            let result = executeNoriCommand(
        ...
        if cliStrategy == .bundledOnly {
            return lastPermissionFailure ?? (
                terminationStatus: -1,
                stdout: "",
                stderr: "Bundled nori CLI command failed without an executable path"
            )
        }

        let fallbackArgs = ["nori"] + args
        let fallbackResult = executeNoriCommand(
    ...
    private enum NoriCLIStrategy: Equatable {
        case any
        case bundledOnly
    }
    ...
    private func resolveNoriCLIPaths(strategy: NoriCLIStrategy) -> [String] {
        ...
        if strategy == .any {
            for key in ["NORI_UI_TEST_CLI_PATH", "NORITERM_CLI"] {
    ...
    private func appendCLIPathCandidates(
        ...
        strategy: NoriCLIStrategy,
        to candidates: inout [String]
    ) {
        candidates.append("\(productsDir)/nori DEV.app/Contents/Resources/bin/nori")
        candidates.append("\(productsDir)/nori.app/Contents/Resources/bin/nori")
        if strategy == .any {
            candidates.append("\(productsDir)/nori")
        }
        ...
        for entry in entries.sorted() where entry.hasSuffix(".app") {
            let cliPath = URL(fileURLWithPath: productsDir)
                ...
                .appendingPathComponent("Contents/Resources/bin/nori")
                .path
            candidates.append(cliPath)
        }
        if strategy == .any {
            for entry in entries.sorted() where entry == "nori" {
                let cliPath = URL(fileURLWithPath: productsDir)
                    .appendingPathComponent(entry)
                    .path
                candidates.append(cliPath)
    ...
    private func executeNoriCommand(
        ...
        do {
            ...
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to run nori command: \(error.localizedDescription) (cliPath=\(executablePath))"
            )
        }
    ...
    private func resolveSocketPath(timeout: TimeInterval, requiredWorkspaceId: String? = nil) -> String? {
        ...
        _ = waitForCondition(timeout: timeout) {
            for candidate in primaryCandidates {
                ...
                // Primary candidate is the explicitly requested NORI_SOCKET_PATH. If it responds,
    ...
    private func expectedSocketCandidates(includeGlobalFallback: Bool) -> [String] {
        ...
        if includeGlobalFallback {
            ...
            candidates.append("/tmp/nori-debug.sock")
            candidates.append(stableSocketPath())
            candidates.append("/tmp/nori.sock")
        }
    ...
    private func stableSocketPath() -> String {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first?
            ...
            .appendingPathComponent("nori.sock", isDirectory: false)
            .path ?? "/tmp/nori.sock"
    }
    ...
    private func discoverTmpSocketCandidates(limit: Int) -> [String] {
        ...
        let matches = entries.filter { $0.hasPrefix("nori") && $0.hasSuffix(".sock") }
