...
final class TerminalCmdClickUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        fixtureDirectoryURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-ui-test-terminal-cmd-click-\(UUID().uuidString)", isDirectory: true)
    ...
    private func launchApp(
        ...
        app.launchEnvironment["NORI_UI_TEST_MODE"] = "1"
        app.launchEnvironment["NORI_UI_TEST_TERMINAL_CMD_CLICK_SETUP"] = "1"
        app.launchEnvironment["NORI_UI_TEST_TERMINAL_CMD_CLICK_PATH"] = setupDataPath
        app.launchEnvironment["NORI_UI_TEST_TERMINAL_CMD_CLICK_COMMAND_PATH"] = commandPath
        app.launchEnvironment["NORI_UI_TEST_TERMINAL_CMD_CLICK_FIXTURE_DIR"] = fixtureDirectoryURL.path
        app.launchEnvironment["NORI_UI_TEST_TERMINAL_CMD_CLICK_FILE_NAME"] = fileName
        app.launchEnvironment["NORI_UI_TEST_TERMINAL_CMD_CLICK_DISPLAY_MODE"] = displayMode.rawValue
        app.launchEnvironment["NORI_UI_TEST_TERMINAL_CMD_CLICK_LINE_FORMAT"] = lineFormat.rawValue
        if !linePrefix.isEmpty {
            app.launchEnvironment["NORI_UI_TEST_TERMINAL_CMD_CLICK_LINE_PREFIX"] = linePrefix
        }
        if !extraFileNames.isEmpty,
           let data = try? JSONSerialization.data(withJSONObject: extraFileNames, options: [.sortedKeys]),
           let json = String(data: data, encoding: .utf8) {
            app.launchEnvironment["NORI_UI_TEST_TERMINAL_CMD_CLICK_EXTRA_FILE_NAMES_JSON"] = json
        }
        if captureOpenPaths {
            app.launchEnvironment["NORI_UI_TEST_CAPTURE_OPEN_PATH"] = openCapturePath
        }
        if captureHoverDiagnostics {
            app.launchEnvironment["NORI_UI_TEST_CMD_HOVER_DIAGNOSTICS_PATH"] = hoverDiagnosticsPath
        }
        if let quicklookOverride {
            app.launchEnvironment["NORI_UI_TEST_TERMINAL_CMD_CLICK_QUICKLOOK_OVERRIDE"] = quicklookOverride
        }
        if let viewportOffsetDelta {
            app.launchEnvironment["NORI_UI_TEST_TERMINAL_CMD_CLICK_VIEWPORT_OFFSET_DELTA"] = String(viewportOffsetDelta)
        }
