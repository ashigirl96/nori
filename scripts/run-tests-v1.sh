...
# This runner is intended for the UTM macOS VM (ssh nori-vm).
# It is intentionally guarded so we don't accidentally kill the host user's nori instances.
if [ "$(id -un)" != "nori" ]; then
  echo "ERROR: This script is intended to be run on the nori-vm (user: nori)." >&2
  echo "Run via: ssh nori-vm 'cd /Users/nori/GhosttyTabs && ./scripts/run-tests-v1.sh'" >&2
  exit 2
fi

cd "$(dirname "$0")/.."

DERIVED_DATA_PATH="$HOME/Library/Developer/Xcode/DerivedData/nori-tests-v1"
APP="$DERIVED_DATA_PATH/Build/Products/Debug/nori DEV.app"
...
if [ ! -d "$APP" ]; then
  echo "ERROR: nori DEV.app not found at expected path: $APP" >&2
  exit 1
fi

cleanup() {
  pkill -x "nori DEV" || true
  pkill -x "nori" || true
  rm -f /tmp/nori*.sock || true
}
...
launch_and_wait() {
  ...
  for _ in {1..50}; do
    pgrep -x "nori DEV" >/dev/null 2>&1 || break
    sleep 0.1
  done

  # Force socket mode for deterministic automation runs, independent of prior user settings.
  defaults write com.nori.app.debug socketControlMode -string full >/dev/null 2>&1 || true

  # Launch directly with UI test mode enabled so startup follows deterministic test codepaths.
  NORI_UI_TEST_MODE=1 "$APP/Contents/MacOS/nori DEV" >/dev/null 2>&1 &

  SOCK=""
  for _ in {1..120}; do
    SOCK=$(ls -t /tmp/nori-debug*.sock /tmp/nori*.sock 2>/dev/null | head -1 || true)
  ...
  if [ -z "$SOCK" ] || [ ! -S "$SOCK" ]; then
    echo "ERROR: Socket not ready (looked for /tmp/nori*.sock)" >&2
    exit 1
  fi
  export NORI_SOCKET_PATH="$SOCK"
  export NORI_SOCKET="$SOCK"
...
from nori import nori  # type: ignore
