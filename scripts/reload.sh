...
Builds the Debug configuration of cmux and (optionally) launches it.
...
# DerivedData directories from the old tagged workflow (cmux-<tag>/) could
...
XCODE_LOG="/tmp/cmux-xcodebuild.log"
set +e
...
APP_PATH="$DERIVED_DATA/Build/Products/Debug/cmux DEV.app"
if [[ ! -d "$APP_PATH" ]]; then
  echo "error: cmux DEV.app not found at $APP_PATH" >&2
  exit 1
fi

# cmuxd has no Xcode build phase, so we build and copy it in ourselves.
...
if [[ "$LAUNCH" -eq 1 ]]; then
  ...
  /usr/bin/osascript -e 'tell application id "com.cmuxterm.app.debug" to quit' >/dev/null 2>&1 || true
  # Exact-name match on the Debug executable ("cmux DEV") so we don't also
  # terminate the Release /Applications/cmux.app process. Matches the CI jobs
  # in .github/workflows/ci.yml.
  pkill -x "cmux DEV" 2>/dev/null || true
  for _ in $(seq 1 40); do
    pgrep -x "cmux DEV" >/dev/null || break
    sleep 0.05
  done

  # Scrub parent-cmux environment variables so the child doesn't inherit a
