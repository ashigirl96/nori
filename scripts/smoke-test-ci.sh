...
SOCKET_PATH="/tmp/nori-debug.sock"
STABILITY_WAIT=15

echo "=== Smoke Test ==="

# --- Find the built app ---
APP=$(find ~/Library/Developer/Xcode/DerivedData -path "*/Build/Products/Debug/nori DEV.app" -print -quit 2>/dev/null || true)
if [ -z "$APP" ]; then
  echo "ERROR: Built app not found in DerivedData"
  exit 1
fi
echo "App: $APP"
BINARY="$APP/Contents/MacOS/nori DEV"
if [ ! -x "$BINARY" ]; then
...
echo "Launching app..."
NORI_SOCKET_MODE=allowAll NORI_UI_TEST_MODE=1 "$BINARY" > /tmp/nori-smoke-stdout.log 2>&1 &
APP_PID=$!
...
if ! kill -0 "$APP_PID" 2>/dev/null; then
  ...
  echo "--- stdout/stderr ---"
  cat /tmp/nori-smoke-stdout.log 2>/dev/null | tail -50 || true
  echo "--- debug log ---"
  tail -50 /tmp/nori-debug.log 2>/dev/null || true
  echo "--- crash reports ---"
  ls -lt ~/Library/Logs/DiagnosticReports/*nori* 2>/dev/null | head -5 || echo "(none)"
  exit 1
fi
...
if [ "$SOCKET_READY" != "true" ]; then
  ...
  echo "--- stdout/stderr ---"
  cat /tmp/nori-smoke-stdout.log 2>/dev/null | tail -30 || true
  echo "--- debug log ---"
  tail -30 /tmp/nori-debug.log 2>/dev/null || true
  ls -la /tmp/nori-debug* 2>/dev/null || true
  pgrep -la "nori" || echo "No nori processes found"
  exit 1
fi
