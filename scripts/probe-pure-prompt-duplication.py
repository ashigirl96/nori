...
Run this from a spare nori pane. The script creates a temporary workspace,
...
def _prompt_block(text: str) -> tuple[list[str], str]:
    ...
    for i in range(len(lines) - 1, -1, -1):
        if _is_prompt_line(lines[i]):
            ...
            break
    if prompt_idx == -1:
        raise noriError(f"Could not find prompt line in surface text:\n{text}")
...
def _read_text(client: nori, workspace_id: str, surface_id: str) -> str:
...
def main() -> int:
    ...
    parser.add_argument(
        "--socket",
        default=os.environ.get("NORI_SOCKET") or os.environ.get("NORI_SOCKET_PATH") or "/tmp/nori-debug.sock",
    )
    ...
    with nori(args.socket) as client:
        ...
        if not original_workspace_id:
            raise noriError(f"workspace.current returned no workspace_id: {current}")

        created = client._call("workspace.create", {}) or {}
        workspace_id = str(created.get("workspace_id") or "")
        if not workspace_id:
            raise noriError(f"workspace.create returned no workspace_id: {created}")
        ...
        while True:
            try:
                ...
                if surface_id:
                    ...
                    probe_text = baseline
                    break
                raise noriError("surface not ready yet")
            except Exception as exc:
                probe_text = str(exc)
                if time.time() - start > 10:
                    raise noriError(f"Timed out waiting for readable terminal surface: {probe_text}")
                time.sleep(0.2)
