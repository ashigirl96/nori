...
## Goal
Ensure commands run through the nori Unix socket/CLI do not steal user focus from the current UI workflow.
...
- [x] Add explicit `rename-tab` command (defaults to `NORI_TAB_ID` / `NORI_SURFACE_ID` / `NORI_WORKSPACE_ID` when flags omitted).
