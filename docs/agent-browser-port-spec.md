...
This document tracks implemented behavior and remaining parity gaps for the nori browser port.

## Goals

1. Provide an LLM-friendly browser automation API in nori with stable handles.
...
1. `./scripts/run-tests-v1.sh` passes on `nori-vm`.
2. `./scripts/run-tests-v2.sh` passes on `nori-vm`.
...
6. `browser.tab.*` compatibility aliases mapped to nori surfaces
7. `browser.console.list`
8. `browser.errors.list`
9. `browser.highlight`
10. `browser.state.save|load` (browser state in nori context)
...
Primary form:
```bash
nori browser --surface <surface-id> <agent-browser-style-command...>
```

Shorthand:
```bash
nori browser <surface-id> <agent-browser-style-command...>
```
...
nori identify
nori capabilities
nori browser identify --surface <surface-id>   # wrapper over system.identify + browser fields
```

Flash:
```bash
nori trigger-flash [--workspace <id>] [--surface <id>]
```
...
- [x] Add `browser` command group in `CLI/nori.swift` that accepts agent-browser-style command grammar.
...
3. `src/protocol.test.ts` -> adapted browser command/shape validation in `tests_v2/test_browser_api_unsupported_matrix.py` and existing `CLI/nori.swift` command grammar checks.
...
5. `src/daemon.test.ts`, `src/stream-server.test.ts`, `test/serverless.test.ts`, `src/ios-manager.test.ts` -> out-of-scope for nori browser parity (different transport/runtime).

### Implemented nori Browser Suites
...
1. `ssh nori-vm 'cd /Users/nori/GhosttyTabs && ./scripts/run-tests-v2.sh'`
2. `ssh nori-vm 'cd /Users/nori/GhosttyTabs && ./scripts/run-tests-v1.sh'`

## Decision Log (Locked - February 12, 2026)

1. `nori browser tab ...` maps to browser `surface` tabs only (no separate workspace-level tab meaning inside `browser` namespace).
...
5. Output shape is nori-native overall, but `browser.snapshot` and selector `not_found` diagnostics intentionally mirror agent-browser semantics for agent usability.
...
2. Whether to expose protocol-only agent-browser actions in first public release of `nori browser` or gate them behind a second rollout phase.
