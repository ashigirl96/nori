...
Tracking issue: https://github.com/manaflow-ai/nori/issues/151
Primary PR: https://github.com/manaflow-ai/nori/pull/1296
CLI relay PR: https://github.com/manaflow-ai/nori/pull/374
...
- `DONE` `nori ssh` creates remote-tagged workspaces and does not require `--name`.
- `DONE` scoped shell niceties are applied only for `nori ssh` launches.
...
- `DONE` local app probes remote platform, verifies a release-pinned `norid-remote` artifact by embedded manifest SHA-256, uploads it when missing, and runs `serve --stdio`.
...
- `DONE` bootstrap installs `~/.nori/bin/nori` wrapper (also tries `/usr/local/bin/nori`) so `nori` is available in PATH on the remote.

### 3.5 CLI Relay (Running nori Commands From Remote)
- `DONE` `norid-remote` includes a table-driven CLI relay (`cli` subcommand) that maps CLI args to v1 text or v2 JSON-RPC messages.
- `DONE` busybox-style argv[0] detection: when invoked as `nori` via wrapper/symlink, auto-dispatches to CLI relay.
...
- `DONE` relay address written to `~/.nori/socket_addr` on the remote only after the reverse forward survives startup validation.
- `DONE` Go CLI no longer polls for relay readiness. It dials the published relay once and only refreshes `~/.nori/socket_addr` a single time to recover from a stale shared address rewrite.
- `DONE` `nori ssh` startup exports session-local `NORI_SOCKET_PATH=127.0.0.1:<relay_port>` so parallel sessions pin to their own relay instead of racing on shared socket_addr.
- `DONE` relay startup writes `~/.nori/relay/<relay_port>.daemon_path`; remote `nori` wrapper uses this to select the right daemon binary per session, including mixed local nori versions.
- `DONE` relay startup writes `~/.nori/relay/<relay_port>.auth` with a relay ID and token; the local relay requires HMAC-SHA256 challenge-response before forwarding any command to the real local socket.
...
### 3.6 Artifact Trust
- `DONE` release and nightly workflows publish `norid-remote` assets for `darwin/linux × arm64/amd64`.
- `DONE` release and nightly apps embed a compact `NORIRemoteDaemonManifestJSON` in `Info.plist` with exact asset URLs and SHA-256 digests.
- `DONE` `nori remote-daemon-status` exposes the current manifest entry, local cache verification state, release download command, and GitHub attestation verification command.
...
1. `DONE` `norid-remote` now supports proxy stream RPC (`proxy.open`, `proxy.close`, `proxy.write`, `proxy.stream.subscribe`) with pushed `proxy.stream.data/eof/error` events.
...
|---|---|---|---|
| M-001 | `nori ssh` workspace creation + metadata + optional `--name` | DONE | Covered by `tests_v2/test_ssh_remote_cli_metadata.py` |
...
| M-004b | CLI relay: run nori commands from within SSH sessions | DONE | Reverse TCP forward + Go CLI relay + bootstrap wrapper |
...
| M-007 | Remote proxy stream RPC in `norid-remote` | DONE | `proxy.open/close/write/proxy.stream.subscribe` plus pushed stream events implemented |
...
| ID | Scenario | Status |
|---|---|---|
| C-001 | `nori ping` from remote session | DONE |
| C-002 | `nori list-workspaces --json` from remote | DONE |
| C-003 | `nori new-workspace` from remote | DONE |
| C-004 | `nori rpc system.capabilities` passthrough | DONE |
...
1. `NORI_SSH_TEST_DOCKER_HOST` sets the SSH destination host/IP used by docker-backed SSH fixtures (default `127.0.0.1`).
2. `NORI_SSH_TEST_DOCKER_BIND_ADDR` sets the bind address used in fixture container publish mappings (default `127.0.0.1`).
