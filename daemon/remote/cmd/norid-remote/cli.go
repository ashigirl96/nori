...
// runCLI is the entry point for the "cli" subcommand (or busybox "nori" invocation).
func runCLI(args []string) int {
	socketPath := os.Getenv("NORI_SOCKET_PATH")
	...
	for i := 0; i < len(args); i++ {
		...
		case "--socket":
			if i+1 >= len(args) {
				fmt.Fprintln(os.Stderr, "nori: --socket requires a path")
				return 2
			}
			socketPath = args[i+1]
			i++
...
doneFlags:
	...
	if socketPath == "" {
		fmt.Fprintln(os.Stderr, "nori: NORI_SOCKET_PATH not set and --socket not provided")
		return 1
	}
	...
	if !ok {
		fmt.Fprintf(os.Stderr, "nori: unknown command %q\n", cmdName)
		return 2
	}
	...
	default:
		fmt.Fprintf(os.Stderr, "nori: internal error: unknown protocol for %q\n", cmdName)
		return 1
	}
}
...
func runRPC(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "nori rpc: requires a method name")
		return 2
	}
	method := args[0]
	...
	if len(args) > 1 {
		if err := json.Unmarshal([]byte(args[1]), &params); err != nil {
			fmt.Fprintf(os.Stderr, "nori rpc: invalid JSON params: %v\n", err)
			return 2
		}
	}
...
// runBrowserRelay handles "nori browser <subcommand>" by mapping to browser.* v2 methods.
func runBrowserRelay(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "nori browser: requires a subcommand (open, navigate, back, forward, reload, get-url)")
		return 2
	}
	...
	case "get-url":
		...
		useSurfaceEnv = true
	default:
		fmt.Fprintf(os.Stderr, "nori browser: unknown subcommand %q\n", sub)
		return 2
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nori browser: %v\n", err)
		return 2
	}
...
func applyWorkspaceEnvFallback(params map[string]any) {
	...
	if envWs := os.Getenv("NORI_WORKSPACE_ID"); envWs != "" {
...
func applySurfaceEnvFallback(params map[string]any) {
	...
	if envSf := os.Getenv("NORI_SURFACE_ID"); envSf != "" {
...
// readSocketAddrFile reads the socket address from ~/.nori/socket_addr as a fallback
// when NORI_SOCKET_PATH is not set. Written by the nori app after the relay establishes.
func readSocketAddrFile() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	data, err := os.ReadFile(filepath.Join(home, ".nori", "socket_addr"))
	if err != nil {
		return ""
	}
...
func readRelayAuthFile(socketPath string) *relayAuthState {
	if strings.Contains(socketPath, ":") && !strings.HasPrefix(socketPath, "/") {
		...
		data, err := os.ReadFile(filepath.Join(home, ".nori", "relay", port+".auth"))
		if err != nil {
			return nil
		}
...
func currentRelayAuth(socketPath string) *relayAuthState {
	relayID := strings.TrimSpace(os.Getenv("NORI_RELAY_ID"))
	relayToken := strings.TrimSpace(os.Getenv("NORI_RELAY_TOKEN"))
...
// dialSocket connects to the nori socket. If addr contains a colon and doesn't
...
func authenticateRelayConn(conn net.Conn, auth *relayAuthState) error {
	...
	if challenge.Protocol != "nori-relay-auth" || challenge.Version != 1 || challenge.RelayID != auth.RelayID || challenge.Nonce == "" {
...
func cliUsage() {
	fmt.Fprintln(os.Stderr, "Usage: nori [--socket <path>] [--json] <command> [args...]")
	...
	fmt.Fprintln(os.Stderr, "  omo [args...]              Launch OpenCode with nori integration")
	fmt.Fprintln(os.Stderr, "  omx [args...]              Launch Oh My Codex with nori integration")
	fmt.Fprintln(os.Stderr, "  omc [args...]              Launch Oh My Claude Code with nori integration")
