...
func TestTmuxWaitForSignalPath(t *testing.T) {
	...
	if !strings.HasPrefix(path, "/tmp/nori-wait-for-") {
...
func TestConfigureAgentEnvironment(t *testing.T) {
	...
	envKeys := []string{
		"NORI_CLAUDE_TEAMS_NORI_BIN", "PATH", "TMUX", "TMUX_PANE",
		"TERM", "NORI_SOCKET_PATH", "NORI_SOCKET", "TERM_PROGRAM",
		"NORI_WORKSPACE_ID", "NORI_SURFACE_ID",
	...
	// Verify socket path
	if os.Getenv("NORI_SOCKET_PATH") != "127.0.0.1:54321" {
		t.Errorf("NORI_SOCKET_PATH = %q", os.Getenv("NORI_SOCKET_PATH"))
	}
	...
	if os.Getenv("NORI_WORKSPACE_ID") != "ws-abc" {
		t.Errorf("NORI_WORKSPACE_ID = %q", os.Getenv("NORI_WORKSPACE_ID"))
	}
	if os.Getenv("NORI_SURFACE_ID") != "surf-789" {
		t.Errorf("NORI_SURFACE_ID = %q", os.Getenv("NORI_SURFACE_ID"))
	}
	// Verify extra env
