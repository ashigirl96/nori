...
func shouldRunCLIForInvocation(argv0 string, args []string) bool {
	...
	if !strings.HasPrefix(base, "norid-remote") || len(args) == 0 {
		return false
	}
...
func usage(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "  norid-remote version")
	_, _ = fmt.Fprintln(w, "  norid-remote serve --stdio")
	_, _ = fmt.Fprintln(w, "  norid-remote cli <command> [args...]")
}
