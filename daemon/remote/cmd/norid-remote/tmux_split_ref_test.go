...
func TestTmuxSplitWindowCanonicalizesCallerSurfaceRefs(t *testing.T) {
	...
	origWorkspace := os.Getenv("NORI_WORKSPACE_ID")
	origSurface := os.Getenv("NORI_SURFACE_ID")
	origPane := os.Getenv("TMUX_PANE")
	os.Setenv("HOME", t.TempDir())
	os.Setenv("NORI_WORKSPACE_ID", "workspace:1")
	os.Setenv("NORI_SURFACE_ID", "surface:1")
	os.Setenv("TMUX_PANE", "%pane:1")
	defer func() {
		os.Setenv("HOME", origHome)
		if origWorkspace != "" {
			os.Setenv("NORI_WORKSPACE_ID", origWorkspace)
		} else {
			os.Unsetenv("NORI_WORKSPACE_ID")
		}
		if origSurface != "" {
			os.Setenv("NORI_SURFACE_ID", origSurface)
		} else {
			os.Unsetenv("NORI_SURFACE_ID")
		}
		if origPane != "" {
...
func TestTmuxSplitWindowIgnoresStaleUUIDColumnSurface(t *testing.T) {
	...
	storePath := filepath.Join(home, ".noriterm", "tmux-compat-store.json")
