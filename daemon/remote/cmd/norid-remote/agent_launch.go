...
const claudeNodeOptionsRestoreModuleScript = `const hadOriginalNodeOptions = process.env.NORI_ORIGINAL_NODE_OPTIONS_PRESENT === "1";
...
// runClaudeTeamsRelay implements `nori claude-teams` on the remote side.
...
func runClaudeTeamsRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nori claude-teams: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	configureAgentEnvironment(agentConfig{
		...
		termEnvVar:     "NORI_CLAUDE_TEAMS_TERM",
	...
	if claudePath == "" {
		fmt.Fprintf(os.Stderr, "nori claude-teams: claude not found in PATH\n")
		return 1
	}
	argv := append([]string{claudePath}, launchArgs...)
	execErr := syscall.Exec(claudePath, argv, os.Environ())
	fmt.Fprintf(os.Stderr, "nori claude-teams: exec failed: %v\n", execErr)
	return 1
}

// runOMORelay implements `nori omo` on the remote side.
func runOMORelay(socketPath string, args []string, refreshAddr func() string) int {
	rc := &rpcContext{socketPath: socketPath, refreshAddr: refreshAddr}

	shimDir, err := createOMOShimDir()
	if err != nil {
		fmt.Fprintf(os.Stderr, "nori omo: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	if opencodePath == "" {
		fmt.Fprintf(os.Stderr, "nori omo: opencode not found in PATH\n"+
	...
	if err := omoEnsurePlugin(originalPath); err != nil {
		fmt.Fprintf(os.Stderr, "nori omo: plugin setup: %v\n", err)
		return 1
	}
	...
	fmt.Fprintf(os.Stderr, "nori omo: exec failed: %v\n", execErr)
	return 1
}

// runOMXRelay implements `nori omx` on the remote side.
func runOMXRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nori omx: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	if omxPath == "" {
		fmt.Fprintf(os.Stderr, "nori omx: omx not found in PATH\n"+
	...
	configureAgentEnvironment(agentConfig{
		...
		termEnvVar:     "NORI_OMX_TERM",
		extraEnv:       map[string]string{},
	})

	launchPath, launchArgv := resolveNodeScriptExec(omxPath, args, originalPath, shimDir)
	execErr := syscall.Exec(launchPath, launchArgv, os.Environ())
	fmt.Fprintf(os.Stderr, "nori omx: exec failed: %v\n", execErr)
	return 1
}

// runOMCRelay implements `nori omc` on the remote side.
func runOMCRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nori omc: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	if omcPath == "" {
		fmt.Fprintf(os.Stderr, "nori omc: omc not found in PATH\n"+
	...
	if restoreModulePath, err := ensureClaudeNodeOptionsRestoreModule(); err == nil {
		...
		fmt.Fprintf(os.Stderr, "nori omc: warning: failed to create NODE_OPTIONS restore module: %v\n", err)
	}

	launchPath, launchArgv := resolveNodeScriptExec(omcPath, args, originalPath, shimDir)
	execErr := syscall.Exec(launchPath, launchArgv, os.Environ())
	fmt.Fprintf(os.Stderr, "nori omc: exec failed: %v\n", execErr)
	return 1
}
...
func createTmuxShimDir(dirName string, tmuxScript string) (string, error) {
	...
	dir := filepath.Join(home, ".noriterm", dirName)
...
func ensureClaudeNodeOptionsRestoreModule() (string, error) {
	dir := filepath.Join(os.TempDir(), "nori-claude-node-options")
	if err := os.MkdirAll(dir, 0755); err != nil {
		return "", err
	}
...
func configureClaudeNodeOptions(restoreModulePath string) {
	...
	if hadExisting {
		os.Setenv("NORI_ORIGINAL_NODE_OPTIONS_PRESENT", "1")
		os.Setenv("NORI_ORIGINAL_NODE_OPTIONS", existing)
	} else {
		os.Setenv("NORI_ORIGINAL_NODE_OPTIONS_PRESENT", "0")
		os.Unsetenv("NORI_ORIGINAL_NODE_OPTIONS")
	}
...
func configureAgentEnvironment(cfg agentConfig) {
	...
	os.Setenv(cfg.noriBinEnvVar, selfPath)
	...
	// Socket path
	os.Setenv("NORI_SOCKET_PATH", cfg.socketPath)
	os.Setenv("NORI_SOCKET", cfg.socketPath)
	...
	if cfg.focused != nil {
		os.Setenv("NORI_WORKSPACE_ID", cfg.focused.workspaceId)
		if cfg.focused.surfaceId != "" {
			os.Setenv("NORI_SURFACE_ID", cfg.focused.surfaceId)
		}
	}
...
func omoShadowConfigDir() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".noriterm", "omo-config")
}
