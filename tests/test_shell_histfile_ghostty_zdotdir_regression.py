...
nori also injects its own zsh integration by setting ZDOTDIR to
Resources/shell-integration. If nori incorrectly treats Ghostty's injected
...
def main() -> int:
    ...
    nori_wrapper_dir = root / "Resources" / "shell-integration"
    ghostty_zsh_dir = root / "ghostty" / "src" / "shell-integration" / "zsh"

    if not (nori_wrapper_dir / ".zshenv").exists():
        print(f"SKIP: missing nori wrapper .zshenv at {nori_wrapper_dir}")
        return 0
    if not (ghostty_zsh_dir / ".zshenv").exists():
        print(f"SKIP: missing Ghostty zsh .zshenv at {ghostty_zsh_dir}")
        return 0

    base = Path("/tmp") / f"nori_histfile_ghostty_stack_{os.getpid()}"
    try:
        ...
        # Simulate the buggy situation: nori stores Ghostty's injected ZDOTDIR
        # as the "original" ZDOTDIR, then sets ZDOTDIR to its own wrapper.
        env["NORI_ORIGINAL_ZDOTDIR"] = str(ghostty_zsh_dir)
        env["ZDOTDIR"] = str(nori_wrapper_dir)
        env["NORI_SHELL_INTEGRATION"] = "0"
        ...
        if seen != expected:
            ...
            print(f"  nori_wrapper_dir={nori_wrapper_dir}")
