#!/usr/bin/env python3
"""
Layout/flash regression tests for nori splits.
...
def _assert_selected_panels_healthy(payload: dict, *, min_wh: float = 80.0) -> None:
    ...
    if not selected:
        raise noriError("layout_debug returned no selectedPanels")
    ...
    for i, row in enumerate(selected):
        ...
        if not panel_id:
            raise noriError(f"selectedPanels[{i}] missing panelId (pane={pane_id})")

        if row.get("inWindow") is not True:
            raise noriError(f"selectedPanels[{i}] panel not in window (pane={pane_id}, panel={panel_id})")

        if row.get("hidden") is True:
            raise noriError(f"selectedPanels[{i}] panel is hidden (pane={pane_id}, panel={panel_id})")

        if not view_frame:
            raise noriError(f"selectedPanels[{i}] missing viewFrame (pane={pane_id}, panel={panel_id})")
...
def main() -> int:
    with nori(SOCKET_PATH) as c:
        ...
        if flashes != 0:
            raise noriError(f"EmptyPanelView appeared during split (count={flashes})")
        ...
        if len(panes) < 2:
            raise noriError(f"Expected >= 2 panes after split, got {len(panes)}")
        ...
        if not drag_selected:
            raise noriError("layout_debug returned no selectedPanels for drag split setup")
        drag_panel_id = drag_selected[0].get("panelId")
        if not drag_panel_id:
            raise noriError("drag split setup selected panel has no panelId")
        ...
        flashes = c.empty_panel_count()
        if flashes != 0:
            raise noriError(f"EmptyPanelView appeared during drag split (count={flashes})")
        ...
        if not browser_id.startswith("OK "):
            raise noriError(browser_id)
        time.sleep(0.4)
        flashes = c.empty_panel_count()
        if flashes != 0:
            raise noriError(f"EmptyPanelView appeared during browser split (count={flashes})")
