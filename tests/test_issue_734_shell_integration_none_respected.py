...
nori wrapper .zshenv should only source Ghostty zsh integration when Ghostty
...
def _run_case(
    *,
    ...
    env["NORI_TEST_OUT"] = str(out_path)

    # Keep input deterministic and local to this test.
    for key in (
        "GHOSTTY_ZSH_ZDOTDIR",
        "NORI_ZSH_ZDOTDIR",
        "NORI_ORIGINAL_ZDOTDIR",
    ...
    if ghostty_enabled:
        ...
        env["NORI_ZSH_ZDOTDIR"] = str(orig_zdotdir)
...
def main() -> int:
    ...
    base = Path("/tmp") / f"nori_issue_734_{os.getpid()}"
    try:
