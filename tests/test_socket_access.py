...
In noriOnly mode (default), only processes descended from the nori
app process can connect. External processes (e.g., SSH) are rejected.

Test strategy:
  Phase 1: noriOnly — external processes get rejected
  Phase 2: noriOnly — internal process CAN connect (inject via shell rc)
...
def _preferred_worktree_slug():
    env_slug = os.environ.get("NORI_TAG") or os.environ.get("NORI_BRANCH_SLUG")
    if env_slug:
...
def _derived_app_candidates_for_current_worktree():
    ...
    for info_path in info_paths:
        ...
        app_path = os.path.join(derived_root, "Build/Products/Debug/nori DEV.app")
...
def _find_app():
    explicit = os.environ.get("NORI_APP_PATH")
    ...
    if preferred_slug:
        preferred_tmp = []
        preferred_tmp.extend(glob.glob(f"/tmp/nori-{preferred_slug}/Build/Products/Debug/nori DEV*.app"))
        preferred_tmp.extend(glob.glob(f"/private/tmp/nori-{preferred_slug}/Build/Products/Debug/nori DEV*.app"))
    ...
    derived_candidates = glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/nori DEV.app"
    ))
    tmp_candidates = []
    tmp_candidates.extend(glob.glob("/tmp/nori-*/Build/Products/Debug/nori DEV*.app"))
    tmp_candidates.extend(glob.glob("/private/tmp/nori-*/Build/Products/Debug/nori DEV*.app"))
...
def _find_cli(preferred_app_path: str = ""):
    ...
    if preferred_app_path:
        ...
        sibling = os.path.join(debug_dir, "nori")
    ...
    candidates.extend(glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/nori"
    )))
    candidates.extend(glob.glob("/tmp/nori-*/Build/Products/Debug/nori"))
    candidates.extend(glob.glob("/private/tmp/nori-*/Build/Products/Debug/nori"))
...
def _kill_nori(app_path: str = None):
    if app_path:
        exe = os.path.join(app_path, "Contents/MacOS/nori DEV")
        subprocess.run(["pkill", "-f", exe], capture_output=True)
    else:
        subprocess.run(["pkill", "-x", "nori DEV"], capture_output=True)
    time.sleep(1.5)


def _launch_nori(app_path: str, socket_path: str, mode: str = None, extra_env: dict = None):
    if os.path.exists(socket_path):
        try:
    ...
    env_args = []
    if mode:
        env_args = ["--env", f"NORI_SOCKET_MODE={mode}"]
    launch_env = {
        "NORI_SOCKET_PATH": socket_path,
...
def test_internal_process_allowed(socket_path: str, app_path: str) -> TestResult:
    """
    Verify a nori-spawned terminal process CAN connect in noriOnly mode.
    Inject a test via the shell rc file, then launch nori in noriOnly mode.
    The shell (a descendant of nori) runs the test on startup.
    """
    result = TestResult("Internal process can connect (noriOnly)")
    marker = os.path.join(tempfile.gettempdir(), f"nori_internal_{os.getpid()}")
    hook_file = os.path.join(tempfile.gettempdir(), f"nori_rc_hook_{os.getpid()}.sh")
...
""")
        ...
        # Kill existing nori, launch in noriOnly mode (default)
        _kill_nori(app_path)
        _launch_nori(app_path, socket_path, mode="noriOnly")
        ...
        if content == "OK":
            result.success("Internal process pinged socket successfully in noriOnly mode")
        else:
...
def test_allowall_mode_works(socket_path: str, app_path: str) -> TestResult:
    """Verify NORI_SOCKET_MODE=allowAll bypasses ancestry check."""
    result = TestResult("allowAll mode allows external")
    try:
        _kill_nori(app_path)
        _launch_nori(app_path, socket_path, mode="allowAll")
...
def test_password_mode_requires_auth(socket_path: str, app_path: str) -> TestResult:
    ...
    password = f"nori-pass-{os.getpid()}"
    try:
        _kill_nori(app_path)
        _launch_nori(
            app_path,
            socket_path,
            mode="password",
            extra_env={"NORI_SOCKET_PASSWORD": password}
        )
...
def test_password_mode_cli_exit_code(socket_path: str, app_path: str) -> TestResult:
    ...
    try:
        ...
        if not cli_path:
            result.failure("Could not find nori CLI binary")
            return result
...
def run_tests():
    print("=" * 60)
    print("nori Socket Access Control Tests")
    print("=" * 60)
    print()

    app_path = _find_app()
    if not app_path:
        print("Error: Could not find nori DEV.app in DerivedData")
        return 1
    print(f"App: {app_path}")

    socket_path = f"/tmp/nori-test-socket-access-{os.getpid()}.sock"
    try:
        os.unlink(socket_path)
    except OSError:
        pass
    ...
    # ── Phase 1: noriOnly — external rejection ──
    print("Phase 1: noriOnly mode — external rejection")
    print("-" * 50)

    # Ensure nori is running in noriOnly mode
    _kill_nori(app_path)
    print("  Launching nori in noriOnly mode...")
    _launch_nori(app_path, socket_path, mode="noriOnly")
    ...
    # ── Phase 2: noriOnly — internal process CAN connect ──
    print("Phase 2: noriOnly mode — internal process allowed")
    print("-" * 50)
    ...
    # ── Cleanup: leave nori in noriOnly mode ──
    _kill_nori(app_path)
    _launch_nori(app_path, socket_path, mode="noriOnly")
