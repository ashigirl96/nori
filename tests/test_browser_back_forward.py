...
def focused_pane_id(client: nori) -> Optional[str]:
...
def get_browser_url(client: nori, panel_id: str) -> str:
    """Get the current URL of a browser panel."""
    return client._send_command(f"get_url {panel_id}").strip()


def navigate_browser(client: nori, panel_id: str, url: str) -> None:
...
def wait_for_url(client: nori, panel_id: str, expected_url: str,
...
def test_cmd_bracket_back_forward(client: nori) -> tuple[bool, str]:
    """
...
def test_cmd_bracket_noop_on_terminal(client: nori) -> tuple[bool, str]:
    """
...
def test_browser_back_forward_socket_commands(client: nori) -> tuple[bool, str]:
    """
