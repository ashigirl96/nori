...
def main() -> int:
    with nori(SOCKET_PATH) as c:
        ...
        if len(terminals) < 2:
            raise noriError(f"expected >=2 terminal surfaces after split, got={terminals}")

        for row in terminals:
            if not row.get("in_window", False):
                raise noriError(f"terminal not attached to window: {row}")
            if row.get("portal") is not True:
                raise noriError(f"terminal is not portal-hosted: {row}")
            depth = row.get("view_depth")
            if not isinstance(depth, int):
                raise noriError(f"missing view_depth in surface_health: {row}")
            if depth > 8:
                raise noriError(f"terminal view depth too deep ({depth}): {row}")
