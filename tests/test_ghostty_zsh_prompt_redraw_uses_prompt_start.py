...
typeset -gi _nori_redraw_done=0
typeset -g _nori_redraw_fd=''
...
_nori_redraw_ready() {
  emulate -L zsh
  local fd="${1:-$_nori_redraw_fd}"
  if [[ -n "$fd" ]]; then
    zle -F "$fd"
    exec {fd}<&-
  fi
  _nori_redraw_fd=''
  (( _nori_redraw_done )) && return 0
  _nori_redraw_done=1
  zle reset-prompt
}

_nori_redraw_line_init() {
  if (( !_nori_redraw_done )) && [[ -z "$_nori_redraw_fd" ]]; then
    exec {_nori_redraw_fd}< <(
      sleep 0.05
      printf 'ready\\n'
    )
    zle -F "$_nori_redraw_fd" _nori_redraw_ready
  fi
}

add-zsh-hook precmd _nori_redraw_precmd
zle -N zle-line-init _nori_redraw_line_init
""".lstrip(),
        encoding="utf-8",
    )
...
def main() -> int:
    ...
    base = Path(tempfile.mkdtemp(prefix="nori_ghostty_prompt_redraw_"))
    try:
        home = base / "home"
