...
nori can launch shells with PATH initially pointing at app resources. If replay
...
def main() -> int:
    ...
    integration_script = root / "Resources" / "shell-integration" / "nori-zsh-integration.zsh"
    if not integration_script.exists():
        print(f"SKIP: missing zsh integration script at {integration_script}")
        return 0

    base = Path("/tmp") / f"nori_scrollback_restore_{os.getpid()}"
    try:
        ...
        env["NORI_RESTORE_SCROLLBACK_FILE"] = str(replay_file)
        env["NORI_TEST_INTEGRATION_SCRIPT"] = str(integration_script)

        result = subprocess.run(
            ["/bin/zsh", "-f", "-c", 'source "$NORI_TEST_INTEGRATION_SCRIPT"'],
            env=env,
            capture_output=True,
            text=True,
            timeout=5,
