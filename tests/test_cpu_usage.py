#!/usr/bin/env python3
"""
CPU usage test for nori.

This test monitors nori's CPU usage during idle periods to catch
performance regressions like runaway animations or continuous view updates.

Run this test after launching nori:
...
# Allow importing tests/nori.py when running from repo root.
...
def main():
    ...
    if pid is None:
        ...
        print("Start nori and run this test again.")
    ...
    if avg_cpu > MAX_IDLE_CPU_PERCENT:
        ...
        sample_file = Path(f"/tmp/nori_cpu_test_sample_{pid}.txt")
        ...
        relevant_lines = [
            l for l in lines
            if "nori" in l and ("body" in l or "Animation" in l or "Timer" in l)
        ][:10]
