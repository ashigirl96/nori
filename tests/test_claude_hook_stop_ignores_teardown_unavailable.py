        ...
        try:
            ...
            with conn:
                ...
                if not self.commands:
                    raise RuntimeError("nori CLI never sent a command to the teardown test socket")
...
def main() -> int:
    ...
    temp_dir = tempfile.TemporaryDirectory(prefix="nori-claude-hook-stop-")
    try:
        root = Path(temp_dir.name)
        socket_path = str(root / "nori.sock")
        ...
        env["NORI_WORKSPACE_ID"] = str(uuid.uuid4())
        env["NORI_SURFACE_ID"] = str(uuid.uuid4())
        env["NORI_CLAUDE_HOOK_STATE_PATH"] = str(state_path)
        env["NORI_CLI_SENTRY_DISABLED"] = "1"
        env["NORI_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
