#!/usr/bin/env python3
"""
Regression test: `nori --version` must not scan huge sibling app lists just to
...
EXPECTED_STDOUT = "nori 9.9.9 (999)"
...
def build_fixture(root: str, cli_path: str) -> str:
    app_path = os.path.join(root, "nori.app")
    contents_path = os.path.join(app_path, "Contents")
    resources_path = os.path.join(contents_path, "Resources")
    bin_path = os.path.join(resources_path, "bin")
    os.makedirs(bin_path, exist_ok=True)

    fixture_cli = os.path.join(bin_path, "nori")
    shutil.copy2(cli_path, fixture_cli)
    copy_runtime_frameworks(cli_path, contents_path)

    info = {
        "CFBundleExecutable": "nori",
        "CFBundleIdentifier": "test.nori.version-memory-guard",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="nori-version-memory-guard-") as root:
        fixture_cli = build_fixture(root, cli_path)
        result = run_with_limits(fixture_cli, "--version")

    if result["failure_reason"]:
        print("FAIL: `nori --version` exceeded runtime guard")
    ...
    if result["exit_code"] != 0:
        print("FAIL: `nori --version` exited non-zero")
    ...
    print(
        "PASS: `nori --version` exits within memory/time limits "
