...
NEW_WORKSPACES = int(os.environ.get("NORI_LAG_NEW_WORKSPACES", "20"))
SWITCH_PASSES = int(os.environ.get("NORI_LAG_SWITCH_PASSES", "1"))
SWITCH_DELAY_S = float(os.environ.get("NORI_LAG_SWITCH_DELAY_S", "0.06"))
HISTORY_SEED_LINES = int(os.environ.get("NORI_LAG_HISTORY_LINES", "120"))
KEY_EVENTS = int(os.environ.get("NORI_LAG_KEY_EVENTS", "180"))
KEY_DELAY_S = float(os.environ.get("NORI_LAG_KEY_DELAY_S", "0.0"))
KEY_COMBO = os.environ.get("NORI_LAG_KEY_COMBO", "up")

MAX_P95_RATIO = float(os.environ.get("NORI_LAG_MAX_P95_RATIO", "1.70"))
MAX_AVG_RATIO = float(os.environ.get("NORI_LAG_MAX_AVG_RATIO", "1.70"))
MAX_CHURN_P95_MS = float(os.environ.get("NORI_LAG_MAX_CHURN_P95_MS", "35.0"))
MAX_P95_DELTA_MS = float(os.environ.get("NORI_LAG_MAX_P95_DELTA_MS", "20.0"))
MAX_AVG_DELTA_MS = float(os.environ.get("NORI_LAG_MAX_AVG_DELTA_MS", "12.0"))
MIN_BASELINE_P95_MS_FOR_RATIO = float(os.environ.get("NORI_LAG_MIN_BASELINE_P95_MS_FOR_RATIO", "6.0"))
MIN_BASELINE_AVG_MS_FOR_RATIO = float(os.environ.get("NORI_LAG_MIN_BASELINE_AVG_MS_FOR_RATIO", "4.0"))
MAX_CPU_PERCENT = float(os.environ.get("NORI_LAG_MAX_CPU_PERCENT", "180.0"))
ENFORCE_CPU = os.environ.get("NORI_LAG_ENFORCE_CPU", "0") == "1"
ALLOW_MAIN_SOCKET = os.environ.get("NORI_LAG_ALLOW_MAIN_SOCKET", "0") == "1"
...
class RawSocketClient:
    ...
    def command(self, command: str, timeout_s: float = 2.0) -> str:
        if self.sock is None:
            raise noriError("Raw socket client not connected")
        ...
        while True:
            ...
            if remaining <= 0:
                raise noriError(f"Timed out waiting for response to: {command}")
            ...
            if not chunk:
                raise noriError("Socket closed while waiting for response")
...
def get_nori_pid_for_socket(socket_path: Optional[str]) -> Optional[int]:
    ...
    result = subprocess.run(
        ["pgrep", "-f", r"nori DEV.*\.app/Contents/MacOS/nori DEV"],
        capture_output=True,
        text=True,
    )
    if result.returncode != 0:
...
def resolve_target_socket() -> str:
    socket_path = os.environ.get("NORI_SOCKET_PATH")
    if not socket_path:
        raise noriError(
            "NORI_SOCKET_PATH is required. Point it to a tagged dev socket (for example /tmp/nori-debug-<tag>.sock)."
        )
    base = os.path.basename(socket_path)
    if not ALLOW_MAIN_SOCKET and base in {"nori.sock", "nori-debug.sock"}:
        raise noriError(
            f"Refusing to run against main socket '{socket_path}'. Set NORI_SOCKET_PATH to a tagged dev instance."
        )
    return socket_path
...
def keep_only_first_workspace(client: nori) -> str:
...
def create_workspaces(client: nori, count: int) -> list[str]:
    created: list[str] = []
...
def cycle_all_workspaces(client: nori, passes: int, delay_s: float) -> list[str]:
...
def focused_terminal_panel(client: nori) -> str:
    surfaces = client.list_surfaces()
    if not surfaces:
        raise noriError("No surfaces available in selected workspace")
...
def seed_history(client: nori, lines: int) -> None:
    for i in range(lines):
        client.send_line(f"echo nori-lag-seed-{i}")
...
def run_baseline_scenario(client: nori, socket_path: str) -> tuple[str, LatencyStats]:
...
def run_churn_scenario(client: nori, socket_path: str, first_workspace_id: str) -> tuple[str, LatencyStats]:
    first_workspace_id = keep_only_first_workspace(client)
...
def main() -> int:
    ...
    client: Optional[nori] = None
    pid: Optional[int] = None
    first_workspace_id: Optional[str] = None

    try:
        target_socket = resolve_target_socket()
        client = nori(socket_path=target_socket)
        client.connect()
        print(f"Using socket: {client.socket_path}")

        pid = get_nori_pid_for_socket(client.socket_path)
        if pid is None:
            print("SKIP: nori process not found for socket")
            return 0
        ...
        if failures:
            ...
            for item in failures:
                print(f"  - {item}")
            sample_path = maybe_write_sample(pid, "nori_workspace_churn_up_arrow_lag")
            if sample_path:
        ...
        print(f"FAIL: {e}")
        sample_path = maybe_write_sample(pid, "nori_workspace_churn_up_arrow_error")
        if sample_path:
            print(f"sample_path: {sample_path}")
        return 1
