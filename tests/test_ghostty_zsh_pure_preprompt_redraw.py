...
typeset -g NORI_TOP='%F{4}%~%f'
typeset -g NORI_LAST_PROMPT=''
typeset -gi NORI_ASYNC_DONE=0
typeset -g NORI_ASYNC_FD=''
...
nori_render_prompt() {
  ...
  PROMPT="${NORI_TOP}${prompt_newline}${cleaned_ps1:-%F{5}❯%f }"

  local expanded_prompt="${(S%%)PROMPT}"
  if [[ ${1:-} == precmd ]]; then
    print
  elif [[ $NORI_LAST_PROMPT != $expanded_prompt ]]; then
    zle && zle .reset-prompt
  fi
  typeset -g NORI_LAST_PROMPT=$expanded_prompt
}

nori_async_ready() {
  emulate -L zsh
  local fd="${1:-$NORI_ASYNC_FD}"
  if [[ -n $fd ]]; then
    zle -F "$fd"
    exec {fd}<&-
  fi
  ...
  (( NORI_ASYNC_DONE )) && return
  NORI_ASYNC_DONE=1
  NORI_TOP='%F{4}%~%f %F{242}main%f%F{218}*%f %F{6}⇣⇡%f'
  nori_render_prompt async
}

precmd() {
  NORI_ASYNC_DONE=0
  nori_render_prompt precmd
}

nori_line_init() {
  if (( !NORI_ASYNC_DONE )) && [[ -z $NORI_ASYNC_FD ]]; then
    exec {NORI_ASYNC_FD}< <(
      sleep 0.05
      printf 'ready\n'
    )
    zle -F "$NORI_ASYNC_FD" nori_async_ready
  fi
}

zle -N zle-line-init nori_line_init
PROMPT='%F{5}❯%f '
""".lstrip()
...
def _capture_session(
    *,
    ...
    base = Path(tempfile.mkdtemp(prefix="nori_ghostty_pure_preprompt_"))
    try:
        home = base / "home"
