...
def resolve_nori_cli() -> str:
    ...
    candidates: list[str] = []
    candidates.extend(glob.glob(os.path.expanduser("~/Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/nori")))
    ...
    in_path = shutil.which("nori")
    if in_path:
        return in_path

    raise RuntimeError("Unable to find nori CLI binary. Set NORI_CLI_BIN.")
...
def run_claude_hook(
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"nori claude-hook {subcommand} failed:\n"
            f"exit={proc.returncode}\nstdout={proc.stdout}\nstderr={proc.stderr}"
        )
    return proc.stdout.strip()


def wait_for_notification_count(client: nori, minimum: int, timeout: float = 4.0) -> list[dict]:
    start = time.time()
    items: list[dict] = []
...
def main() -> int:
    ...
    state_path = Path(tempfile.gettempdir()) / f"nori_claude_hook_state_{os.getpid()}.json"
    ...
    project_dir = Path(tempfile.gettempdir()) / f"nori_claude_map_project_{os.getpid()}"
    ...
    try:
        with nori() as client:
            ...
            hook_env["NORI_SOCKET_PATH"] = client.socket_path
            hook_env["NORI_WORKSPACE_ID"] = workspace_id
            hook_env["NORI_SURFACE_ID"] = surface_id
            hook_env["NORI_CLAUDE_HOOK_STATE_PATH"] = str(state_path)
    ...
    except (noriError, RuntimeError) as exc:
        return fail(str(exc))
    finally:
        try:
            if state_path.exists():
