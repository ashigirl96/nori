...
Validates:
1) `nori codex-hook session-start` records the inferred agent root PID.
...
from claude_teams_test_utils import resolve_nori_cli
...
def _wait_for_port(client: nori, workspace_id: str, port: int, timeout: float = 18.0) -> dict[str, str]:
    def pred():
...
def _wait_for_port_absent(client: nori, workspace_id: str, port: int, timeout: float = 18.0) -> dict[str, str]:
    def pred():
        state = _parse_sidebar_state(client.sidebar_state(tab=workspace_id))
        raw = state.get("ports", "")
        if raw == "none" or not raw:
...
host = os.environ["NORI_TEST_BIND_HOST"]
port = int(os.environ["NORI_TEST_PORT"])
cli_path = os.environ["NORI_TEST_CLI_PATH"]
socket_path = os.environ["NORI_TEST_SOCKET_PATH"]
workspace_id = os.environ["NORI_WORKSPACE_ID"]
surface_id = os.environ["NORI_SURFACE_ID"]
state_path = os.environ["NORI_CLAUDE_HOOK_STATE_PATH"]
session_id = os.environ["NORI_TEST_SESSION_ID"]
cwd = os.environ["NORI_TEST_CWD"]
ready_file = Path(os.environ["NORI_TEST_READY_FILE"])
start_file = Path(os.environ["NORI_TEST_START_FILE"])
server_pid_file = Path(os.environ["NORI_TEST_SERVER_PID_FILE"])
server_log_file = Path(os.environ["NORI_TEST_SERVER_LOG_FILE"])

hook_env = os.environ.copy()
hook_env["NORI_SOCKET_PATH"] = socket_path
hook_env["NORI_WORKSPACE_ID"] = workspace_id
hook_env["NORI_SURFACE_ID"] = surface_id
hook_env["NORI_CLAUDE_HOOK_STATE_PATH"] = state_path
...
""",
    ...
    env = os.environ.copy()
    env["NORI_TEST_BIND_HOST"] = _PREFERRED_BIND_HOST
    env["NORI_TEST_PORT"] = str(port)
    env["NORI_TEST_CLI_PATH"] = cli_path
    env["NORI_TEST_SOCKET_PATH"] = socket_path
    env["NORI_WORKSPACE_ID"] = workspace_id
    env["NORI_SURFACE_ID"] = surface_id
    env["NORI_CLAUDE_HOOK_STATE_PATH"] = str(state_path)
    env["NORI_TEST_SESSION_ID"] = session_id
    env["NORI_TEST_CWD"] = str(cwd)
    env["NORI_TEST_READY_FILE"] = str(ready_file)
    env["NORI_TEST_START_FILE"] = str(start_file)
    env["NORI_TEST_SERVER_PID_FILE"] = str(server_pid_file)
    env["NORI_TEST_SERVER_LOG_FILE"] = str(server_log_file)
...
def main() -> int:
    try:
        cli_path = resolve_nori_cli()
    except Exception as exc:
        return fail(str(exc))

    state_path = Path(tempfile.gettempdir()) / f"nori_codex_hook_state_{os.getpid()}.json"
    lock_path = Path(str(state_path) + ".lock")
    base = Path(tempfile.gettempdir()) / f"nori_codex_ports_{os.getpid()}"
    ...
    except (noriError, RuntimeError, AssertionError, ValueError) as exc:
        return fail(str(exc))
    finally:
