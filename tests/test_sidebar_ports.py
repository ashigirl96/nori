...
Run with a tagged instance to avoid unix socket conflicts:
  NORI_TAG=<tag> python3 tests/test_sidebar_ports.py
"""
...
from nori import nori, noriError  # noqa: E402
...
def _start_external_server(base: Path, port: int) -> subprocess.Popen:
    """
    Start an http.server outside nori and ensure it is actually listening.
...
def _start_agent_server(base: Path, port: int, pid_file: Path, log_file: Path) -> subprocess.Popen:
    """
    Start a long-lived "agent" shell outside nori. The shell owns a child
...
def _wait_for_port(client: nori, port: int, timeout: float = 18.0) -> dict[str, str]:
    def pred():
...
def _wait_for_port_absent(client: nori, port: int, timeout: float = 18.0) -> dict[str, str]:
    def pred():
        state = _parse_sidebar_state(client.sidebar_state())
        raw = state.get("ports", "")
        if raw == "none" or not raw:
...
def _assert_port_absent_for_duration(client: nori, port: int, duration: float = 6.0, interval: float = 0.15) -> None:
    """
...
def main() -> int:
    tag = os.environ.get("NORI_TAG") or ""
    if not tag:
        print("Tip: set NORI_TAG=<tag> when running this test to avoid socket conflicts.")

    base = Path("/tmp") / f"nori_ports_test_{os.getpid()}"
    ...
    try:
        ...
        # Start a listening server outside nori. A fresh tab should NOT show this port,
        ...
        with nori() as client:
            ...
            client.send("echo nori_ports_test\n")
    ...
    except (noriError, AssertionError, RuntimeError, ValueError) as e:
