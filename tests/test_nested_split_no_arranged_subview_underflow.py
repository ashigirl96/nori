...
def main() -> int:
    with nori(SOCKET_PATH) as c:
        ...
        panes = c.list_panes()
        if len(panes) < 2:
            raise noriError(f"expected >=2 panes after first split, got {len(panes)}: {panes}")
        ...
        if underflows != 0:
            ...
            raise noriError(f"bonsplit arranged-subview underflow observed ({underflows}); screenshot: {shot}")
