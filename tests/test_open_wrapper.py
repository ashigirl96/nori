...
def run_wrapper(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="nori-open-wrapper-test-") as td:
        tmp = Path(td)
        wrapper = tmp / "open"
...
key="${3:-}"
case "$key" in
  browserInterceptTerminalOpenCommandInNoriBrowser)
    ...
    exit 1
    ;;
  browserOpenTerminalLinksInNoriBrowser)
...
if [[ -n "${FAKE_NORI_FAIL_URLS:-}" ]]; then
  IFS=',' read -r -a failures <<< "$FAKE_NORI_FAIL_URLS"
...
""",
        ...
        env = os.environ.copy()
        env["NORI_SOCKET_PATH"] = "/tmp/nori-open-wrapper-test.sock"
        env["NORI_BUNDLE_ID"] = "com.nori.app.debug.test"
        env["NORI_OPEN_WRAPPER_SYSTEM_OPEN"] = str(system_open)
        env["NORI_OPEN_WRAPPER_DEFAULTS"] = str(defaults)
        env["FAKE_OPEN_LOG"] = str(open_log)
        env["FAKE_NORI_LOG"] = str(nori_log)
        if python_bin is None:
            env.pop("NORI_OPEN_WRAPPER_PYTHON3", None)
        else:
            env["NORI_OPEN_WRAPPER_PYTHON3"] = python_bin
        ...
        if fail_urls:
            env["FAKE_NORI_FAIL_URLS"] = ",".join(fail_urls)
        else:
            env.pop("FAKE_NORI_FAIL_URLS", None)
        ...
        return read_log(open_log), read_log(nori_log), result.returncode, result.stderr.strip()
...
def test_toggle_disabled_passthrough(failures: list[str]) -> None:
    ...
    open_log, nori_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="0",
        whitelist="",
    )
    expect(code == 0, f"toggle off: wrapper exited {code}: {stderr}", failures)
    expect(nori_log == [], f"toggle off: nori should not be called, got {nori_log}", failures)
...
def test_toggle_disabled_case_insensitive_passthrough(failures: list[str]) -> None:
    ...
    expect(
        nori_log == [],
        f"toggle off (case-insensitive): nori should not be called, got {nori_log}",
        failures,
    )
    expect(
        open_log == [url],
...
def test_whitelist_miss_passthrough(failures: list[str]) -> None:
    ...
    expect(nori_log == [], f"whitelist miss: nori should not be called, got {nori_log}", failures)
    expect(open_log == [url], f"whitelist miss: expected system open [{url}], got {open_log}", failures)


def test_whitelist_match_routes_to_nori(failures: list[str]) -> None:
    ...
    expect(nori_log == [f"browser open {url}"], f"whitelist match: unexpected nori log {nori_log}", failures)
...
def test_external_literal_pattern_is_deferred_to_app(failures: list[str]) -> None:
    ...
    expect(
        nori_log == [f"browser open {url}"],
        f"external literal deferred: expected wrapper to pass URL to nori, got {nori_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_external_regex_pattern_is_deferred_to_app(failures: list[str]) -> None:
    ...
    expect(
        nori_log == [f"browser open {url}"],
        f"external regex deferred: expected wrapper to pass URL to nori, got {nori_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_external_regex_with_icu_features_is_deferred_to_app(failures: list[str]) -> None:
    ...
    expect(
        nori_log == [f"browser open {url}"],
        f"external regex icu deferred: expected wrapper to pass URL to nori, got {nori_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_external_invalid_regex_is_ignored_silently(failures: list[str]) -> None:
    ...
    expect(
        nori_log == [f"browser open {url}"],
        f"external invalid regex: expected nori open for {url}, got {nori_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_partial_failures_only_fallback_failed_urls(failures: list[str]) -> None:
    ...
    expect(
        nori_log == [f"browser open {good}", f"browser open {failed}"],
        f"partial failure: nori log mismatch {nori_log}",
        failures,
    )
    expect(
...
def test_legacy_toggle_fallback_passthrough(failures: list[str]) -> None:
    ...
    expect(nori_log == [], f"legacy fallback: nori should not be called, got {nori_log}", failures)
...
def test_legacy_toggle_fallback_case_insensitive_passthrough(failures: list[str]) -> None:
    ...
    expect(
        nori_log == [],
        f"legacy fallback (case-insensitive): nori should not be called, got {nori_log}",
        failures,
    )
    expect(
        open_log == [url],
...
def test_uppercase_scheme_routes_to_nori(failures: list[str]) -> None:
    url = "HTTPS://api.example.com/path?q=1"
    open_log, nori_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="1",
    ...
    expect(nori_log == [f"browser open {url}"], f"uppercase scheme: unexpected nori log {nori_log}", failures)


def test_local_html_file_routes_to_nori(failures: list[str]) -> None:
    ...
    expect(len(nori_log) == 1, f"local html file: expected exactly one nori call, got {nori_log}", failures)
    if nori_log:
        expect(
            nori_log[0].startswith("browser open file://"),
            f"local html file: expected file:// target, got {nori_log[0]}",
            failures,
        )
        expect(
            "hello%20page.HTML" in nori_log[0],
            f"local html file: expected URL-encoded filename in nori target, got {nori_log[0]}",
            failures,
        )


def test_file_url_html_routes_to_nori(failures: list[str]) -> None:
    url = "file:///tmp/nori-open-wrapper-fixture.html"
    open_log, nori_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="1",
        whitelist="",
    ...
    expect(nori_log == [f"browser open {url}"], f"file url html: unexpected nori log {nori_log}", failures)


def test_file_url_html_routes_to_nori_without_python_binary(failures: list[str]) -> None:
    url = "file:///tmp/nori-open-wrapper-fixture.html"
    open_log, nori_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="1",
    ...
    expect(
        nori_log == [f"browser open {url}"],
        f"file url html no-python fallback: unexpected nori log {nori_log}",
        failures,
    )


def test_local_html_file_routes_to_nori_without_python_binary(failures: list[str]) -> None:
    ...
    expect(
        len(nori_log) == 1,
        f"local html no-python fallback: expected exactly one nori call, got {nori_log}",
        failures,
    )
    if nori_log:
        expect(
            nori_log[0].startswith("browser open file://"),
            f"local html no-python fallback: expected file:// target, got {nori_log[0]}",
            failures,
        )
        expect(
            "no%20python%20fallback.html" in nori_log[0],
            f"local html no-python fallback: expected URL-encoded filename, got {nori_log[0]}",
            failures,
        )
...
def test_domain_like_html_argument_passthrough(failures: list[str]) -> None:
    ...
    expect(
        nori_log == [],
        f"domain-like html argument: nori should not be called, got {nori_log}",
        failures,
    )
    expect(
        open_log == [arg],
...
def test_non_file_scheme_html_passthrough(failures: list[str]) -> None:
    ...
    expect(nori_log == [], f"non-file scheme html: nori should not be called, got {nori_log}", failures)
...
def test_mailto_html_passthrough(failures: list[str]) -> None:
    ...
    expect(nori_log == [], f"mailto html: nori should not be called, got {nori_log}", failures)
...
def test_local_non_html_file_passthrough(failures: list[str]) -> None:
    ...
    expect(nori_log == [], f"local non-html file: nori should not be called, got {nori_log}", failures)
...
def test_unicode_whitelist_matches_punycode_url(failures: list[str]) -> None:
    ...
    expect(nori_log == [f"browser open {url}"], f"unicode whitelist: unexpected nori log {nori_log}", failures)
...
def test_punycode_whitelist_matches_unicode_url(failures: list[str]) -> None:
    ...
    expect(nori_log == [f"browser open {url}"], f"punycode whitelist: unexpected nori log {nori_log}", failures)
...
def main() -> int:
    ...
    test_whitelist_match_routes_to_nori(failures)
    ...
    test_uppercase_scheme_routes_to_nori(failures)
    test_local_html_file_routes_to_nori(failures)
    test_file_url_html_routes_to_nori(failures)
    test_file_url_html_routes_to_nori_without_python_binary(failures)
    test_local_html_file_routes_to_nori_without_python_binary(failures)
