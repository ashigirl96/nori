#!/usr/bin/env python3
"""
Regression test: `nori claude-teams` supports Claude's tmux teammate flow.
"""
    ...
    def handle(self, method: str, params: dict[str, object]) -> dict[str, object]:
        with self.lock:
            ...
            if method == "surface.send_text":
                return {"ok": True}
            raise RuntimeError(f"Unsupported fake nori method: {method}")
...
class FakeNoriUnixServer(socketserver.ThreadingUnixStreamServer):
    allow_reuse_address = True

    def __init__(self, socket_path: str, state: FakeNoriState) -> None:
        self.state = state
        super().__init__(socket_path, FakeNoriHandler)


class FakeNoriHandler(socketserver.StreamRequestHandler):
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="nori-claude-teams-seq-") as td:
        tmp = Path(td)
        home = tmp / "home"
        home.mkdir(parents=True, exist_ok=True)

        socket_path = tmp / "fake-nori.sock"
        state = FakeNoriState()
        server = FakeNoriUnixServer(str(socket_path), state)
...
printf '%s\\n' "${NORI_SOCKET_PATH-__UNSET__}" > "$FAKE_SOCKET_LOG"
...
""",
        ...
        try:
            ...
            print("FAIL: `nori claude-teams --version` timed out")
        ...
        if socket_value != str(socket_path):
            print(f"FAIL: expected NORI_SOCKET_PATH={socket_path}, got {socket_value!r}")
            return 1

        window_target = read_text(window_target_log)
        if window_target != "nori:1":
            print(f"FAIL: expected tmux window target 'nori:1', got {window_target!r}")
            return 1
    ...
    print("PASS: nori claude-teams supports Claude's tmux teammate flow")
    return 0
