...
def _wait_for_surface_focus(c: nori, panel_id: str, timeout_s: float = 5.0) -> None:
    ...
    raise noriError(f"Timed out waiting for surface focus: {panel_id}")


def _wait_for_render_context(c: nori, panel_id: str, timeout_s: float = 5.0) -> dict:
    ...
    while time.time() - start < timeout_s:
        ...
        if bool(last.get("inWindow")):
            return last
        time.sleep(0.1)
    raise noriError(f"Expected inWindow render context, got: {last}")


def main() -> int:
    token = f"NORI_INIT_{int(time.time() * 1000)}"
    tmp = f"/tmp/nori_init_{token}.txt"
    with nori(SOCKET_PATH) as c:
        ...
        if not surfaces:
            raise noriError("Expected at least 1 surface after new_workspace")
