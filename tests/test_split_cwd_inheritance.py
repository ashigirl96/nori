...
Requires:
  - nori running with allowAll socket mode
  - bash shell integration sourced (nori-bash-integration.bash)

Run with a tagged instance:
  NORI_TAG=<tag> python3 tests/test_split_cwd_inheritance.py
"""
...
from nori import nori  # noqa: E402
...
def _focus_first_surface(client: nori) -> str:
...
def main() -> int:
    tag = os.environ.get("NORI_TAG", "")

    socket_path = None
    if tag:
        socket_path = f"/tmp/nori-debug-{tag}.sock"
    client = nori(socket_path=socket_path)
    client.connect()

    # Use resolved paths to avoid /tmp -> /private/tmp symlink mismatch on macOS
    test_dir_a = str(Path("/tmp/nori_split_cwd_test_a").resolve())
    test_dir_b = str(Path("/tmp/nori_split_cwd_test_b").resolve())
