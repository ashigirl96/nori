...
def _snapshot_path(bundle_id: str) -> Path:
    ...
    return Path.home() / "Library/Application Support/nori" / f"session-{safe_bundle}.json"
...
def _socket_candidates(app_path: Path, preferred: Path) -> list[Path]:
    ...
    if app_name.startswith(prefix):
        ...
        candidates.append(Path(f"/tmp/nori-debug-{slug}.sock"))
    deduped: list[Path] = []
    seen: set[str] = set()
...
def _kill_existing(app_path: Path) -> None:
    exe = app_path / "Contents" / "MacOS" / "nori DEV"
...
def _launch(app_path: Path, preferred_socket_path: Path) -> Path:
    ...
    subprocess.run(
        [
            ...
            str(app_path),
            "--env",
            f"NORI_SOCKET_PATH={preferred_socket_path}",
            "--env",
...
def _connect(socket_path: Path) -> nori:
    client = nori(socket_path=str(socket_path))
    client.connect()
    if not client.ping():
...
def _read_scrollback(client: nori) -> str:
    return client._send_command("read_screen --scrollback")


def _wait_for_marker(client: nori, marker: str, timeout: float = 8.0) -> bool:
    deadline = time.time() + timeout
    while time.time() < deadline:
...
def _consume_visible_markers(client: nori, remaining: set[str], timeout: float = 4.0) -> None:
    if not remaining:
        return
    deadline = time.time() + timeout
...
def _ensure_workspaces(client: nori, count: int) -> None:
    while len(client.list_workspaces()) < count:
        client.new_workspace()
        time.sleep(0.3)


def _list_windows(client: nori) -> list[str]:
...
def _new_window(client: nori) -> str:
...
def _focus_window(client: nori, window_id: str) -> None:
...
def main() -> int:
    app_path_str = os.environ.get("NORI_APP_PATH", "").strip()
    if not app_path_str:
        print("SKIP: set NORI_APP_PATH to a built nori DEV .app path")
        return 0
    app_path = Path(app_path_str)
    if not app_path.exists():
        print(f"SKIP: NORI_APP_PATH does not exist: {app_path}")
        return 0
    ...
    socket_path = Path(f"/tmp/nori-mw-restore-{bundle_suffix}.sock")

    markers = {
        "w1_ws0": "NORI_MW_RESTORE_W1_WS0",
        "w1_ws1": "NORI_MW_RESTORE_W1_WS1",
        "w2_ws0": "NORI_MW_RESTORE_W2_WS0",
        "w2_ws1": "NORI_MW_RESTORE_W2_WS1",
    }
    failures: list[str] = []
