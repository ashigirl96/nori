...
overlay used for focus tracking. Fix: NoriWebView.mouseDown posts a notification
...
def _marker(name: str) -> Path:
    return MARKER_DIR / f"nori_mwf_{name}_{os.getpid()}"
...
def _verify_responsive(c: nori, marker: Path, surface_idx: int, retries: int = 3) -> bool:
...
def _wait_terminal_in_window(c: nori, surface_idx: int, timeout: float = 5.0) -> bool:
    start = time.time()
    while time.time() - start < timeout:
        try:
...
def test_multi_workspace_terminal_responsive(c: nori) -> None:
    """
...
def test_three_workspaces_non_last_responsive(c: nori) -> None:
    """
...
def test_rapid_workspace_switching_preserves_focus(c: nori) -> None:
    """
...
def test_browser_panel_focus_and_return(c: nori) -> None:
    """
    ...
    if not surfaces:
        raise noriError("No surfaces after new_workspace")
...
def test_browser_focus_across_workspaces(c: nori) -> None:
    """
    ...
    with nori() as c:
        ...
        for name, test_fn in tests:
            ...
            try:
                ...
                print("PASS")
                passed += 1
            except (AssertionError, noriError) as e:
                print(f"FAIL: {e}")
                failed += 1
