...
def _wait_for_terminal_focus(c: nori, panel_id: str, timeout_s: float = 6.0) -> bool:
    start = time.time()
...
def _panel_snapshot_retry(c: nori, panel_id: str, label: str, timeout_s: float = 3.0) -> dict:
    start = time.time()
    ...
    while time.time() - start < timeout_s:
        try:
            ...
            if "Failed to capture panel image" not in str(e):
                raise
            time.sleep(0.05)
    raise noriError(f"Timed out waiting for panel_snapshot: panel_id={panel_id} label={label}: {last_err!r}")
...
def main() -> int:
    with nori(SOCKET_PATH) as c:
        ...
        draw_cmd = "for i in {1..40}; do echo NORI_DRAW_$i; done"
        ...
        if w1 <= 0 or h1 <= 0 or (w1, h1) != (w2, h2):
            raise noriError(f"panel_snapshot dims differ: {(w1,h1)} {(w2,h2)}; paths: {s1.get('path')} {s2.get('path')}")
