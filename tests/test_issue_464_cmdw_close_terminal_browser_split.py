...
Run against a Debug app socket (typically with NORI_SOCKET_MODE=allowAll).
"""
...
def _wait_url_contains(client: nori, panel_id: str, needle: str, timeout_s: float = 20.0) -> None:
    def _matches() -> bool:
    ...
    if not _wait_until(_matches, timeout_s=timeout_s, interval_s=0.1):
        ...
        raise noriError(f"Timed out waiting for browser URL containing '{needle}', got: {current}")


def _capture_screenshot(client: nori, label: str) -> str:
...
def _focused_terminal_ready(client: nori, panel_id: str) -> bool:
    try:
...
def _drag_hit_chain(client: nori, nx: float, ny: float) -> str:
    ...
    with nori(SOCKET_PATH) as client:
        ...
        if not ping_ok:
            raise noriError(
                ...
                "Launch Debug app with NORI_SOCKET_MODE=allowAll for this test."
            )
        ...
        try:
            ...
            if not _wait_until(lambda: _focused_terminal_ready(client, terminal_id), timeout_s=4.0):
                raise noriError(f"Terminal did not become first responder before Cmd+W: {terminal_id}")
