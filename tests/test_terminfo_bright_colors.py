#!/usr/bin/env python3
"""
Regression: nori relies on Ghostty's xterm-ghostty terminfo entry.

In nori (embedded GhosttyKit), "bright" SGR 90-97 can render incorrectly
for some palettes. zsh-autosuggestions defaults to `fg=8`, which historically
resolved to SGR 90 via terminfo `setaf`.

nori ships a terminfo overlay that forces bright colors to use indexed
