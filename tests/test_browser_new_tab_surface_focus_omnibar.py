...
def v2_call(client: nori, method: str, params: dict[str, Any] | None = None, request_id: str = "1") -> dict[str, Any]:
    payload = {
        "id": request_id,
        "method": method,
        "params": params or {},
    ...
    try:
        ...
        raise noriError(f"Invalid v2 JSON response for {method}: {raw}") from exc

    if not parsed.get("ok"):
        raise noriError(f"v2 {method} failed: {parsed.get('error')}")
...
def browser_address_bar_focus_state(client: nori, surface_id: str | None = None, request_id: str = "browser-focus") -> dict[str, Any]:
    params: dict[str, Any] = {}
    if surface_id:
...
def set_command_palette_visible(client: nori, window_id: str, target_visible: bool) -> bool:
    for idx in range(5):
        state = v2_call(
            client,
...
def command_palette_results(client: nori, window_id: str, limit: int = 20) -> list[dict[str, Any]]:
    payload = v2_call(
        client,
...
def command_palette_selected_index(client: nori, window_id: str) -> int:
    payload = v2_call(
        client,
...
def move_command_palette_selection_to_index(client: nori, window_id: str, target_index: int) -> bool:
...
def main() -> int:
    ...
    try:
        ...
        if not set_command_palette_visible(client, window_id, False):
            raise noriError("Failed to ensure command palette is hidden for scenario 1")
        ...
        if not terminal_id:
            raise noriError("Missing terminal surface for focus setup")
        ...
        if not did_focus_address_bar:
            raise noriError("Blank browser surface did not focus omnibar after focus_surface")
        ...
        window_id = current_window_id(client)
        if not set_command_palette_visible(client, window_id, False):
            raise noriError("Failed to ensure command palette is hidden for scenario 2")
        ...
        if not left_terminal_id:
            raise noriError("Missing initial terminal surface for split setup")
        ...
        if not left_pane or not browser_pane:
            raise noriError("Failed to locate split panes for pane-focus scenario")
        ...
        if not did_focus_split_browser:
            raise noriError("Blank browser pane did not focus omnibar after focus_pane")
        ...
        window_id = current_window_id(client)
        if not set_command_palette_visible(client, window_id, False):
            raise noriError("Failed to reset command palette before scenario 3")
        ...
        if not terminal_id:
            raise noriError("Missing terminal surface for command palette scenario")
        ...
        if not set_command_palette_visible(client, window_id, True):
            raise noriError("Failed to open command palette")
        ...
        if not palette_visible_after_focus:
            raise noriError("Command palette closed unexpectedly after focus_surface")
        ...
        if bool(blank_focus_state.get("focused")):
            raise noriError("Blank browser tab stole omnibar focus while command palette was visible")
        ...
        window_id = current_window_id(client)
        if not set_command_palette_visible(client, window_id, False):
            raise noriError("Failed to reset command palette before scenario 4 (target setup)")
        ...
        if not did_focus_target_browser:
            raise noriError("Failed to focus omnibar on target workspace browser before Cmd+P switch")
        ...
        window_id = current_window_id(client)
        if not set_command_palette_visible(client, window_id, False):
            raise noriError("Failed to reset command palette before scenario 4 (source setup)")
        ...
        if not source_terminal_id:
            raise noriError("Missing terminal surface for Cmd+P workspace switcher scenario")
        ...
        if not wait_for(
            ...
            raise noriError("Cmd+P did not open command palette switcher")
        ...
        if has_surface_rows:
            raise noriError("Cmd+P switcher listed unexpected surface rows; expected workspace-only results")
        ...
        if target_index is None:
            raise noriError(f"Cmd+P switcher did not list target workspace command {target_command_id}")

        if not move_command_palette_selection_to_index(client, window_id, target_index):
            raise noriError(f"Failed to move Cmd+P selection to result index {target_index}")
        ...
        if not did_focus_switcher_target:
            raise noriError("Cmd+P workspace switch did not restore blank browser omnibar focus")
        ...
        window_id = current_window_id(client)
        if not set_command_palette_visible(client, window_id, False):
            raise noriError("Failed to reset command palette before scenario 5")
        ...
        if not wait_for(
            ...
            raise noriError("Cmd+P did not open command palette switcher before Escape scenario")
        ...
        if not did_dismiss_switcher_on_escape:
            raise noriError("Cmd+P Escape did not dismiss command palette switcher")
