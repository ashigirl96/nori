#!/usr/bin/env python3
"""
Regression: the nori zsh wrapper should prefer a bundled Ghostty zsh
integration file in NORI_SHELL_INTEGRATION_DIR over the fallback integration
under GHOSTTY_RESOURCES_DIR.

Without this, tagged nori builds can silently load Ghostty's installed app
...
def main() -> int:
    ...
    base = Path("/tmp") / f"nori_bundled_ghostty_zsh_{os.getpid()}"
    try:
        ...
        env["NORI_SHELL_INTEGRATION_DIR"] = str(bundled)
        env["GHOSTTY_RESOURCES_DIR"] = str(fallback)
        env["NORI_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        env["NORI_SHELL_INTEGRATION"] = "0"
        env["NORI_TEST_OUT"] = str(marker)
