...
Run with a tagged instance to avoid unix socket conflicts:
  NORI_TAG=<tag> python3 tests/test_sidebar_cwd_git.py
"""
...
def _init_git_repo(repo: Path) -> None:
    ...
    _git(repo, "init")
    _git(repo, "config", "user.email", "nori-test@example.com")
    _git(repo, "config", "user.name", "nori-test")
...
def main() -> int:
    ...
    base = Path("/tmp") / f"nori_sidebar_test_{os.getpid()}"
    repo = base / "repo"
    other = base / "other"
    ...
    except (noriError, subprocess.CalledProcessError, AssertionError) as e:
