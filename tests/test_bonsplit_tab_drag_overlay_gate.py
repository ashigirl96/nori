...
def wait_for_overlay_probe_ready(client: nori, timeout_s: float = 8.0) -> None:
    start = time.time()
    last_error = None
    while time.time() - start < timeout_s:
        try:
            ...
            last_error = e
            time.sleep(0.1)
    raise noriError(f"overlay_hit_gate probe unavailable: {last_error}")


def assert_gate(client: nori, event_type: str, expected: bool, reason: str) -> None:
...
def assert_drop_gate(client: nori, source: str, expected: bool, reason: str) -> None:
...
def assert_portal_gate(client: nori, event_type: str, expected: bool, reason: str) -> None:
...
def assert_sidebar_gate(client: nori, state: str, expected: bool, reason: str) -> None:
...
def main() -> int:
    ...
    if not os.path.exists(socket_path):
        ...
        print("Tip: start nori first (or set NORI_TAG / NORI_SOCKET_PATH).")
        return 0
