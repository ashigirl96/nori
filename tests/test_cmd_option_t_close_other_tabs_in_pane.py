...
Run this against an app launched with NORI_SOCKET_MODE=allowAll.
"""
...
def _pane_state(client: nori) -> list[dict]:
    rows: list[dict] = []
...
def main() -> int:
    with nori(SOCKET_PATH) as client:
        ...
        try:
            ...
            if len(before) < 3:
                raise noriError(f"Expected >=3 tabs before shortcut, got {before}")

            selected_rows = [row for row in before if row["selected"]]
            if len(selected_rows) != 1:
                raise noriError(f"Expected exactly one selected tab before shortcut, got {before}")
