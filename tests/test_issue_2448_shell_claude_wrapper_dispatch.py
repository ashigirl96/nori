...
def prepare_bundle(tmp: Path) -> tuple[Path, Path]:
    ...
    for name in (".zshenv", ".zprofile", ".zshrc", "nori-zsh-integration.zsh", "nori-bash-integration.bash"):
...
def run_zsh(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    env["NORI_ZSH_ZDOTDIR"] = str(orig)
    env["NORI_SHELL_INTEGRATION"] = "1"
    env["NORI_SHELL_INTEGRATION_DIR"] = str(shell_dir)
    env["NORI_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "0"
    env["NORI_TEST_LOG"] = str(log_path)
    env["NORI_TEST_REAL_BIN"] = str(real_bin)
    env["PATH"] = f"{real_bin}:/usr/bin:/bin"
    env.pop("GHOSTTY_BIN_DIR", None)

    result = subprocess.run(
        ["zsh", "-d", "-i", "-c", 'PATH="$NORI_TEST_REAL_BIN:$PATH"; claude zsh-case'],
        env=env,
        capture_output=True,
        text=True,
        timeout=8,
...
def run_zsh_with_alias(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    result = subprocess.run(
        [
            "zsh",
            "-fic",
            f'alias claude="echo alias"; source "{shell_dir / "nori-zsh-integration.zsh"}"; '
            'PATH="$NORI_TEST_REAL_BIN:$PATH"; claude zsh-alias-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def run_bash(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    result = subprocess.run(
        [
            ...
            "--norc",
            "-c",
            f'source "{shell_dir / "nori-bash-integration.bash"}"; PATH="$NORI_TEST_REAL_BIN:$PATH"; claude bash-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def run_bash_with_alias(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    result = subprocess.run(
        [
            ...
            "--norc",
            "-ic",
            f'alias claude="$NORI_TEST_REAL_BIN/user-claude"; source "{shell_dir / "nori-bash-integration.bash"}"; '
            'PATH="$NORI_TEST_REAL_BIN:$PATH"; claude bash-alias-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def run_bash_with_function(shell_dir: Path, real_bin: Path, log_path: Path) -> tuple[int, str, list[str]]:
    ...
    result = subprocess.run(
        [
            ...
            "--norc",
            "-ic",
            f'claude() {{ "$NORI_TEST_REAL_BIN/user-claude-function" "$@"; }}; '
            f'source "{shell_dir / "nori-bash-integration.bash"}"; '
            'PATH="$NORI_TEST_REAL_BIN:$PATH"; claude bash-function-case',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="nori-issue-2448-") as td:
        tmp = Path(td)
