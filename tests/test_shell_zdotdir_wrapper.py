...
The nori zsh integration sets ZDOTDIR to the app's wrapper directory so zsh
...
def main() -> int:
    ...
    base = Path("/tmp") / f"nori_zdotdir_test_{os.getpid()}"
    try:
        if base.exists():
        ...
        (orig / ".zshenv").write_text(
            'echo "$ZDOTDIR" > "$NORI_ZDOTDIR_TEST_OUTPUT"\n',
            encoding="utf-8",
        )
        ...
        env["NORI_ZDOTDIR_TEST_OUTPUT"] = str(seen_path)
