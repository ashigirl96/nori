...
Requires nori to be running with socket control enabled.
"""
...
def get_nori_pid() -> Optional[int]:
    """Get the PID of the running nori process."""
    socket_path = os.environ.get("NORI_SOCKET_PATH")
    if not socket_path:
        # Ask nori.py to resolve default socket path (supports NORI_TAG and last-socket file).
        try:
            socket_path = nori().socket_path
        except Exception:
            socket_path = None
    ...
    result = subprocess.run(
        ["pgrep", "-f", r"nori\.app/Contents/MacOS/nori$"],
        capture_output=True,
        text=True,
    )
    if result.returncode != 0:
        # Try DEV build
        result = subprocess.run(
            ["pgrep", "-f", r"nori DEV\.app/Contents/MacOS/nori"],
            capture_output=True,
            text=True,
        )
    if result.returncode != 0:
...
def test_cpu_after_notification_burst(client: nori, pid: int) -> tuple[bool, str]:
    """
...
def test_cpu_after_popover_close(client: nori, pid: int) -> tuple[bool, str]:
    """
    ...
    # Ensure the correct nori instance is frontmost (tag-safe).
    bundle_id = nori.default_bundle_id()
    subprocess.run(
...
def test_cpu_idle_with_notifications(client: nori, pid: int) -> tuple[bool, str]:
    """
...
def main():
    print("=" * 60)
    print("nori Notification CPU Tests")
    print("=" * 60)
    ...
    pid = get_nori_pid()
    if pid is None:
        print("\n❌ SKIP: nori is not running")
        return 0

    print(f"\nFound nori process: PID {pid}")

    # Try to connect to the socket
    client = nori(socket_path)
    try:
        client.connect()
        print(f"Connected to {socket_path}")
    except noriError:
        print("\n❌ SKIP: Could not connect to nori socket")
        print("Tip: set NORI_TAG=<tag> or NORI_SOCKET_PATH=<path> to target a tagged instance.")
        return 0
