...
def _assert_all_panels_visible(c: nori, panel_ids: list[str], *, min_wh: int = 80) -> None:
    health = {row["id"].lower(): row for row in c.surface_health()}
    for pid in panel_ids:
        h = health.get(pid.lower())
        if not h:
            raise noriError(f"surface_health missing panel {pid}")
        if h.get("in_window") is not True:
            raise noriError(f"panel not in window: {pid} health={h}")

        snap = c.panel_snapshot(pid, label="nested_split")
        if snap["width"] < min_wh or snap["height"] < min_wh:
            raise noriError(f"panel snapshot too small: {pid} snap={snap}")


def _wait_until_all_panels_visible(c: nori, panel_ids: list[str], timeout_s: float) -> None:
    ...
    while time.time() < deadline:
        try:
            ...
            last_err = str(e)
            time.sleep(0.05)
    raise noriError(last_err or "panels never became visible")
...
def main() -> int:
    with nori(SOCKET_PATH) as c:
        ...
        for it in range(8):
            ...
            if last_err:
                raise noriError(f"iteration {it}: nested split caused disappearance: {last_err}")
