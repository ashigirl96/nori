...
private final class CLISocketSentryTelemetry {
    ...
    private static func currentSentryBundleIdentifier() -> String? {
        if let bundleIdentifier = ProcessInfo.processInfo.environment["NORI_BUNDLE_ID"]?
    ...
    init(command: String, commandArgs: [String], socketPath: String, processEnv: [String: String]) {
        ...
        self.envSocketPath = processEnv["NORI_SOCKET_PATH"] ?? processEnv["NORI_SOCKET"]
        self.workspaceId = processEnv["NORI_WORKSPACE_ID"]
        self.surfaceId = processEnv["NORI_SURFACE_ID"]
        self.disabledByEnv =
            processEnv["NORI_CLI_SENTRY_DISABLED"] == "1" ||
            processEnv["NORI_CLAUDE_HOOK_SENTRY_DISABLED"] == "1"
    }
    ...
    func breadcrumb(_ message: String, data: [String: Any] = [:]) {
        ...
        let crumb = Breadcrumb(level: .info, category: "nori.cli")
    ...
    func captureError(stage: String, error: Error) {
        ...
        _ = SentrySDK.capture(error: error) { scope in
            ...
            scope.setTag(value: "nori-cli", key: "component")
    ...
    private func socketDiagnostics() -> [String: Any] {
        ...
        if !tmpSockets.isEmpty {
            context["tmp_nori_sockets"] = tmpSockets
        }
        ...
        if CLISocketPathResolver.isImplicitDefaultPath(socketPath),
           ...
           !taggedSockets.isEmpty {
            context["possible_root_cause"] = "NORI_SOCKET_PATH/NORI_SOCKET missing while tagged sockets exist"
        }
    ...
    private static func discoverSockets(in directory: String, limit: Int) -> [String] {
        ...
        for name in entries.sorted() {
            guard name.hasPrefix("nori"), name.hasSuffix(".sock") else { continue }
...
private final class ClaudeHookSessionStore {
    private static let defaultStatePath = "~/.nori/claude-hook-sessions.json"
    ...
    init(
        ...
        if let overridePath = processEnv["NORI_CLAUDE_HOOK_STATE_PATH"]?.trimmingCharacters(in: .whitespacesAndNewlines),
...
enum SocketPasswordResolver {
    private static let service = "com.nori.app.socket-control"
    private static let account = "local-socket-password"
    private static let directoryName = "nori"
    ...
    static func resolve(explicit: String?, socketPath: String) -> String? {
        ...
        if let env = normalized(ProcessInfo.processInfo.environment["NORI_SOCKET_PASSWORD"]) {
            return env
        }
...
private enum CLISocketPathResolver {
    private static let appSupportDirectoryName = "nori"
    private static let stableSocketFileName = "nori.sock"
    private static let lastSocketPathFileName = "last-socket-path"
    static let legacyDefaultSocketPath = "/tmp/nori.sock"
    private static let fallbackSocketPath = "/tmp/nori-debug.sock"
    private static let stagingSocketPath = "/tmp/nori-staging.sock"
    private static let legacyLastSocketPathFile = "/tmp/nori-last-socket-path"
    ...
    private static func discoverTaggedSockets(limit: Int) -> [String] {
        ...
        for directory in socketDiscoveryDirectories() {
            ...
            for name in entries where name.hasPrefix("nori") && name.hasSuffix(".sock") {
...
final class SocketClient {
    ...
    private static let responseTimeoutSeconds: TimeInterval = {
        ...
        if let raw = env["NORITERM_CLI_RESPONSE_TIMEOUT_SEC"],
    ...
    private static func relayCredentials(for endpoint: RelayEndpoint) throws -> RelayCredentials {
        ...
        if let relayID = trimmedEnvValue(environment["NORI_RELAY_ID"]),
           let relayTokenHex = trimmedEnvValue(environment["NORI_RELAY_TOKEN"]),
        ...
        let authURL = URL(fileURLWithPath: NSHomeDirectory(), isDirectory: true)
            .appendingPathComponent(".nori/relay/\(endpoint.port).auth", isDirectory: false)
    ...
    private func authenticateRelay(credentials: RelayCredentials) throws {
        ...
        guard let challengeData = challengeLine.data(using: .utf8),
              ...
              (challenge["protocol"] as? String) == "nori-relay-auth",
    ...
    static func waitForConnectableSocket(path: String, timeout: TimeInterval) throws -> SocketClient {
        ...
        guard let watchDirectory = existingWatchDirectory(forPath: path) else {
            throw CLIError(message: "nori app did not start in time (socket not found at \(path))")
        }
        ...
        let queue = DispatchQueue(label: "com.nori.cli.socket-watch.\(UUID().uuidString)")
    ...
    static func waitForFilesystemPath(_ path: String, timeout: TimeInterval) throws {
        ...
        let queue = DispatchQueue(label: "com.nori.cli.path-watch.\(UUID().uuidString)")
        let semaphore = DispatchSemaphore(value: 0)
        var found = false
        let source = DispatchSource.makeFileSystemObjectSource(
            fileDescriptor: watchFD,
...
struct NORICLI {
    ...
    private static let debugLastSocketHintPath = "/tmp/nori-last-socket-path"
    ...
    private static func debugSocketPathFromHintFile() -> String? {
        ...
        guard let hinted = normalizedEnvValue(raw),
              hinted.hasPrefix("/tmp/nori-debug"),
    ...
    private static func defaultSocketPath(environment: [String: String]) -> String {
        if let explicit = normalizedEnvValue(environment["NORI_SOCKET_PATH"]) {
            return explicit
        }
#if DEBUG
    ...
    func run() throws {
        ...
        let envSocketPath: String? = {
            for key in ["NORI_SOCKET_PATH", "NORI_SOCKET"] {
        ...
        // so help text is available even when nori is not running.
        if command != "__tmux-compat",
           ...
           (commandArgs.contains("--help") || commandArgs.contains("-h")) {
            ...
            print("Unknown command '\(command)'. Run 'nori help' to see available commands.")
            return
        }
        ...
        // Codex hook handler: gracefully no-op when not inside nori
        // (before socket connection, so it doesn't fail when no socket exists)
        if command == "codex-hook" {
            guard ProcessInfo.processInfo.environment["NORI_SURFACE_ID"] != nil else {
                print("{}")
                return
            }
        }
        ...
        // Cursor hook handler: gracefully no-op when not inside nori
        ...
        // Gemini hook handler: gracefully no-op when not inside nori
        ...
        // Generic agent hook handlers: gracefully no-op outside nori
        ...
        case "rpc":
            guard let method = commandArgs.first?.trimmingCharacters(in: .whitespacesAndNewlines),
                  !method.isEmpty else {
                throw CLIError(message: "Usage: nori rpc <method> [json-params]")
            }
        ...
        case "identify":
            ...
            if includeCaller {
                ...
                let workspaceArg = idWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["NORI_WORKSPACE_ID"] : nil)
                let surfaceArg = optionValue(commandArgs, name: "--surface") ?? (idWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["NORI_SURFACE_ID"] : nil)
        ...
        case "new-split":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? ProcessInfo.processInfo.environment["NORI_WORKSPACE_ID"] : nil)
            let surfaceRaw = sfArg ?? panelArg ?? (wsArg == nil && windowId == nil ? ProcessInfo.processInfo.environment["NORI_SURFACE_ID"] : nil)
        ...
        case "close-surface":
            ...
            let workspaceArg = csWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["NORI_WORKSPACE_ID"] : nil)
            let surfaceRaw = optionValue(commandArgs, name: "--surface") ?? optionValue(commandArgs, name: "--panel") ?? (csWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["NORI_SURFACE_ID"] : nil)
            var params: [String: Any] = [:]
            let wsId = try normalizeWorkspaceHandle(workspaceArg, client: client)
            if let wsId { params["workspace_id"] = wsId }
            let sfId = try normalizeSurfaceHandle(surfaceRaw, client: client, workspaceHandle: wsId)
        ...
        case "trigger-flash":
            ...
            let callerWorkspaceArg = preferTTYFallback
                ? nil
                : (windowId == nil ? ProcessInfo.processInfo.environment["NORI_WORKSPACE_ID"] : nil)
            ...
            let callerSurfaceArg = explicitSurfaceArg == nil && preferTTYFallback == false && windowId == nil
                ? ProcessInfo.processInfo.environment["NORI_SURFACE_ID"]
                : nil
        ...
        case "read-screen":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? ProcessInfo.processInfo.environment["NORI_WORKSPACE_ID"] : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowId == nil ? ProcessInfo.processInfo.environment["NORI_SURFACE_ID"] : nil)
        ...
        case "clear-notifications":
            ...
            if let wsFlag = optionValue(commandArgs, name: "--workspace") {
                ...
                socketCmd += " --tab=\(wsId)"
                      ...
                      let envWs = ProcessInfo.processInfo.environment["NORI_WORKSPACE_ID"],
    ...
    private func runMarkdownCommand(
        ...
        if let first = args.first, first.lowercased() == "open" {
            ...
            if let first = args.first, first.hasPrefix("-") {
                throw CLIError(
                    message:
                        "markdown open: unknown flag '\(first)'. Usage: nori markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
                )
            } else if let first = args.first, looksLikePath(first) || first.contains(".") {
                subArgs = args
            } else if let first = args.first {
                throw CLIError(message: "Unknown markdown subcommand: \(first). Usage: nori markdown open <path>")
            } else {
                subArgs = []
            }
        }
        ...
        guard let rawPath = subArgs.first, !rawPath.isEmpty else {
            throw CLIError(message: "markdown open requires a file path. Usage: nori markdown open <path>")
        }
        let trailingArgs = Array(subArgs.dropFirst())
        if let unknownFlag = trailingArgs.first(where: { $0.hasPrefix("-") }) {
            throw CLIError(
                message:
                    "markdown open: unknown flag '\(unknownFlag)'. Usage: nori markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
            )
        }
        if let extraArg = trailingArgs.first {
            throw CLIError(
                message:
                    "markdown open: unexpected argument '\(extraArg)'. Usage: nori markdown open <path> [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--direction right|down|left|up]"
            )
        }
        ...
        let workspaceRaw = workspaceOpt ?? (windowOpt == nil ? ProcessInfo.processInfo.environment["NORI_WORKSPACE_ID"] : nil)
        if let workspaceRaw {
    ...
    /// Open a path in nori by creating a new workspace with the given directory.
    ...
    private func runFeedback(
        ...
        if emailOpt == nil && bodyOpt == nil && imagePaths.isEmpty {
            var params: [String: Any] = [:]
            let env = ProcessInfo.processInfo.environment
            if let workspaceId = env["NORI_WORKSPACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
               !workspaceId.isEmpty {
    ...
    private func runWorkspaceAction(
        ...
        let workspaceArg = workspaceOpt ?? (windowOverride == nil ? ProcessInfo.processInfo.environment["NORI_WORKSPACE_ID"] : nil)
    ...
    private func runTabAction(
        ...
        let tabArg = tabOpt
            ...
            ?? (workspaceOpt == nil && windowOverride == nil
                ? (ProcessInfo.processInfo.environment["NORI_TAB_ID"] ?? ProcessInfo.processInfo.environment["NORI_SURFACE_ID"])
                : nil)
    ...
    private func runSSH(
        ...
        do {
            ...
            // `nori ssh` is an explicit "open this remote workspace now" action,
    ...
    private func parseSSHCommandOptions(_ commandArgs: [String], localSocketPath: String = "", remoteRelayPort: Int = 0) throws -> SSHCommandOptions {
        ...
        guard let destination else {
            throw CLIError(message: "ssh requires a destination (example: nori ssh user@host)")
        }
    ...
    private func buildSSHBootstrapCommandSnippet(
        ...
        var lines: [String] = [
            "nori_workspace_id=\"${NORI_WORKSPACE_ID:-}\"",
            "nori_surface_id=\"${NORI_SURFACE_ID:-}\"",
            "nori_remote_bootstrap_b64=\(shellQuote(encodedBootstrapScript))",
            "nori_remote_bootstrap=\"$(printf %s \"$nori_remote_bootstrap_b64\" | base64 -d 2>/dev/null || printf %s \"$nori_remote_bootstrap_b64\" | base64 -D 2>/dev/null)\"",
            "nori_remote_bootstrap=\"$(printf '%s' \"$nori_remote_bootstrap\" | sed \"s/__NORI_WORKSPACE_ID__/$nori_workspace_id/g; s/__NORI_SURFACE_ID__/$nori_surface_id/g\")\"",
            "if ! printf '%s' \"$nori_remote_bootstrap\" | command \(installSSHPrefix) -T \(shellQuote(options.destination)) \(shellQuote(remoteBootstrapInstallCommand)); then",
            "  exit 1",
            "fi",
            "nori_remote_command_template=\(shellQuote(remoteCommandTemplate))",
            "nori_remote_command=\"$(printf '%s' \"$nori_remote_command_template\" | sed \"s/__NORI_WORKSPACE_ID__/$nori_workspace_id/g; s/__NORI_SURFACE_ID__/$nori_surface_id/g\")\"",
        ]

        var sshInvocation = "command \(sessionSSHPrefix) -o \"RemoteCommand=$nori_remote_command\""
        if !hasSSHOptionKey(options.sshOptions, key: "RequestTTY") {
            sshInvocation += " -tt"
        }
    ...
    private func stagedRemoteBootstrapCommandShell(
        ...
        lines.append("/bin/sh \"$HOME/.nori/relay/\(remoteRelayPort).bootstrap.sh\"")
        return lines.joined(separator: "\n")
    }
    ...
    private func remoteBootstrapInstallShell(remoteRelayPort: Int) -> String {
        [
            "set -eu",
            "umask 077",
            "nori_bootstrap_path=\"$HOME/.nori/relay/\(remoteRelayPort).bootstrap.sh\"",
            "mkdir -p \"$HOME/.nori/relay\"",
            "cat > \"$nori_bootstrap_path\"",
            "chmod 700 \"$nori_bootstrap_path\" >/dev/null 2>&1 || true",
        ].joined(separator: "\n")
    }
    ...
    private func runtimeEncodedRemoteBootstrapCommandShell(
        ...
        lines += [
            "nori_tmp=$(mktemp \"${TMPDIR:-/tmp}/nori-ssh-bootstrap.XXXXXX\") || exit 1",
            "(printf %s '\(base64Placeholder)' | base64 -d 2>/dev/null || printf %s '\(base64Placeholder)' | base64 -D 2>/dev/null) > \"$nori_tmp\" || { rm -f \"$nori_tmp\"; exit 1; }",
            "chmod 700 \"$nori_tmp\" >/dev/null 2>&1 || true",
            "/bin/sh \"$nori_tmp\"",
            "nori_status=$?",
            "rm -f \"$nori_tmp\"",
            "exit $nori_status",
    ...
    private func remoteBootstrapTTYCaptureLines(
        ...
        var lines: [String] = [
            "nori_bootstrap_tty=\"$(tty 2>/dev/null || true)\"",
            "nori_bootstrap_tty=\"${nori_bootstrap_tty##*/}\"",
            "if [ -n \"$nori_bootstrap_tty\" ] && [ \"$nori_bootstrap_tty\" != \"not a tty\" ]; then",
            "  mkdir -p \"$HOME/.nori/relay\" >/dev/null 2>&1 || true",
            "  printf '%s' \"$nori_bootstrap_tty\" > \"$HOME/.nori/relay/\(remoteRelayPort).tty\" 2>/dev/null || true",
            "  export NORI_BOOTSTRAP_TTY=\"$nori_bootstrap_tty\"",
        ]

        if includeRelayRPC {
            lines += [
                "  nori_relay_cli=\"$HOME/.nori/bin/nori\"",
                "  if [ ! -x \"$nori_relay_cli\" ]; then nori_relay_cli=\"$(command -v nori 2>/dev/null || true)\"; fi",
                "  if [ -n \"$nori_relay_cli\" ]; then",
                "    nori_relay_report_tty='{\"workspace_id\":\"__NORI_WORKSPACE_ID__\",\"tty_name\":\"'$nori_bootstrap_tty'\"}'",
                "    nori_relay_ports_kick='{\"workspace_id\":\"__NORI_WORKSPACE_ID__\",\"reason\":\"command\"}'",
                "    if [ -n \"__NORI_SURFACE_ID__\" ]; then",
                "      nori_relay_report_tty='{\"workspace_id\":\"__NORI_WORKSPACE_ID__\",\"surface_id\":\"__NORI_SURFACE_ID__\",\"tty_name\":\"'$nori_bootstrap_tty'\"}'",
                "      nori_relay_ports_kick='{\"workspace_id\":\"__NORI_WORKSPACE_ID__\",\"surface_id\":\"__NORI_SURFACE_ID__\",\"reason\":\"command\"}'",
                "    fi",
                "    NORI_SOCKET_PATH=\"127.0.0.1:\(remoteRelayPort)\" NORI_SOCKET=\"127.0.0.1:\(remoteRelayPort)\" \"$nori_relay_cli\" rpc surface.report_tty \"$nori_relay_report_tty\" >/dev/null 2>&1 || true",
                "    NORI_SOCKET_PATH=\"127.0.0.1:\(remoteRelayPort)\" NORI_SOCKET=\"127.0.0.1:\(remoteRelayPort)\" \"$nori_relay_cli\" rpc surface.ports_kick \"$nori_relay_ports_kick\" >/dev/null 2>&1 || true",
                "    unset nori_relay_cli nori_relay_report_tty nori_relay_ports_kick",
                "  fi",
            ]
        }
    ...
    func buildInteractiveRemoteShellScript(
        ...
        let remoteCallerExportLines = [
            "if [ -n '__NORI_WORKSPACE_ID__' ]; then export NORI_WORKSPACE_ID='__NORI_WORKSPACE_ID__'; fi",
            "if [ -n '__NORI_WORKSPACE_ID__' ]; then export NORI_TAB_ID='__NORI_WORKSPACE_ID__'; fi",
            "if [ -n '__NORI_SURFACE_ID__' ]; then export NORI_SURFACE_ID='__NORI_SURFACE_ID__'; export NORI_PANEL_ID='__NORI_SURFACE_ID__'; fi",
        ]
        ...
        commonShellExportLines.append("export PATH=\"$HOME/.nori/bin:$PATH\"")
        commonShellExportLines.append("export NORI_BUNDLED_CLI_PATH=\"$HOME/.nori/bin/nori\"")
        commonShellExportLines.append("export NORI_SHELL_INTEGRATION_DIR=\"\(shellStateDir)\"")
        if let relaySocket {
            commonShellExportLines.append("export NORI_SOCKET_PATH=\(relaySocket)")
            commonShellExportLines.append("export NORI_SOCKET=\(relaySocket)")
        }
        ...
        zshShellLines.append(
            #"if [ "${NORI_SHELL_INTEGRATION:-1}" != "0" ] && [ -r "${NORI_SHELL_INTEGRATION_DIR}/nori-zsh-integration.zsh" ]; then . "${NORI_SHELL_INTEGRATION_DIR}/nori-zsh-integration.zsh"; fi"#
        )
        var bashShellLines = commonShellExportLines
        bashShellLines.append(
            #"if [ "${NORI_SHELL_INTEGRATION:-1}" != "0" ] && [ -r "${NORI_SHELL_INTEGRATION_DIR}/nori-bash-integration.bash" ]; then . "${NORI_SHELL_INTEGRATION_DIR}/nori-bash-integration.bash"; fi"#
        )
        ...
        let bundledZshIntegration = bundledShellIntegrationScript(named: "nori-zsh-integration.zsh")
        let bundledBashIntegration = bundledShellIntegrationScript(named: "nori-bash-integration.bash")
        let bashRCLines = [
        ...
        var outerLines: [String] = [
            "mkdir -p \"$HOME/.nori/relay\"",
            "nori_shell_dir=\"\(shellStateDir)\"",
            "mkdir -p \"$nori_shell_dir\"",
        ]
        if let bundledZshIntegration {
            outerLines += [
                "cat > \"$nori_shell_dir/nori-zsh-integration.zsh\" <<'NORINORIZSH'",
        ...
        if let bundledBashIntegration {
            outerLines += [
                "cat > \"$nori_shell_dir/nori-bash-integration.bash\" <<'NORINORIBASH'",
        ...
        outerLines += [
            "NORI_LOGIN_SHELL=\"${SHELL:-/bin/zsh}\"",
            "case \"${NORI_LOGIN_SHELL##*/}\" in",
            "  zsh)",
            "    cat > \"$nori_shell_dir/.zshenv\" <<'NORIZSHENV'",
        ]
        outerLines.append(contentsOf: zshEnvLines)
        outerLines += [
            "NORIZSHENV",
            "    cat > \"$nori_shell_dir/.zprofile\" <<'NORIZSHPROFILE'",
        ]
        outerLines.append(contentsOf: zshProfileLines)
        outerLines += [
            "NORIZSHPROFILE",
            "    cat > \"$nori_shell_dir/.zshrc\" <<'NORIZSHRC'",
        ]
        outerLines.append(contentsOf: zshRCLines)
        outerLines += [
            "NORIZSHRC",
            "    cat > \"$nori_shell_dir/.zlogin\" <<'NORIZSHLOGIN'",
        ]
        outerLines.append(contentsOf: zshLoginLines)
        outerLines += [
            "NORIZSHLOGIN",
            "    chmod 600 \"$nori_shell_dir/.zshenv\" \"$nori_shell_dir/.zprofile\" \"$nori_shell_dir/.zshrc\" \"$nori_shell_dir/.zlogin\" >/dev/null 2>&1 || true",
        ]
        outerLines.append(contentsOf: relayWarmupLines.map { "    " + $0 })
        outerLines += [
            "    export NORI_REAL_ZDOTDIR=\"${ZDOTDIR:-$HOME}\"",
            "    export ZDOTDIR=\"$nori_shell_dir\"",
            "    exec \"$NORI_LOGIN_SHELL\" -il",
            "    ;;",
            "  bash)",
            "    cat > \"$nori_shell_dir/.bashrc\" <<'NORIBASHRC'",
        ]
        outerLines.append(contentsOf: bashRCLines)
        outerLines += [
            "NORIBASHRC",
            "    chmod 600 \"$nori_shell_dir/.bashrc\" >/dev/null 2>&1 || true",
        ]
        outerLines.append(contentsOf: relayWarmupLines.map { "    " + $0 })
        outerLines += [
            "    exec \"$NORI_LOGIN_SHELL\" --rcfile \"$nori_shell_dir/.bashrc\" -i",
            "    ;;",
            "  *)",
        ]
        outerLines.append(contentsOf: commonShellExportLines)
    ...
    private func shellStateDirForRemoteRelayPort(_ remoteRelayPort: Int) -> String {
        "$HOME/.nori/relay/\(max(remoteRelayPort, 0)).shell"
    }
    ...
    private func interactiveRemoteRelayWarmupLines(remoteRelayPort: Int) -> [String] {
        ...
        return [
            "nori_relay_cli=\"${NORI_BUNDLED_CLI_PATH:-$HOME/.nori/bin/nori}\"",
            "if [ ! -x \"$nori_relay_cli\" ]; then nori_relay_cli=\"$(command -v nori 2>/dev/null || true)\"; fi",
            "nori_relay_tty=\"${NORI_BOOTSTRAP_TTY:-}\"",
            "if [ -z \"$nori_relay_tty\" ]; then nori_relay_tty=\"$(tty 2>/dev/null || true)\"; fi",
            "nori_relay_tty=\"${nori_relay_tty##*/}\"",
            "if [ -n \"$nori_relay_tty\" ] && [ \"$nori_relay_tty\" != \"not a tty\" ]; then",
            "  mkdir -p \"$HOME/.nori/relay\" >/dev/null 2>&1 || true",
            "  printf '%s' \"$nori_relay_tty\" > \"$HOME/.nori/relay/\(remoteRelayPort).tty\" 2>/dev/null || true",
            "fi",
            "if [ -n \"$nori_relay_cli\" ] && [ -n \"$NORI_WORKSPACE_ID\" ] && [ -n \"$nori_relay_tty\" ] && [ \"$nori_relay_tty\" != \"not a tty\" ]; then",
            "  nori_relay_report_tty=\"{\\\"workspace_id\\\":\\\"$NORI_WORKSPACE_ID\\\",\\\"tty_name\\\":\\\"$nori_relay_tty\\\"}\"",
            "  nori_relay_ports_kick=\"{\\\"workspace_id\\\":\\\"$NORI_WORKSPACE_ID\\\",\\\"reason\\\":\\\"command\\\"}\"",
            "  if [ -n \"$NORI_SURFACE_ID\" ]; then",
            "    nori_relay_report_tty=\"{\\\"workspace_id\\\":\\\"$NORI_WORKSPACE_ID\\\",\\\"surface_id\\\":\\\"$NORI_SURFACE_ID\\\",\\\"tty_name\\\":\\\"$nori_relay_tty\\\"}\"",
            "    nori_relay_ports_kick=\"{\\\"workspace_id\\\":\\\"$NORI_WORKSPACE_ID\\\",\\\"surface_id\\\":\\\"$NORI_SURFACE_ID\\\",\\\"reason\\\":\\\"command\\\"}\"",
            "  fi",
            "  \"$nori_relay_cli\" rpc surface.report_tty \"$nori_relay_report_tty\" >/dev/null 2>&1 || true",
            "  \"$nori_relay_cli\" rpc surface.ports_kick \"$nori_relay_ports_kick\" >/dev/null 2>&1 || true",
            "fi",
            "unset NORI_BOOTSTRAP_TTY nori_relay_cli nori_relay_tty nori_relay_report_tty nori_relay_ports_kick",
        ]
    }
    ...
    func encodedRemoteBootstrapCommand(
        ...
        lines += [
            "nori_tmp=$(mktemp \"${TMPDIR:-/tmp}/nori-ssh-bootstrap.XXXXXX\") || exit 1",
            "(printf %s \(encodedLiteral) | base64 -d 2>/dev/null || printf %s \(encodedLiteral) | base64 -D 2>/dev/null) > \"$nori_tmp\" || { rm -f \"$nori_tmp\"; exit 1; }",
            "chmod 700 \"$nori_tmp\" >/dev/null 2>&1 || true",
            "/bin/sh \"$nori_tmp\"",
            "nori_status=$?",
            "rm -f \"$nori_tmp\"",
    ...
    func buildSSHStartupCommand(
        ...
        scriptLines += [
            "NORI_SSH_SESSION_ENDED=0",
            "nori_ssh_session_end() { if [ \"${NORI_SSH_SESSION_ENDED:-0}\" = 1 ]; then return; fi; NORI_SSH_SESSION_ENDED=1; \(lifecycleCleanup); }",
            "trap 'nori_ssh_session_end' EXIT HUP INT TERM",
        ]
        if isShellSnippet {
        ...
        scriptLines += [
            ...
            "nori_ssh_session_end",
            "exit $nori_ssh_status",
        ]
    ...
    private func writeSSHStartupScript(_ scriptBody: String, remoteRelayPort: Int) throws -> String {
        ...
        let scriptURL = tempDir.appendingPathComponent(
            "nori-ssh-startup-\(remoteRelayPort)-\(UUID().uuidString.lowercased()).sh"
        )
        let script = "#!/bin/sh\n\(scriptBody)\n"
    ...
    private func buildSSHSessionEndShellCommand(remoteRelayPort: Int) -> String {
        [
            "if [ -n \"${NORI_BUNDLED_CLI_PATH:-}\" ]",
            "&& [ -x \"${NORI_BUNDLED_CLI_PATH}\" ]",
            "&& [ -n \"${NORI_SOCKET_PATH:-}\" ]",
            "&& [ -n \"${NORI_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${NORI_SURFACE_ID:-}\" ]; then",
            "\"${NORI_BUNDLED_CLI_PATH}\" --socket \"${NORI_SOCKET_PATH}\" ssh-session-end --relay-port \(remoteRelayPort) --workspace \"${NORI_WORKSPACE_ID}\" --surface \"${NORI_SURFACE_ID}\" >/dev/null 2>&1 || true;",
            "elif command -v nori >/dev/null 2>&1",
            "&& [ -n \"${NORI_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${NORI_SURFACE_ID:-}\" ]; then",
            "nori ssh-session-end --relay-port \(remoteRelayPort) --workspace \"${NORI_WORKSPACE_ID}\" --surface \"${NORI_SURFACE_ID}\" >/dev/null 2>&1 || true;",
            "fi",
        ].joined(separator: " ")
    }
    ...
    private func runSSHSessionEnd(commandArgs: [String], client: SocketClient) throws {
        ...
        let workspaceRaw = optionValue(commandArgs, name: "--workspace") ?? ProcessInfo.processInfo.environment["NORI_WORKSPACE_ID"]
        let surfaceRaw = optionValue(commandArgs, name: "--surface") ?? ProcessInfo.processInfo.environment["NORI_SURFACE_ID"]
        guard let workspaceRaw,
              let workspaceId = try normalizeWorkspaceHandle(workspaceRaw, client: client),
              !workspaceId.isEmpty else {
            throw CLIError(message: "ssh-session-end requires --workspace or NORI_WORKSPACE_ID")
        }
        guard let surfaceRaw,
            ...
            throw CLIError(message: "ssh-session-end requires --surface or NORI_SURFACE_ID")
        }
    ...
    private func runRemoteDaemonStatus(commandArgs: [String], jsonOutput: Bool) throws {
        ...
        let downloadCommand = "gh release download \(releaseTag) --repo manaflow-ai/nori --pattern \(assetName)"
        let downloadChecksumsCommand = "gh release download \(releaseTag) --repo manaflow-ai/nori --pattern \(checksumsAssetName)"
        let checksumVerifyCommand = "shasum -a 256 -c \(checksumsAssetName) --ignore-missing"
        let signerWorkflow = releaseTag == "nightly"
            ? "manaflow-ai/nori/.github/workflows/nightly.yml"
            : "manaflow-ai/nori/.github/workflows/release.yml"
        let verifyCommand = "gh attestation verify ./\(assetName) --repo manaflow-ai/nori --signer-workflow \(signerWorkflow)"

        let payload: [String: Any] = [
            "app_version": remoteDaemonVersionString(from: info),
            "build": info["CFBundleVersion"] ?? NSNull(),
            "commit": info["NORICommit"] ?? NSNull(),
            ...
            "dev_local_build_fallback": ProcessInfo.processInfo.environment["NORI_REMOTE_DAEMON_ALLOW_LOCAL_BUILD"] == "1",
        ...
        if manifest == nil {
            print("note: this build has no embedded remote daemon manifest. Set NORI_REMOTE_DAEMON_ALLOW_LOCAL_BUILD=1 only for dev builds.")
        }
    }
    ...
    private func remoteDaemonManifest() -> RemoteDaemonManifest? {
        for plistURL in candidateInfoPlistURLs() {
            guard let raw = NSDictionary(contentsOf: plistURL) as? [String: Any],
                  let rawManifest = raw["NORIRemoteDaemonManifestJSON"] as? String,
    ...
    private func remoteDaemonCacheURL(version: String, goOS: String, goArch: String) -> URL {
        ...
        do {
            ...
            return URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
                .appendingPathComponent("nori-remote-daemons", isDirectory: true)
    ...
    private func deferredRemoteReconnectLocalCommand(
        ...
        let escapedForegroundAuthToken = foregroundAuthToken
            ...
            .replacingOccurrences(of: "\"", with: "\\\"")
        return [
            preferredCLIPath.map { "nori_reconnect_cli=\(shellQuote($0));" } ?? "nori_reconnect_cli=\"\";",
            "nori_reconnect_socket=\"${NORI_SOCKET_PATH:-${NORI_SOCKET:-}}\";",
            "if [ -z \"$nori_reconnect_cli\" ] && [ -n \"${NORI_BUNDLED_CLI_PATH:-}\" ]; then nori_reconnect_cli=\"$NORI_BUNDLED_CLI_PATH\"; fi;",
            "if [ ! -x \"$nori_reconnect_cli\" ]; then nori_reconnect_cli=\"$(command -v nori 2>/dev/null || true)\"; fi;",
            "if [ -n \"${NORI_WORKSPACE_ID:-}\" ]; then",
            "if [ -z \"$nori_reconnect_socket\" ]; then printf '%s\\n' 'nori: deferred SSH reconnect skipped, local nori socket not found' >&2;",
            "elif [ -z \"$nori_reconnect_cli\" ] || [ ! -x \"$nori_reconnect_cli\" ]; then printf '%s\\n' 'nori: deferred SSH reconnect skipped, local nori CLI not found' >&2;",
            "else",
            "nori_reconnect_payload=\"{\\\"workspace_id\\\":\\\"$NORI_WORKSPACE_ID\\\",\\\"foreground_auth_token\\\":\\\"\(escapedForegroundAuthToken)\\\"}\";",
            "\"$nori_reconnect_cli\" --socket \"$nori_reconnect_socket\" rpc workspace.remote.foreground_auth_ready \"$nori_reconnect_payload\" >/dev/null 2>&1 || true;",
            "unset nori_reconnect_payload;",
            "fi;",
            "fi;",
            "unset nori_reconnect_socket nori_reconnect_cli;",
        ].joined(separator: " ")
    }
    ...
    private func defaultSSHControlPathTemplate(remoteRelayPort: Int? = nil) -> String {
        if let remoteRelayPort, remoteRelayPort > 0 {
            return "/tmp/nori-ssh-\(getuid())-\(remoteRelayPort)-%C"
        }
        return "/tmp/nori-ssh-\(getuid())-%C"
    }
    ...
    private func cliDebugLog(_ message: @autoclosure () -> String) {
#if DEBUG
        let trimmedExplicit = ProcessInfo.processInfo.environment["NORI_DEBUG_LOG"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let path: String? = {
            if let trimmedExplicit, !trimmedExplicit.isEmpty {
                return trimmedExplicit
            }
            guard let marker = try? String(contentsOfFile: "/tmp/nori-last-debug-log-path", encoding: .utf8) else {
                return nil
            }
        ...
        let line = "\(timestamp) [nori-cli] \(message())\n"
    ...
    private func runBrowserCommand(
        ...
        func displaySnapshotText(_ payload: [String: Any]) -> String {
            ...
            if url.isEmpty || url == "about:blank" {
                lines.append("hint: run 'nori browser <surface> get url' to verify navigation")
            }
        ...
        func displayBrowserValue(_ value: Any) -> String {
            if let dict = value as? [String: Any],
               let type = dict["__nori_t"] as? String,
               type == "undefined" {
                return "undefined"
            }
            if value is NSNull {
        ...
        if subcommand == "open" || subcommand == "open-split" || subcommand == "new" {
            ...
            let respectExternalOpenRules: Bool = {
                guard let raw = ProcessInfo.processInfo.environment["NORI_RESPECT_EXTERNAL_OPEN_RULES"] else {
                    return false
                }
            ...
            if let outPathOpt {
                ...
                if !hasText(screenshotPath) && !hasText(screenshotURL) {
                    let outputDir = FileManager.default.temporaryDirectory
                        .appendingPathComponent("nori-browser-screenshots-cli", isDirectory: true)
    ...
    private func subcommandUsage(_ command: String) -> String? {
        ...
        case "ping":
            ...
            Check connectivity to the nori socket server.
            """
        case "capabilities":
            return """
            Usage: nori capabilities

            Print server capabilities as JSON.
            """
        case "rpc":
            return """
            Usage: nori rpc <method> [json-params]

            Call a raw v2 method with an optional JSON object for params.
            Example: nori rpc surface.report_tty '{"workspace_id":"...","surface_id":"...","tty_name":"ttys001"}'
            """
        case "help":
            return """
            Usage: nori help
        ...
        case "welcome":
            ...
            Show a welcome screen with the nori logo and useful shortcuts.
            Auto-runs once on first launch.
            """
        case "shortcuts":
            return """
            Usage: nori shortcuts
        ...
        case "feedback":
            return """
            Usage: nori feedback
                   nori feedback --email <email> --body <text> [--image <path> ...]
        ...
        case "themes":
            ...
            Usage: nori themes
                   nori themes list
                   nori themes set <theme>
                   nori themes set --light <theme> [--dark <theme>]
                   nori themes set --dark <theme> [--light <theme>]
                   nori themes clear

            When run in a TTY, `nori themes` opens an interactive theme picker with
            live app preview. Use `nori themes list` for a plain listing.

            The picker previews the selected theme across the running nori app and
            ...
            Commands:
              ...
              clear                     Remove the nori theme override and fall back to other config

            Examples:
              nori themes
              nori themes list
              nori themes set "Catppuccin Mocha"
              nori themes set --light "Catppuccin Latte" --dark "Catppuccin Mocha"
              nori themes clear
            """
        case "claude-teams":
            return String(localized: "cli.claude-teams.usage", defaultValue: """
            Usage: nori claude-teams [claude-args...]

            Launch Claude Code with agent teams enabled.

            This command:
              - defaults Claude teammate mode to auto
              - sets a tmux-like environment so Claude auto mode uses nori splits
            ...
            The tmux shim translates supported tmux window/pane commands into nori
            workspace and split operations in the current nori session.

            Examples:
              nori claude-teams
              nori claude-teams --continue
              nori claude-teams --model sonnet
            """)
        case "omo":
            return String(localized: "cli.omo.usage", defaultValue: """
            Usage: nori omo [opencode-args...]

            Launch OpenCode with oh-my-openagent in a nori-aware environment.
            ...
            nori splits with sidebar metadata and notifications.

            This command:
              - sets a tmux-like environment so oh-my-openagent uses nori splits
              - prepends a private tmux shim to PATH
              - forwards all remaining arguments to opencode

            The tmux shim translates tmux window/pane commands into nori workspace
            and split operations in the current nori session.

            Examples:
              nori omo
              nori omo --continue
              nori omo --model claude-sonnet-4-6
            """)
        case "omx":
            return String(localized: "cli.omx.usage", defaultValue: """
            Usage: nori omx [omx-args...]

            Launch Oh My Codex (OMX) with native nori pane integration.
            ...
            become native nori splits.

            This command:
              - sets a tmux-like environment so OMX uses nori splits
              - prepends a private tmux shim to PATH
            ...
            Examples:
              nori omx
              nori omx --madmax --high
              nori omx team
            """)
        case "omc":
            return String(localized: "cli.omc.usage", defaultValue: """
            Usage: nori omc [omc-args...]

            Launch Oh My Claude Code (OMC) with native nori pane integration.
            ...
            This command:
              - sets a tmux-like environment so OMC uses nori splits
              - prepends a private tmux shim to PATH
            ...
            Examples:
              nori omc
              nori omc team 3:claude "implement feature"
              nori omc --watch
            """)
        case "identify":
            return """
            Usage: nori identify [--workspace <id|ref|index>] [--surface <id|ref|index>] [--no-caller]

            Print server identity and caller context details.

            Flags:
              --workspace <id|ref|index>   Caller workspace context (default: $NORI_WORKSPACE_ID)
              --surface <id|ref|index>     Caller surface context (default: $NORI_SURFACE_ID)
              --no-caller                  Omit caller context from the request
            """
        case "list-windows":
            return """
            Usage: nori list-windows

            List open windows.
            """
        case "current-window":
            return """
            Usage: nori current-window

            Print the currently selected window ID.
            """
        case "new-window":
            return """
            Usage: nori new-window
        ...
        case "focus-window":
            return """
            Usage: nori focus-window --window <id|ref|index>
            ...
            Example:
              nori focus-window --window 0
              nori focus-window --window window:1
            """
        case "close-window":
            return """
            Usage: nori close-window --window <id|ref|index>
            ...
            Example:
              nori close-window --window 0
              nori close-window --window window:1
            """
        case "move-workspace-to-window":
            return """
            Usage: nori move-workspace-to-window --workspace <id|ref|index> --window <id|ref|index>
            ...
            Example:
              nori move-workspace-to-window --workspace workspace:2 --window window:1
            """
        case "move-surface":
            return """
            Usage: nori move-surface [--surface <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              nori move-surface --surface surface:1 --workspace workspace:2
              nori move-surface surface:1 --pane pane:2 --index 0
            """
        case "reorder-surface":
            return """
            Usage: nori reorder-surface [--surface <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              nori reorder-surface --surface surface:1 --index 0
              nori reorder-surface --surface surface:3 --after surface:1
            """
        case "reorder-workspace":
            return """
            Usage: nori reorder-workspace [--workspace <id|ref|index> | <id|ref|index>] [flags]
            ...
            Example:
              nori reorder-workspace --workspace workspace:2 --index 0
              nori reorder-workspace --workspace workspace:3 --after workspace:1
            """
        case "workspace-action":
            return """
            Usage: nori workspace-action --action <name> [flags]
            ...
            Flags:
              ...
              --workspace <id|ref|index>   Target workspace (default: current/$NORI_WORKSPACE_ID)
            ...
            Example:
              nori workspace-action --workspace workspace:2 --action pin
              nori workspace-action --action rename --title "infra"
              nori workspace-action close-others
              nori workspace-action --action set-color --color blue
              nori workspace-action --action set-color --color "#C0392B"
              nori workspace-action set-color Amber
              nori workspace-action --action set-description --description "Ship checklist"
              nori workspace-action --action set-description $'Ship checklist\n- verify build\n- post notes'
              nori workspace-action clear-color
            """
        case "tab-action":
            return """
            Usage: nori tab-action --action <name> [flags]
            ...
            Flags:
              --action <name>              Action name (required if not positional)
              --tab <id|ref|index>         Target tab (accepts tab:<n> or surface:<n>; default: $NORI_TAB_ID, then $NORI_SURFACE_ID, then focused tab)
              --surface <id|ref|index>     Alias for --tab (backward compatibility)
              --workspace <id|ref|index>   Workspace context (default: current/$NORI_WORKSPACE_ID)
              --title <text>               Title for rename (or pass trailing title text)
              --url <url>                  Optional URL for new-browser-right

            Example:
              nori tab-action --tab tab:3 --action pin
              nori tab-action --action close-right
              nori tab-action --tab tab:2 --action rename --title "build logs"
            """
        case "rename-tab":
            return """
            Usage: nori rename-tab [--workspace <id|ref>] [--tab <id|ref>] [--surface <id|ref>] [--] <title>
            ...
            1) --tab
            2) --surface
            3) $NORI_TAB_ID / $NORI_SURFACE_ID
            4) currently focused tab (optionally within --workspace)

            Flags:
              --workspace <id|ref>   Workspace context (default: current/$NORI_WORKSPACE_ID)
            ...
            Examples:
              nori rename-tab "build logs"
              nori rename-tab --tab tab:3 "staging server"
              nori rename-tab --workspace workspace:2 --surface surface:5 --title "agent run"
            """
        case "new-workspace":
            return """
            Usage: nori new-workspace [--name <title>] [--description <text>] [--cwd <path>] [--command <text>]
            ...
            Example:
              nori new-workspace
              nori new-workspace --name "Build Server"
              nori new-workspace --name "Launch" --description "Ship checklist"
              nori new-workspace --cwd ~/projects/myapp
              nori new-workspace --cwd . --command "npm test"
            """
        case "list-workspaces":
            return """
            Usage: nori list-workspaces
        ...
        case "ssh":
            return """
            Usage: nori ssh <destination> [flags] [-- <remote-command-args>]

            Create a new workspace, mark it as remote-SSH, and start an SSH session in that workspace.
            nori will also establish a local SSH proxy endpoint so browser traffic can egress from the remote host.
            ...
            Example:
              nori ssh dev@my-host
              nori ssh dev@my-host --name "gpu-box" --port 2222 --identity ~/.ssh/id_ed25519
              nori ssh dev@my-host --ssh-option UserKnownHostsFile=/dev/null --ssh-option StrictHostKeyChecking=no
            """
        case "remote-daemon-status":
            return """
            Usage: nori remote-daemon-status [--os <darwin|linux>] [--arch <arm64|amd64>]

            Show the embedded norid-remote release manifest, local cache status, checksum verification state,
            and the GitHub attestation verification command for a target platform.

            Example:
              nori remote-daemon-status
              nori remote-daemon-status --os linux --arch arm64
            """
        case "new-split":
            return """
            Usage: nori new-split <left|right|up|down> [flags]

            Split the current pane in the given direction.

            Flags:
              --workspace <id|ref>   Target workspace (default: $NORI_WORKSPACE_ID)
              --surface <id|ref>     Surface to split from (default: $NORI_SURFACE_ID)
              --panel <id|ref>       Alias for --surface

            Example:
              nori new-split right
              nori new-split down --workspace workspace:1
            """
        case "list-panes":
            return """
            Usage: nori list-panes [--workspace <id|ref>]

            List panes in a workspace.

            Flags:
              --workspace <id|ref>   Workspace context (default: $NORI_WORKSPACE_ID)

            Example:
              nori list-panes
              nori list-panes --workspace workspace:2
            """
        case "list-pane-surfaces":
            return """
            Usage: nori list-pane-surfaces [--workspace <id|ref>] [--pane <id|ref>]
            ...
            Example:
              nori list-pane-surfaces
              nori list-pane-surfaces --workspace workspace:2 --pane pane:1
            """
        case "tree":
            return """
            Usage: nori tree [flags]
            ...
            Output:
              ...
              - ◀ here (caller surface where `nori tree` was invoked)
              - workspace [selected]
              - pane [focused]
              - surface [selected]
            ...
            Example:
              nori tree
              nori tree --all
              nori tree --workspace workspace:2
              nori --json tree --all
            """
        case "focus-pane":
            return """
            Usage: nori focus-pane [--pane <id|ref> | <id|ref>] [flags]
            ...
            Example:
              ...
              nori focus-pane pane:1
              nori focus-pane --pane pane:1 --workspace workspace:2
            """
        case "new-pane":
            return """
            Usage: nori new-pane [flags]
            ...
            Example:
              nori new-pane
              nori new-pane --type browser --direction down --url https://example.com
            """
        case "new-surface":
            return """
            Usage: nori new-surface [flags]
        ...
        case "close-surface":
            return """
            Usage: nori close-surface [flags]

            Close a surface. Defaults to the focused surface if none specified.

            Flags:
              --surface <id|ref>     Surface to close (default: $NORI_SURFACE_ID)
              --panel <id|ref>       Alias for --surface
              --workspace <id|ref>   Workspace context (default: $NORI_WORKSPACE_ID)

            Example:
              nori close-surface
              nori close-surface --surface surface:3
            """
        case "drag-surface-to-split":
            return """
            Usage: nori drag-surface-to-split --surface <id|ref> <left|right|up|down>
            ...
            Example:
              nori drag-surface-to-split --surface surface:1 right
              nori drag-surface-to-split --panel surface:2 down
            """
        case "refresh-surfaces":
            return """
            Usage: nori refresh-surfaces

            Refresh surface snapshots for the focused workspace.
            """
        case "reload-config":
            return """
            Usage: nori reload-config

            Run the same configuration reload as the Reload Configuration shortcut.
            This reloads Ghostty config, re-reads ~/.config/nori/settings.json, and refreshes terminals.
        ...
        case "surface-health":
            return """
            Usage: nori surface-health [--workspace <id|ref>]
        ...
        case "trigger-flash":
            return """
            Usage: nori trigger-flash [--workspace <id|ref>] [--surface <id|ref>] [--panel <id|ref>]

            Trigger the unread flash indicator for a surface.

            Flags:
              --workspace <id|ref>   Workspace context (default: $NORI_WORKSPACE_ID)
              --surface <id|ref>     Target surface (default: $NORI_SURFACE_ID)
              --panel <id|ref>       Alias for --surface

            Example:
              nori trigger-flash
              nori trigger-flash --workspace workspace:2 --surface surface:3
            """
        case "list-panels":
            return """
            Usage: nori list-panels [--workspace <id|ref>]
            ...
            Example:
              nori list-panels
              nori list-panels --workspace workspace:2
            """
        case "focus-panel":
            return """
            Usage: nori focus-panel --panel <id|ref> [--workspace <id|ref>]
            ...
            Example:
              nori focus-panel --panel surface:2
              nori focus-panel --panel surface:5 --workspace workspace:2
            """
        case "close-workspace":
            return """
            Usage: nori close-workspace --workspace <id|ref|index>
            ...
            Example:
              nori close-workspace --workspace workspace:2
            """
        case "select-workspace":
            return """
            Usage: nori select-workspace --workspace <id|ref|index>
            ...
            Example:
              nori select-workspace --workspace workspace:2
              nori select-workspace --workspace 0
            """
        case "rename-workspace", "rename-window":
            return """
            Usage: nori rename-workspace [--workspace <id|ref|index>] [--] <title>

            Rename a workspace. Defaults to the current workspace.
            tmux-compatible alias: rename-window

            Flags:
              --workspace <id|ref|index>   Workspace to rename (default: current/$NORI_WORKSPACE_ID)

            Example:
              nori rename-workspace "backend logs"
              nori rename-window --workspace workspace:2 "agent run"
            """
        case "current-workspace":
            return """
            Usage: nori current-workspace

            Print the currently selected workspace ID.
            """
        case "capture-pane":
            return """
            Usage: nori capture-pane [--workspace <id|ref>] [--surface <id|ref>] [--scrollback] [--lines <n>]

            tmux-compatible alias for reading terminal text from a pane.

            Flags:
              --workspace <id|ref>   Workspace context (default: $NORI_WORKSPACE_ID)
              --surface <id|ref>     Surface context (default: $NORI_SURFACE_ID)
              --scrollback           Include scrollback
              --lines <n>            Return only the last N lines (implies --scrollback)

            Example:
              nori capture-pane --workspace workspace:2 --surface surface:1 --scrollback --lines 200
            """
        case "resize-pane":
            return """
            Usage: nori resize-pane [--pane <id|ref>] [--workspace <id|ref>] [-L|-R|-U|-D] [--amount <n>]
        ...
        case "pipe-pane":
            return """
            Usage: nori pipe-pane [--workspace <id|ref>] [--surface <id|ref>] [--command <shell-command> | <shell-command>]
        ...
        case "wait-for":
            return """
            Usage: nori wait-for [-S|--signal] <name> [--timeout <seconds>]
        ...
        case "swap-pane":
            return """
            Usage: nori swap-pane --pane <id|ref> --target-pane <id|ref> [--workspace <id|ref>]
        ...
        case "break-pane":
            return """
            Usage: nori break-pane [--workspace <id|ref>] [--pane <id|ref>] [--surface <id|ref>] [--no-focus]
        ...
        case "join-pane":
            return """
            Usage: nori join-pane --target-pane <id|ref> [--workspace <id|ref>] [--pane <id|ref>] [--surface <id|ref>] [--no-focus]
        ...
        case "last-pane":
            return """
            Usage: nori last-pane [--workspace <id|ref>]
        ...
        case "find-window":
            return """
            Usage: nori find-window [--content] [--select] [query]
        ...
        case "clear-history":
            return """
            Usage: nori clear-history [--workspace <id|ref>] [--surface <id|ref>]
        ...
        case "set-hook":
            return """
            Usage: nori set-hook [--list] [--unset <event>] | <event> <command>
        ...
        case "set-buffer":
            return """
            Usage: nori set-buffer [--name <name>] [--] <text>
        ...
        case "paste-buffer":
            return """
            Usage: nori paste-buffer [--name <name>] [--workspace <id|ref>] [--surface <id|ref>]
        ...
        case "list-buffers":
            return """
            Usage: nori list-buffers

            List tmux-compat buffers.
            """
        case "respawn-pane":
            return """
            Usage: nori respawn-pane [--workspace <id|ref>] [--surface <id|ref>] [--command <cmd> | <cmd>]
        ...
        case "display-message":
            return """
            Usage: nori display-message [-p|--print] <text>
        ...
        case "read-screen":
            ...
            Example:
              nori read-screen
              nori read-screen --surface surface:2 --scrollback --lines 200
            """
        case "send":
            return """
            Usage: nori send [flags] [--] <text>
            ...
            Example:
              nori send "echo hello"
              nori send --surface surface:2 "ls -la\\n"
            """
        case "send-key":
            return """
            Usage: nori send-key [flags] [--] <key>
            ...
            Example:
              nori send-key enter
              nori send-key --surface surface:2 ctrl+c
            """
        case "send-panel":
            return """
            Usage: nori send-panel --panel <id|ref> [flags] [--] <text>
            ...
            Example:
              nori send-panel --panel surface:2 "echo hello\\n"
            """
        case "send-key-panel":
            return """
            Usage: nori send-key-panel --panel <id|ref> [flags] [--] <key>
            ...
            Example:
              nori send-key-panel --panel surface:2 enter
              nori send-key-panel --panel surface:2 ctrl+c
            """
        case "notify":
            return """
            Usage: nori notify [flags]
            ...
            Example:
              nori notify --title "Build done" --body "All tests passed"
              nori notify --title "Error" --subtitle "test.swift" --body "Line 42: syntax error"
            """
        case "list-notifications":
            return """
            Usage: nori list-notifications

            List queued notifications.
            """
        case "clear-notifications":
            return """
            Usage: nori clear-notifications

            Clear all queued notifications.
            """
        case "set-status":
            return """
            Usage: nori set-status <key> <value> [flags]
            ...
            Example:
              nori set-status build "compiling" --icon hammer --color "#ff9500"
              nori set-status deploy "v1.2.3" --workspace workspace:2
            """
        case "clear-status":
            return """
            Usage: nori clear-status <key> [flags]
            ...
            Example:
              nori clear-status build
            """
        case "list-status":
            return """
            Usage: nori list-status [flags]
            ...
            Example:
              nori list-status
              nori list-status --workspace workspace:2
            """
        case "set-progress":
            return """
            Usage: nori set-progress <0.0-1.0> [flags]
            ...
            Example:
              nori set-progress 0.5 --label "Building..."
              nori set-progress 1.0 --label "Done"
            """
        case "clear-progress":
            return """
            Usage: nori clear-progress [flags]
        ...
        case "log":
            ...
            Example:
              nori log "Build started"
              nori log --level error --source build "Compilation failed"
              nori log --level success -- "All 42 tests passed"
            """
        case "clear-log":
            return """
            Usage: nori clear-log [flags]
        ...
        case "list-log":
            ...
            Example:
              nori list-log
              nori list-log --limit 5
            """
        case "sidebar-state":
            return """
            Usage: nori sidebar-state [flags]
            ...
            Example:
              nori sidebar-state
              nori sidebar-state --workspace workspace:2
            """
        case "set-app-focus":
            return """
            Usage: nori set-app-focus <active|inactive|clear>

            Override app focus state for notification routing tests.

            Example:
              nori set-app-focus inactive
              nori set-app-focus clear
            """
        case "simulate-app-active":
            return """
            Usage: nori simulate-app-active

            Trigger the app-active handler used by notification focus tests.
            """
        case "claude-hook":
            return """
            Usage: nori claude-hook <session-start|active|stop|idle|notification|notify|prompt-submit> [flags]
            ...
            Example:
              echo '{"session_id":"abc"}' | nori claude-hook session-start
              echo '{}' | nori claude-hook stop
            """
        case "codex":
            return """
            Usage: nori codex <install-hooks|uninstall-hooks>

            Manage Codex CLI hooks integration.

            Subcommands:
              install-hooks     Install nori hooks into ~/.codex/hooks.json
              uninstall-hooks   Remove nori hooks from ~/.codex/hooks.json
            """
        case "codex-hook":
            return """
            Usage: nori codex-hook <session-start|prompt-submit|stop> [flags]

            Hook for Codex CLI integration. Reads JSON from stdin.
            Gracefully no-ops when not running inside nori.
        ...
        case "browser":
            return """
            Usage: nori browser [--surface <id|ref|index> | <surface>] <subcommand> [args]
            ...
            Subcommands:
              open|open-split|new [url] [--workspace <id|ref|index>] [--window <id|ref|index>]
                open/open-split/new default to $NORI_WORKSPACE_ID when --workspace is omitted and --window is not set
            ...
            Example:
              nori browser open https://example.com
              nori browser surface:1 navigate https://google.com
              nori browser --surface surface:1 snapshot --interactive
            """
        // Legacy browser aliases — point users to `nori browser --help`
        case "open-browser":
            return "Legacy alias for 'nori browser open'. Run 'nori browser --help' for details."
        case "navigate":
            return "Legacy alias for 'nori browser navigate'. Run 'nori browser --help' for details."
        case "browser-back":
            return "Legacy alias for 'nori browser back'. Run 'nori browser --help' for details."
        case "browser-forward":
            return "Legacy alias for 'nori browser forward'. Run 'nori browser --help' for details."
        case "browser-reload":
            return "Legacy alias for 'nori browser reload'. Run 'nori browser --help' for details."
        case "get-url":
            return "Legacy alias for 'nori browser get-url'. Run 'nori browser --help' for details."
        case "focus-webview":
            return "Legacy alias for 'nori browser focus-webview'. Run 'nori browser --help' for details."
        case "is-webview-focused":
            return "Legacy alias for 'nori browser is-webview-focused'. Run 'nori browser --help' for details."
        case "markdown":
            return """
            Usage: nori markdown open <path> [options]
                   nori markdown <path>       (shorthand for 'open')
            ...
            Options:
              --workspace <id|ref|index>   Target workspace (default: $NORI_WORKSPACE_ID)
            ...
            Examples:
              ...
              nori markdown ~/project/CHANGELOG.md
              nori markdown open ./docs/design.md --workspace 0
              nori markdown open plan.md --direction down
            """
        default:
            return nil
        }
    ...
    private static let noriThemeOverrideBundleIdentifier = "com.nori.app"
    private static let noriThemesBlockStart = "# nori themes start"
    private static let noriThemesBlockEnd = "# nori themes end"
    private static let noriThemesReloadNotificationName = "com.nori.themes.reload-config"
    ...
    private func runInteractiveThemes() throws {
        ...
        environment["NORI_THEME_PICKER_CONFIG"] = try noriThemeOverrideConfigURL().path
        environment["NORI_THEME_PICKER_BUNDLE_ID"] = currentNoriAppBundleIdentifier() ?? Self.noriThemeOverrideBundleIdentifier
        environment["NORI_THEME_PICKER_TARGET"] = defaultThemePickerTargetMode(current: selection).rawValue
        environment["NORI_THEME_PICKER_COLOR_SCHEME"] = defaultAppearancePrefersDarkThemes() ? "dark" : "light"
        if let light = selection.light {
            environment["NORI_THEME_PICKER_INITIAL_LIGHT"] = light
        }
        if let dark = selection.dark {
            environment["NORI_THEME_PICKER_INITIAL_DARK"] = dark
        }
    ...
    private func runThemes(commandArgs: [String], jsonOutput: Bool) throws {
        ...
        default:
            if subcommand.hasPrefix("-") {
                throw CLIError(message: "Unknown themes subcommand '\(subcommand)'. Run 'nori themes --help'.")
            }
    ...
    private func printThemesList(jsonOutput: Bool) throws {
        ...
        let configPath = try noriThemeOverrideConfigURL().path
    ...
    private func validatedThemeName(_ rawValue: String, availableThemes: [String]) throws -> String {
        ...
        throw CLIError(message: "Unknown theme '\(trimmed)'. Run 'nori themes' to list available themes.")
    }
    ...
    private func themeConfigSearchURLs() -> [URL] {
        let rawPaths = [
            ...
            "~/Library/Application Support/\(Self.noriThemeOverrideBundleIdentifier)/config",
            "~/Library/Application Support/\(Self.noriThemeOverrideBundleIdentifier)/config.ghostty",
        ]
    ...
    private func noriThemeOverrideConfigURL() throws -> URL {
        guard let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
            throw CLIError(message: "Unable to resolve Application Support directory")
        }
        return appSupport
            .appendingPathComponent(Self.noriThemeOverrideBundleIdentifier, isDirectory: true)
            .appendingPathComponent("config.ghostty", isDirectory: false)
    }

    private func writeManagedThemeOverride(rawThemeValue: String) throws -> URL {
        let fileManager = FileManager.default
        let configURL = try noriThemeOverrideConfigURL()
        ...
        let strippedContents = removingManagedThemeOverride(from: existingContents)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let block = """
        \(Self.noriThemesBlockStart)
        theme = \(rawThemeValue)
        \(Self.noriThemesBlockEnd)
        """
    ...
    private func reloadThemesIfPossible() -> ThemeReloadStatus {
        let bundleIdentifier = currentNoriAppBundleIdentifier() ?? Self.noriThemeOverrideBundleIdentifier
        DistributedNotificationCenter.default().post(
            name: Notification.Name(Self.noriThemesReloadNotificationName),
            object: nil,
    ...
    private func currentNoriAppBundleIdentifier() -> String? {
        if let bundleIdentifier = ProcessInfo.processInfo.environment["NORI_BUNDLE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !bundleIdentifier.isEmpty {
            return bundleIdentifier
        }
    ...
    private func workspaceFromArgsOrEnv(_ args: [String], windowOverride: String? = nil) -> String? {
        ...
        return ProcessInfo.processInfo.environment["NORI_WORKSPACE_ID"]
    }
    ...
    private func treeCallerContextFromEnvironment() -> [String: Any]? {
        let env = ProcessInfo.processInfo.environment
        let workspaceRaw = env["NORI_WORKSPACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines)
        let surfaceRaw = env["NORI_SURFACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines)
        var caller: [String: Any] = [:]
    ...
    private func tmuxCallerWorkspaceHandle() -> String? {
        normalizedTmuxTarget(ProcessInfo.processInfo.environment["NORI_WORKSPACE_ID"])
    }
    ...
    private func tmuxCallerPaneHandle() -> String? {
        guard let pane = normalizedTmuxTarget(ProcessInfo.processInfo.environment["TMUX_PANE"])
            ?? normalizedTmuxTarget(ProcessInfo.processInfo.environment["NORI_PANE_ID"]) else {
            return nil
        }
    ...
    private func tmuxCallerSurfaceHandle() -> String? {
        normalizedTmuxTarget(ProcessInfo.processInfo.environment["NORI_SURFACE_ID"])
    }
    ...
    private func tmuxResolveSurfaceTarget(
        ...
        if tmuxPaneSelector(from: raw) != nil {
            ...
            // exact surface (NORI_SURFACE_ID) over the pane's currently selected
    ...
    private func isNoriClaudeWrapper(at path: String) -> Bool {
        guard let data = FileManager.default.contents(atPath: path) else { return false }
        let prefixData = data.prefix(512)
        guard let prefix = String(data: prefixData, encoding: .utf8) else { return false }
        return prefix.contains("nori claude wrapper - injects hooks and session tracking")
    }
    ...
    private func resolveClaudeExecutable(searchPath: String?) -> String? {
        resolveExecutableInSearchPath(
            ...
            skip: { self.isNoriClaudeWrapper(at: $0) }
        )
    }
    ...
    private func configureTmuxCompatEnvironment(
        ...
        if let focusedContext {
            setenv("NORI_WORKSPACE_ID", focusedContext.workspaceId, 1)
            if let surfaceId = focusedContext.surfaceId, !surfaceId.isEmpty {
                setenv("NORI_SURFACE_ID", surfaceId, 1)
            }
        }
    }

    private static let claudeNodeOptionsRestoreModule = """
    const hadOriginalNodeOptions = process.env.NORI_ORIGINAL_NODE_OPTIONS_PRESENT === "1";
    if (hadOriginalNodeOptions) {
        process.env.NODE_OPTIONS = process.env.NORI_ORIGINAL_NODE_OPTIONS ?? "";
    } else {
        delete process.env.NODE_OPTIONS;
    }
    delete process.env.NORI_ORIGINAL_NODE_OPTIONS;
    delete process.env.NORI_ORIGINAL_NODE_OPTIONS_PRESENT;
    """
    ...
    private func configureClaudeTeamsEnvironment(
        ...
        configureTmuxCompatEnvironment(
            ...
            tmuxPathPrefix: "nori-claude-teams",
            noriBinEnvVar: "NORI_CLAUDE_TEAMS_NORI_BIN",
            termOverrideEnvVar: "NORI_CLAUDE_TEAMS_TERM",
            extraEnvVars: [
                (key: "CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS", value: "1"),
            ]
        )
        guard let restoreModuleURL = try? createClaudeNodeOptionsRestoreModule() else {
            unsetenv("NORI_ORIGINAL_NODE_OPTIONS_PRESENT")
            unsetenv("NORI_ORIGINAL_NODE_OPTIONS")
            return
        }
        if let existing = processEnvironment["NODE_OPTIONS"] {
            setenv("NORI_ORIGINAL_NODE_OPTIONS_PRESENT", "1", 1)
            setenv("NORI_ORIGINAL_NODE_OPTIONS", existing, 1)
        } else {
            setenv("NORI_ORIGINAL_NODE_OPTIONS_PRESENT", "0", 1)
            unsetenv("NORI_ORIGINAL_NODE_OPTIONS")
        }
        setenv(
            "NODE_OPTIONS",
    ...
    private func createTmuxCompatShimDirectory(
        ...
        let root = URL(fileURLWithPath: homePath, isDirectory: true)
            .appendingPathComponent(".noriterm", isDirectory: true)
            .appendingPathComponent(directoryName, isDirectory: true)
    ...
    private func createClaudeTeamsShimDirectory() throws -> URL {
        ...
        #!/usr/bin/env bash
        set -euo pipefail
        exec "${NORI_CLAUDE_TEAMS_NORI_BIN:-nori}" __tmux-compat "$@"
        """
    ...
    private func createClaudeNodeOptionsRestoreModule() throws -> URL {
        let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("nori-claude-node-options", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true, attributes: nil)
    ...
    private func runClaudeTeams(
        ...
        launcherEnvironment["NORI_SOCKET_PATH"] = socketPath
        launcherEnvironment["NORI_SOCKET"] = socketPath
        if let explicitPassword,
           !explicitPassword.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            launcherEnvironment["NORI_SOCKET_PASSWORD"] = explicitPassword
        }
        let shimDirectory = try createClaudeTeamsShimDirectory()
        let executablePath = resolvedExecutableURL()?.path ?? (args.first ?? "nori")
        ...
        let claudeExecutablePath: String? = {
            ...
            let candidates = [
                launcherEnvironment["NORI_CUSTOM_CLAUDE_PATH"],
            ...
            for raw in candidates {
                ...
                guard FileManager.default.fileExists(atPath: trimmed, isDirectory: &isDir),
                      ...
                      !isNoriClaudeWrapper(at: trimmed) else { continue }
                return trimmed
            }
    ...
    // MARK: - nori omo (OpenCode + oh-my-openagent)
    ...
    private func createOMOShimDirectory() throws -> URL {
        // tmux shim: redirects tmux commands to nori __tmux-compat
        ...
        exec "${NORI_OMO_NORI_BIN:-nori}" __tmux-compat "$@"
        """
        ...
        // terminal-notifier shim: intercepts macOS notifications and routes to nori notify
        let notifierURL = root.appendingPathComponent("terminal-notifier", isDirectory: false)
        let notifierScript = """
        #!/usr/bin/env bash
        # Intercept terminal-notifier calls and route through nori notify.
        ...
        exec "${NORI_OMO_NORI_BIN:-nori}" notify --title "${TITLE:-OpenCode}" --body "${BODY:-}"
        """
    ...
    private func configureOMOEnvironment(
        ...
        configureTmuxCompatEnvironment(
            ...
            explicitPassword: explicitPassword,
            focusedContext: focusedContext,
            tmuxPathPrefix: "nori-omo",
            noriBinEnvVar: "NORI_OMO_NORI_BIN",
            termOverrideEnvVar: "NORI_OMO_TERM",
    ...
    private func createOMXShimDirectory() throws -> URL {
        ...
        exec "${NORI_OMX_NORI_BIN:-nori}" __tmux-compat "$@"
        """
        return try createTmuxCompatShimDirectory(
    ...
    private func configureOMXEnvironment(
        ...
        configureTmuxCompatEnvironment(
            ...
            explicitPassword: explicitPassword,
            focusedContext: focusedContext,
            tmuxPathPrefix: "nori-omx",
            noriBinEnvVar: "NORI_OMX_NORI_BIN",
            termOverrideEnvVar: "NORI_OMX_TERM"
        )
    }
    ...
    // MARK: - nori omc (Oh My Claude Code)
    ...
    private func createOMCShimDirectory() throws -> URL {
        ...
        exec "${NORI_OMC_NORI_BIN:-nori}" __tmux-compat "$@"
        """
        return try createTmuxCompatShimDirectory(
    ...
    private func configureOMCEnvironment(
        ...
        configureTmuxCompatEnvironment(
            ...
            explicitPassword: explicitPassword,
            focusedContext: focusedContext,
            tmuxPathPrefix: "nori-omc",
            noriBinEnvVar: "NORI_OMC_NORI_BIN",
            termOverrideEnvVar: "NORI_OMC_TERM"
        )
    ...
    private func runClaudeTeamsTmuxCompat(
        ...
        case "new-session", "new":
            ...
            if parsed.hasFlag("-A") {
                throw CLIError(message: "new-session -A is not supported in nori claude-teams mode")
            }
        ...
        case "new-window", "neww":
            ...
            if parsed.value("-t") != nil {
                throw CLIError(message: "new-window -t is not supported in nori claude-teams mode")
            }
            var params: [String: Any] = ["focus": false]
            if let cwd = parsed.value("-c") {
                params["cwd"] = resolvePath(cwd)
    ...
    private func tmuxCompatStoreURL() -> URL {
        ...
        return URL(fileURLWithPath: homePath)
            .appendingPathComponent(".noriterm")
    ...
    private func tmuxWaitForSignalURL(name: String) -> URL {
        ...
        return URL(fileURLWithPath: "/tmp/nori-wait-for-\(String(sanitized)).sig")
    }
    ...
    private func runTmuxCompatCommand(
        ...
        case "capture-pane":
            ...
            let (sfArg, rem1) = parseOption(rem0, name: "--surface")
            let (linesArg, rem2) = parseOption(rem1, name: "--lines")
            let workspaceArg = wsArg ?? (windowOverride == nil ? ProcessInfo.processInfo.environment["NORI_WORKSPACE_ID"] : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowOverride == nil ? ProcessInfo.processInfo.environment["NORI_SURFACE_ID"] : nil)
        ...
        case "popup":
            throw CLIError(message: "popup is not supported yet in nori CLI parity mode")

        case "bind-key", "unbind-key", "copy-mode":
            throw CLIError(message: "\(command) is not supported yet in nori CLI parity mode")
        ...
        case "display-message":
            ...
            let payload = try client.sendV2(method: "notification.create", params: ["title": "nori", "body": message])
            if jsonOutput {
                print(jsonString(payload))
            } else {
                print(message)
    ...
    private func runClaudeHook(
        ...
        let workspaceArg = hookWsFlag ?? ProcessInfo.processInfo.environment["NORI_WORKSPACE_ID"]
        let surfaceArg = optionValue(hookArgs, name: "--surface") ?? (hookWsFlag == nil ? ProcessInfo.processInfo.environment["NORI_SURFACE_ID"] : nil)
        ...
        case "session-start", "active":
            ...
            let claudePid: Int? = {
                guard let raw = ProcessInfo.processInfo.environment["NORI_CLAUDE_PID"]?
                    .trimmingCharacters(in: .whitespacesAndNewlines),
                    let pid = Int(raw),
                    pid > 0 else {
                    return nil
        ...
        case "help", "--help", "-h":
            ...
            print(
                """
                nori claude-hook <session-start|stop|session-end|notification|prompt-submit|pre-tool-use> [--workspace <id|index>] [--surface <id|index>]
                """
            )
    ...
    private func resolveCallerTTYName() -> String? {
        let env = ProcessInfo.processInfo.environment
        for key in ["NORI_CLI_TTY_NAME", "NORI_TTY_NAME", "TTY", "SSH_TTY"] {
    ...
    /// The hooks.json content that nori installs into ~/.codex/.
    /// Each hook calls `nori codex-hook <event>` which gracefully no-ops
    /// when not running inside nori. The command checks for nori on PATH
    /// first so it silently succeeds even when nori is not installed
    /// (e.g. user opened codex in a non-nori terminal).
    ...
    private struct AgentHookDef {
        ...
        let sessionStoreSuffix: String // e.g. "cursor" -> ~/.nori/cursor-hook-sessions.json
        let disableEnvVar: String   // e.g. "NORI_CURSOR_HOOKS_DISABLED"
        let hookMarker: String      // Marker in commands: "nori cursor-hook"
    ...
    private static let agentDefs: [AgentHookDef] = [
        AgentHookDef(
            ...
            sessionStoreSuffix: "codex", disableEnvVar: "NORI_CODEX_HOOKS_DISABLED",
            hookMarker: "nori codex-hook", format: .nested(timeoutMs: 5000),
            events: [
                .init(agentEvent: "SessionStart", noriSubcommand: "session-start"),
                .init(agentEvent: "UserPromptSubmit", noriSubcommand: "prompt-submit"),
                .init(agentEvent: "Stop", noriSubcommand: "stop"),
            ],
        ...
        AgentHookDef(
            ...
            sessionStoreSuffix: "cursor", disableEnvVar: "NORI_CURSOR_HOOKS_DISABLED",
            hookMarker: "nori cursor-hook", format: .flat,
            events: [
                .init(agentEvent: "beforeSubmitPrompt", noriSubcommand: "prompt-submit"),
                .init(agentEvent: "stop", noriSubcommand: "stop"),
                .init(agentEvent: "afterAgentResponse", noriSubcommand: "agent-response"),
                .init(agentEvent: "beforeShellExecution", noriSubcommand: "shell-exec"),
                .init(agentEvent: "afterShellExecution", noriSubcommand: "shell-done"),
            ]
        ),
        AgentHookDef(
            name: "gemini", displayName: "Gemini", statusKey: "gemini",
            configDir: ".gemini", configFile: "settings.json",
            sessionStoreSuffix: "gemini", disableEnvVar: "NORI_GEMINI_HOOKS_DISABLED",
            hookMarker: "nori gemini-hook", format: .nested(timeoutMs: 10000),
            events: [
                .init(agentEvent: "SessionStart", noriSubcommand: "session-start"),
                .init(agentEvent: "BeforeAgent", noriSubcommand: "prompt-submit"),
                .init(agentEvent: "AfterAgent", noriSubcommand: "stop"),
                .init(agentEvent: "SessionEnd", noriSubcommand: "session-end"),
            ]
        ),
        AgentHookDef(
            name: "copilot", displayName: "Copilot", statusKey: "copilot",
            configDir: ".copilot", configFile: "config.json",
            sessionStoreSuffix: "copilot", disableEnvVar: "NORI_COPILOT_HOOKS_DISABLED",
            hookMarker: "nori copilot-hook", format: .nested(timeoutMs: 5000),
            events: [
                .init(agentEvent: "SessionStart", noriSubcommand: "session-start"),
                .init(agentEvent: "Stop", noriSubcommand: "stop"),
                .init(agentEvent: "Notification", noriSubcommand: "stop"),
                .init(agentEvent: "SessionEnd", noriSubcommand: "session-end"),
            ]
        ),
        AgentHookDef(
            ...
            sessionStoreSuffix: "codebuddy", disableEnvVar: "NORI_CODEBUDDY_HOOKS_DISABLED",
            hookMarker: "nori codebuddy-hook", format: .nested(timeoutMs: 5000),
            events: [
                .init(agentEvent: "SessionStart", noriSubcommand: "session-start"),
                .init(agentEvent: "Stop", noriSubcommand: "stop"),
                .init(agentEvent: "Notification", noriSubcommand: "stop"),
        ...
        AgentHookDef(
            ...
            sessionStoreSuffix: "factory", disableEnvVar: "NORI_FACTORY_HOOKS_DISABLED",
            hookMarker: "nori factory-hook", format: .nested(timeoutMs: 5000),
            events: [
                .init(agentEvent: "SessionStart", noriSubcommand: "session-start"),
                .init(agentEvent: "Stop", noriSubcommand: "stop"),
                .init(agentEvent: "Notification", noriSubcommand: "stop"),
        ...
        AgentHookDef(
            ...
            sessionStoreSuffix: "qoder", disableEnvVar: "NORI_QODER_HOOKS_DISABLED",
            hookMarker: "nori qoder-hook", format: .nested(timeoutMs: 5000),
            events: [
                .init(agentEvent: "SessionStart", noriSubcommand: "session-start"),
                .init(agentEvent: "Stop", noriSubcommand: "stop"),
                .init(agentEvent: "SessionEnd", noriSubcommand: "session-end"),
    ...
    private func hookCommand(for def: AgentHookDef, event: AgentHookDef.HookEvent) -> String {
        "[ -n \"$NORI_SURFACE_ID\" ] && [ \"$\(def.disableEnvVar)\" != \"1\" ] && command -v nori >/dev/null 2>&1 && nori \(def.name)-hook \(event.noriSubcommand) || echo '{}'"
    }
    ...
    private func installAgentHooks(_ def: AgentHookDef) throws {
        ...
        // Remove existing nori-owned entries
    ...
    private func uninstallAgentHooks(_ def: AgentHookDef) throws {
        ...
        try newData.write(to: URL(fileURLWithPath: filePath), options: .atomic)
        print("Removed \(removed) nori hook(s) from \(filePath)")
    ...
    private func runGenericAgentHook(def: AgentHookDef, commandArgs: [String], client: SocketClient, telemetry: CLISocketSentryTelemetry) throws {
        ...
        let hookWsFlag = optionValue(hookArgs, name: "--workspace")
        let workspaceArg = hookWsFlag ?? env["NORI_WORKSPACE_ID"]
        let surfaceArg = optionValue(hookArgs, name: "--surface") ?? (hookWsFlag == nil ? env["NORI_SURFACE_ID"] : nil)
        ...
        let store = ClaudeHookSessionStore(
            processEnv: env.merging(
                ["NORI_CLAUDE_HOOK_STATE_PATH": "~/.nori/\(def.sessionStoreSuffix)-hook-sessions.json"],
                uniquingKeysWith: { _, new in new }
            )
        )

        let sessionId = input.sessionId ?? env["NORI_SURFACE_ID"] ?? ""
    ...
    private func runSetupHooks(uninstall: Bool = false) throws {
        ...
        print("nori \(isUninstall ? "uninstall" : "setup")-hooks: \(verb) agent hooks")
        if !isUninstall {
    ...
    private func versionSummary() -> String {
        let info = resolvedVersionInfo()
        let commit = info["NORICommit"].flatMap { normalizedCommitHash($0) }
        let baseSummary: String
        if let version = info["CFBundleShortVersionString"], let build = info["CFBundleVersion"] {
            baseSummary = "nori \(version) (\(build))"
        } else if let version = info["CFBundleShortVersionString"] {
            baseSummary = "nori \(version)"
        } else if let build = info["CFBundleVersion"] {
            baseSummary = "nori build \(build)"
        } else {
            baseSummary = "nori version unknown"
        }
    ...
    private func printWelcome() {
        ...
        print(shortcuts)
        print()
        print("  \(bold)Docs\(reset)\(subdued)                https://nori.com/docs\(reset)")
        print("  \(bold)Discord\(reset)\(subdued)             https://discord.gg/xsgFEVrWCZ\(reset)")
        print("  \(bold)GitHub\(reset)\(subdued)              https://github.com/manaflow-ai/nori (please leave a star ⭐)\(reset)")
        print("  \(bold)Email\(reset)\(subdued)               founders@manaflow.com\(reset)")
        print()
        print("  \(subdued)Run \(reset)\(bold)nori --help\(reset)\(subdued) for all commands.\(reset)")
        print("  \(subdued)Run \(reset)\(bold)nori shortcuts\(reset)\(subdued) to edit shortcuts.\(reset)")
        print("  \(subdued)Run \(reset)\(bold)nori feedback\(reset)\(subdued) to report a bug.\(reset)")
        print()
    }
    ...
    private func resolvedVersionInfo() -> [String: String] {
        ...
        if info["NORICommit"] == nil,
           let commit = normalizedCommitHash(ProcessInfo.processInfo.environment["NORI_COMMIT"]) {
            info["NORICommit"] = commit
        }
    ...
    private func versionInfo(from dictionary: [String: Any]?) -> [String: String]? {
        ...
        if let commit = dictionary["NORICommit"] as? String,
           let normalizedCommit = normalizedCommitHash(commit) {
            info["NORICommit"] = normalizedCommit
        }
    ...
    private func usage() -> String {
        return """
        nori - control nori via Unix socket

        Usage:
          nori <path>                Open a directory in a new workspace (launches nori if needed)
          nori [global-options] <command> [options]
        ...
        Socket Auth:
          --password takes precedence, then NORI_SOCKET_PASSWORD env var, then password saved in Settings.
        ...
        Environment:
          NORI_WORKSPACE_ID   Auto-set in nori terminals. Used as default --workspace for
                              ALL commands (send, list-panels, new-split, notify, etc.).
          NORI_TAB_ID         Optional alias used by `tab-action`/`rename-tab` as default --tab.
          NORI_SURFACE_ID     Auto-set in nori terminals. Used as default --surface.
          NORI_SOCKET_PATH    Override the Unix socket path. Without this, the CLI defaults
                              to ~/Library/Application Support/nori/nori.sock and auto-discovers tagged/debug sockets.
        """
    }
...
struct NORITermMain {
    static func main() {
        ...
        let cli = NORICLI(args: CommandLine.arguments)
        do {
            try cli.run()
        } catch {
