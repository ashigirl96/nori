...
final class WorkspaceRemoteConnectionTests: XCTestCase {
    ...
    private func runRelayZshHistfile(
        ...
        let home = fileManager.temporaryDirectory.appendingPathComponent("nori-relay-zsh-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".nori/relay/64011.shell")
    ...
    func testRemoteRelayMetadataCleanupScriptRemovesMatchingSocketAddr() {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("nori-relay-cleanup-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".nori/relay")
        let socketAddrURL = home.appendingPathComponent(".nori/socket_addr")
    ...
    func testRemoteRelayMetadataCleanupScriptPreservesDifferentSocketAddr() {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("nori-relay-cleanup-preserve-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".nori/relay")
        let socketAddrURL = home.appendingPathComponent(".nori/socket_addr")
    ...
    func testRemoteTerminalSurfaceLookupTracksOnlyActiveSSHSurfaces() throws {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/nori-debug-test.sock",
    ...
    func testTeardownRemoteConnectionRequestsControlMasterCleanupWhileStillConnecting() {
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connecting,
            detail: "Connecting to nori-macmini",
            target: "nori-macmini"
        )
    ...
    func testTransferredRemoteSurfaceCleansUpControlMasterWhenSessionEndsInLocalWorkspace() throws {
        ...
        XCTAssertEqual(cleanupArguments.first?.suffix(2), ["exit", "nori-macmini"])
    }
    ...
    func testRemoteDropPathUsesLowercasedExtensionAndProvidedUUID() throws {
        ...
        XCTAssertEqual(remotePath, "/tmp/nori-drop-12345678-1234-1234-1234-1234567890ab.png")
    }
    ...
    func testDetectedSSHUploadFailureCleansUpEarlierRemoteUploads() throws {
        ...
        let directoryURL = fileManager.temporaryDirectory.appendingPathComponent(
            "nori-detected-ssh-upload-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testDetectsForegroundSSHSessionWithShortControlPathFlag() {
        ...
        XCTAssertEqual(session?.controlPath, "/tmp/nori-ssh-%C")
        let scpArgs = session?.scpArgumentsForTesting(
            localPath: "/tmp/local.png",
            remotePath: "/tmp/nori-drop-123.png"
        ) ?? []
        XCTAssertTrue(scpArgs.contains("ControlPath=/tmp/nori-ssh-%C"))
    ...
    func testDaemonTransportArgumentsReuseConfiguredControlPath() {
        ...
        let arguments = WorkspaceRemoteSSHBatchCommandBuilder.daemonTransportArguments(
            ...
            remotePath: "/remote/norid-remote"
        )

        XCTAssertFalse(arguments.contains("-S"))
        XCTAssertTrue(arguments.contains("ControlMaster=no"))
        XCTAssertTrue(arguments.contains(where: { $0 == "ControlPath /tmp/nori-ssh-%C" || $0 == "ControlPath=/tmp/nori-ssh-%C" }))
        XCTAssertTrue(arguments.contains("nori-macmini"))
        XCTAssertTrue(arguments.last?.contains("/remote/norid-remote") ?? false)
    }
    ...
    func testReverseRelayControlMasterArgumentsReuseConfiguredControlSocket() throws {
        ...
        XCTAssertFalse(arguments.contains("-S"))
        XCTAssertTrue(arguments.contains("ControlMaster=no"))
        XCTAssertTrue(arguments.contains("ControlPath=/tmp/nori-ssh-%C"))
    ...
    func testDetectedSSHSessionBracketsIPv6LiteralSCPDestination() {
        ...
        XCTAssertEqual(scpArgs.last, "lawrence@[2001:db8::1]:/tmp/nori-drop-123.png")
    }
    ...
    func testProxyOnlyErrorsKeepSSHWorkspaceConnectedAndLoggedInSidebar() {
        ...
        let proxyError = "Remote proxy to nori-macmini unavailable: Failed to start local daemon proxy: daemon RPC timeout waiting for hello response (retry in 3s)"
        workspace.applyRemoteConnectionStateUpdate(.error, detail: proxyError, target: "nori-macmini")
        ...
        XCTAssertEqual(
            ...
            "Remote proxy unavailable (nori-macmini): \(proxyError)"
        )
        ...
        workspace.applyRemoteConnectionStateUpdate(.connecting, detail: "Connecting to nori-macmini", target: "nori-macmini")
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connected,
            detail: "Connected to nori-macmini via shared local proxy 127.0.0.1:9999",
            target: "nori-macmini"
        )
...
final class CLINotifyProcessIntegrationTests: XCTestCase {
    ...
    private func bundledCLIPath() throws -> String {
        ...
        while let item = enumerator?.nextObject() as? URL {
            guard item.lastPathComponent == "nori",
                  item.path.contains(".app/Contents/Resources/bin/nori") else {
                continue
            }
            return item.path
        }

        throw XCTSkip("Bundled nori CLI not found in \(appBundleURL.path)")
    }
    ...
    func testNotifyFallsBackFromStaleCallerWorkspaceAndSurfaceIDs() throws {
        ...
        environment["NORI_SOCKET_PATH"] = socketPath
        environment["NORI_WORKSPACE_ID"] = staleWorkspace
        environment["NORI_SURFACE_ID"] = staleSurface
        environment["NORI_CLI_SENTRY_DISABLED"] = "1"
        environment["NORI_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    func testSSHCommandCreatesConfiguresAndSelectsRemoteWorkspaceViaCLI() throws {
        ...
        let result = runProcess(
            ...
            arguments: [
                ...
                "--ssh-option", "ControlPath /tmp/nori-ssh-%C",
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertEqual(result.stdout, "OK workspace=\(workspaceRef) target=nori-macmini state=disconnected\n")
        XCTAssertTrue(result.stderr.isEmpty, result.stderr)
        ...
        XCTAssertEqual(configureParams["destination"] as? String, "nori-macmini")
        ...
        XCTAssertTrue(sshOptions.contains("ControlPath /tmp/nori-ssh-%C"))
        XCTAssertTrue(sshOptions.contains("StrictHostKeyChecking=accept-new"))

        // `nori ssh` should land the user in the new SSH workspace immediately.
    ...
    func testSSHCommandDoesNotDeferReconnectWhenWhitespaceControlMasterDisablesMultiplexing() throws {
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertEqual(result.stdout, "OK workspace=\(workspaceRef) target=nori-macmini state=connecting\n")
        XCTAssertTrue(result.stderr.isEmpty, result.stderr)
    ...
    func testSSHBootstrapStartupCommandPassesRemoteInstallScriptAsSingleSSHCommand() throws {
        ...
        let fileManager = FileManager.default
        let tempRoot = fileManager.temporaryDirectory.appendingPathComponent("nori-ssh-bootstrap-\(UUID().uuidString)")
        ...
        args = sys.argv[1:]
        with open(os.environ["NORI_FAKE_SSH_LOG"], "a", encoding="utf-8") as handle:
        ...
        startupEnvironment["NORI_FAKE_SSH_LOG"] = fakeSSHLog.path
        startupEnvironment["NORI_SOCKET_PATH"] = socketPath
        startupEnvironment["NORI_WORKSPACE_ID"] = workspaceID
        startupEnvironment["NORI_CLI_SENTRY_DISABLED"] = "1"
        startupEnvironment["NORI_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
        ...
        let destinationIndex = try XCTUnwrap(firstInvocation.lastIndex(of: "nori-macmini"))
    ...
    func testNotifyPrefersCallerTTYOverFocusedSurfaceWhenCallerIDsAreStale() throws {
        ...
        environment["NORI_WORKSPACE_ID"] = staleWorkspace
        environment["NORI_SURFACE_ID"] = staleSurface
        environment["NORI_CLI_TTY_NAME"] = callerTTY
        environment["NORI_CLI_SENTRY_DISABLED"] = "1"
        environment["NORI_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    func testNotifyInTmuxPrefersCallerTTYOverStaleValidSurfaceID() throws {
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["NORI_SOCKET_PATH"] = socketPath
        environment["NORI_WORKSPACE_ID"] = workspaceId
        environment["NORI_SURFACE_ID"] = staleSurface
        environment["NORI_CLI_TTY_NAME"] = callerTTY
