...
final class WorkspaceStressProfileTests: XCTestCase {
    private struct StressConfig {
        ...
        static func current(environment: [String: String] = ProcessInfo.processInfo.environment) -> StressConfig {
            StressConfig(
                workspaceCount: parseInt(environment["NORI_WORKSPACE_STRESS_WORKSPACES"], default: 48, minimum: 2),
                tabsPerWorkspace: parseInt(environment["NORI_WORKSPACE_STRESS_TABS_PER_WORKSPACE"], default: 10, minimum: 1),
                switchPasses: parseInt(environment["NORI_WORKSPACE_STRESS_SWITCH_PASSES"], default: 6, minimum: 1),
                createP95BudgetMs: parseDouble(environment["NORI_WORKSPACE_STRESS_CREATE_P95_BUDGET_MS"]),
                switchP95BudgetMs: parseDouble(environment["NORI_WORKSPACE_STRESS_SWITCH_P95_BUDGET_MS"])
            )
        }
