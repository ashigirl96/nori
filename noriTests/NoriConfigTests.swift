...
final class NoriConfigDecodingTests: XCTestCase {

    private func decode(_ json: String) throws -> NoriConfigFile {
...
final class NoriCommandIdentityTests: XCTestCase {

    func testCommandIdIsDeterministic() {
        let cmd = NoriCommandDefinition(name: "Run tests", command: "test")
        XCTAssertEqual(cmd.id, "nori.config.command.Run%20tests")
    }

    func testCommandIdEncodesSpecialCharacters() {
        let cmd = NoriCommandDefinition(name: "build & deploy", command: "make")
        XCTAssertTrue(cmd.id.hasPrefix("nori.config.command."))
        XCTAssertFalse(cmd.id.contains("&"))
        XCTAssertFalse(cmd.id.contains(" "))
    }

    func testCommandIdIsUniqueForDifferentNames() {
        let cmd1 = NoriCommandDefinition(name: "build", command: "make build")
        let cmd2 = NoriCommandDefinition(name: "test", command: "make test")
        XCTAssertNotEqual(cmd1.id, cmd2.id)
    }

    func testCommandIdDoesNotCollideWithBuiltinPrefix() {
        let cmd = NoriCommandDefinition(name: "palette.newWorkspace", command: "echo")
        XCTAssertTrue(cmd.id.hasPrefix("nori.config.command."))
...
final class NoriSplitDefinitionTests: XCTestCase {
    ...
    func testSplitOrientationVertical() {
        let split = NoriSplitDefinition(direction: .vertical, split: nil, children: [])
...
@MainActor
final class NoriConfigCwdResolutionTests: XCTestCase {

    private let baseCwd = "/Users/test/project"

    func testNilCwdReturnsBase() {
        XCTAssertEqual(
            NoriConfigStore.resolveCwd(nil, relativeTo: baseCwd),
            baseCwd
        )
    }
    ...
    func testEmptyCwdReturnsBase() {
        XCTAssertEqual(
            NoriConfigStore.resolveCwd("", relativeTo: baseCwd),
            baseCwd
        )
    }
    ...
    func testAbsolutePathReturnedAsIs() {
        XCTAssertEqual(
            NoriConfigStore.resolveCwd("/tmp/other", relativeTo: baseCwd),
            "/tmp/other"
        )
    }
    ...
    func testRelativePathJoinedToBase() {
        XCTAssertEqual(
            NoriConfigStore.resolveCwd("backend/src", relativeTo: baseCwd),
    ...
    func testTildeSlashExpandsToHomePlusPath() {
        let home = FileManager.default.homeDirectoryForCurrentUser.path
        XCTAssertEqual(
            NoriConfigStore.resolveCwd("~/Documents/work", relativeTo: baseCwd),
    ...
    func testSingleSubdirectory() {
        XCTAssertEqual(
            NoriConfigStore.resolveCwd("src", relativeTo: baseCwd),
...
final class NoriLayoutEncodingTests: XCTestCase {

    func testPaneNodeRoundTrips() throws {
        let original = NoriLayoutNode.pane(NoriPaneDefinition(surfaces: [
            NoriSurfaceDefinition(type: .terminal, name: "shell")
        ]))
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(NoriLayoutNode.self, from: data)
    ...
    func testSplitNodeRoundTrips() throws {
        let original = NoriLayoutNode.split(NoriSplitDefinition(
            direction: .vertical,
            split: 0.7,
            children: [
                .pane(NoriPaneDefinition(surfaces: [NoriSurfaceDefinition(type: .terminal)])),
                .pane(NoriPaneDefinition(surfaces: [NoriSurfaceDefinition(type: .browser, url: "http://localhost")]))
            ]
        ))
        let data = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(NoriLayoutNode.self, from: data)
