...
final class CommandPaletteSwitcherSearchIndexerTests: XCTestCase {
    func testKeywordsIncludeDirectoryBranchAndPortMetadata() {
        let metadata = CommandPaletteSwitcherSearchMetadata(
            directories: ["/Users/example/dev/noriterm-hq/worktrees/feat-cmd-palette"],
        ...
        XCTAssertTrue(keywords.contains("/Users/example/dev/noriterm-hq/worktrees/feat-cmd-palette"))
    ...
    func testFuzzyMatcherMatchesDirectoryBranchAndPortMetadata() {
        let metadata = CommandPaletteSwitcherSearchMetadata(
            directories: ["/tmp/noriterm/worktrees/issue-123-switcher-search"],
    ...
    func testSurfaceDetailOutranksWorkspaceDetailForPathToken() throws {
        let metadata = CommandPaletteSwitcherSearchMetadata(
            directories: ["/tmp/worktrees/nori"],
        ...
        let workspaceScore = try XCTUnwrap(
            CommandPaletteFuzzyMatcher.score(query: "nori", candidates: workspaceKeywords)
        )
        let surfaceScore = try XCTUnwrap(
            CommandPaletteFuzzyMatcher.score(query: "nori", candidates: surfaceKeywords)
        )
