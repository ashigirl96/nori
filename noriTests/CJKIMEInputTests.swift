...
private extension GhosttyNSView {
    @objc func noriUnitTest_interpretKeyEvents(_ eventArray: [NSEvent]) {
        if let hook = cjkIMEInterpretKeyEventsHook, hook(self, eventArray) {
            return
        }
        noriUnitTest_interpretKeyEvents(eventArray)
    }
}
...
private func installCJKIMEInterpretKeyEventsSwizzle() {
    ...
    let swizzledSelector = #selector(GhosttyNSView.noriUnitTest_interpretKeyEvents(_:))
...
// MARK: - Space release regression (Codex hold-to-talk in nori)
