...
final class FinderServicePathResolverTests: XCTestCase {
    private func withTemporaryDirectory<T>(_ body: (URL) throws -> T) throws -> T {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(
            "nori-finder-service-tests-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testOrderedUniqueDirectoriesUsesParentForFilesAndDedupes() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/nori-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/nori-services/project/README.md", isDirectory: false),
            URL(fileURLWithPath: "/tmp/nori-services/../nori-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/nori-services/other", isDirectory: true),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/tmp/nori-services/project",
                "/tmp/nori-services/other",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesPreservesFirstSeenOrder() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/nori-services/b", isDirectory: true),
            URL(fileURLWithPath: "/tmp/nori-services/a/file.txt", isDirectory: false),
            URL(fileURLWithPath: "/tmp/nori-services/a", isDirectory: true),
            URL(fileURLWithPath: "/tmp/nori-services/b/file.txt", isDirectory: false),
        ]
    ...
    func testOrderedUniqueDirectoriesSkipsBundleAndEmbeddedPathsWhenExcludingBundleRoot() {
        let bundleURL = URL(fileURLWithPath: "/Applications/Tools/../nori.app", isDirectory: true)
        let input: [URL] = [
            bundleURL,
            URL(fileURLWithPath: "/Applications/nori.app/Contents/MacOS/nori", isDirectory: false),
            URL(fileURLWithPath: "/Applications/nori.app/Contents/Resources/bin/nori", isDirectory: false),
            URL(fileURLWithPath: "/Users/tester/Projects/nori", isDirectory: true),
            URL(fileURLWithPath: "/Users/tester/Projects/nori/README.md", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/Users/tester/Projects/nori",
            ]
        )
    }

    func testOrderedUniqueDirectoriesExclusionDoesNotFilterSiblingPaths() {
        let bundleURL = URL(fileURLWithPath: "/Applications/nori.app", isDirectory: true)
        let input: [URL] = [
            URL(fileURLWithPath: "/Applications/nori.app backup/project", isDirectory: true),
            URL(fileURLWithPath: "/Applications/nori.app.beta/project/file.txt", isDirectory: false),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/Applications/nori.app backup/project",
                "/Applications/nori.app.beta/project",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesResolvesSymlinksOnlyForExcludedRootComparison() throws {
        try withTemporaryDirectory { root in
            ...
            let actualBundle = applicationsDirectory.appendingPathComponent("nori.app", isDirectory: true)
            let actualBinary = actualBundle.appendingPathComponent("Contents/MacOS/nori", isDirectory: false)
            ...
            let directories = FinderServicePathResolver.orderedUniqueDirectories(
                from: [
                    aliasApplications.appendingPathComponent("nori.app", isDirectory: true),
                    aliasApplications.appendingPathComponent("nori.app/Contents/MacOS/nori", isDirectory: false),
                    aliasWorkspace,
                ],
...
final class VSCodeServeWebURLBuilderTests: XCTestCase {
    ...
    func testOpenFolderURLAppendsFolderQueryWhilePreservingToken() {
        ...
        let url = VSCodeServeWebURLBuilder.openFolderURL(
            ...
            directoryPath: "/Users/tester/Projects/nori"
        )

        let components = URLComponents(url: url!, resolvingAgainstBaseURL: false)
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "tkn" })?.value, "test-token")
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "folder" })?.value, "/Users/tester/Projects/nori")
    }
