...
final class GhosttyConfigTests: XCTestCase {
    ...
    func testThemeSearchPathsIncludeXDGDataDirsThemes() {
        let pathA = "/tmp/nori-theme-a"
        let pathB = "/tmp/nori-theme-b"
    ...
    func testLoadReadsSymlinkedGhosttyConfigFile() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("nori-ghostty-config-symlink-\(UUID().uuidString)")
    ...
    func testLoadThemeResolvesPairedThemeValueByColorScheme() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-ghostty-theme-pair-\(UUID().uuidString)")
    ...
    func testLoadThemeResolvesBuiltinAliasFromGhosttyResourcesDir() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-ghostty-themes-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLoadThemeResolvesITerm2SolarizedLightAliasToLegacyThemeName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-ghostty-solarized-light-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLoadThemeResolvesITerm2SolarizedDarkAliasToLegacyThemeName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-ghostty-solarized-dark-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testNoriAppSupportConfigURLsUseReleaseConfigForDebugBundleWithoutCurrentConfig() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            let releaseConfigURL = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.nori.app",
                filename: "config",
                contents: "font-size = 13\n"
            )

            XCTAssertEqual(
                GhosttyApp.noriAppSupportConfigURLs(
                    currentBundleIdentifier: "com.nori.app.debug",
                    appSupportDirectory: appSupportDirectory
                ),
                [releaseConfigURL]
            )
    ...
    func testNoriAppSupportConfigURLsPreferCurrentBundleConfigWhenPresent() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            ...
            let currentConfigURL = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.nori.app.debug.issue-829",
            ...
            XCTAssertEqual(
                GhosttyApp.noriAppSupportConfigURLs(
                    currentBundleIdentifier: "com.nori.app.debug.issue-829",
                    appSupportDirectory: appSupportDirectory
                ),
                [currentConfigURL]
            )
    ...
    func testNoriAppSupportConfigURLsSkipReleaseFallbackForNonDebugBundle() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            _ = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.nori.app",
    ...
    func testNoriAppSupportConfigURLsIgnoreMissingOrEmptyFiles() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            _ = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.nori.app",
    ...
    func testClaudeCodeIntegrationDefaultsToEnabledWhenUnset() {
        let suiteName = "nori.tests.claude-hooks.\(UUID().uuidString)"
    ...
    func testTelemetryDefaultsToEnabledWhenUnset() {
        let suiteName = "nori.tests.telemetry.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated user defaults suite")
            return
        }
    ...
    private func withTemporaryAppSupportDirectory(
        ...
        let directory = fileManager.temporaryDirectory
            .appendingPathComponent("nori-app-support-\(UUID().uuidString)", isDirectory: true)
...
final class WindowTransparencyDecisionTests: XCTestCase {
    ...
    func testTranslucentOpacityForcesClearWindowBackgroundOutsideSidebarBlendModePath() {
        withTemporaryWindowBackgroundDefaults {
            ...
            XCTAssertFalse(noriShouldUseTransparentBackgroundWindow())
            XCTAssertTrue(noriShouldUseClearWindowBackground(for: 0.80))
            XCTAssertFalse(noriShouldUseClearWindowBackground(for: 1.0))
        }
    }

    func testGlassEnabledDecisionIgnoresGlassImplementationAvailability() {
        XCTAssertTrue(
            noriShouldApplyWindowGlass(
    ...
    func testBehindWindowGlassPathKeepsTransparentWindowEnabled() {
        withTemporaryWindowBackgroundDefaults {
            ...
            XCTAssertTrue(noriShouldUseTransparentBackgroundWindow())
            XCTAssertTrue(noriShouldUseClearWindowBackground(for: 1.0))
        }
    }
...
final class WorkspaceRemoteDaemonManifestTests: XCTestCase {
    func testParsesEmbeddedRemoteDaemonManifestJSON() throws {
        let manifestJSON = """
        {
          ...
          "appVersion": "0.62.0",
          "releaseTag": "v0.62.0",
          "releaseURL": "https://github.com/manaflow-ai/nori/releases/tag/v0.62.0",
          "checksumsAssetName": "norid-remote-checksums.txt",
          "checksumsURL": "https://github.com/manaflow-ai/nori/releases/download/v0.62.0/norid-remote-checksums.txt",
          "entries": [
            {
              "goOS": "linux",
              "goArch": "amd64",
              "assetName": "norid-remote-linux-amd64",
              "downloadURL": "https://github.com/manaflow-ai/nori/releases/download/v0.62.0/norid-remote-linux-amd64",
              "sha256": "abc123"
            }
          ]
        }
        ...
        XCTAssertEqual(manifest?.entry(goOS: "linux", goArch: "amd64")?.assetName, "norid-remote-linux-amd64")
    }
    ...
    func testRemoteDaemonCachePathIsVersionedByPlatform() throws {
        ...
        XCTAssertTrue(url.path.contains("/Application Support/nori/remote-daemons/0.62.0/linux-arm64/"))
        XCTAssertEqual(url.lastPathComponent, "norid-remote")
    }
}
...
final class RemoteLoopbackHTTPRequestRewriterTests: XCTestCase {
    func testRewritesLoopbackAliasHostHeadersToLocalhost() {
        let original = Data(
            (
                "GET /demo HTTP/1.1\r\n" +
                "Host: nori-loopback.localtest.me:3000\r\n" +
                "Origin: http://nori-loopback.localtest.me:3000\r\n" +
                "Referer: http://nori-loopback.localtest.me:3000/app\r\n" +
                "\r\n"
            ).utf8
        )
        ...
        let rewritten = RemoteLoopbackHTTPRequestRewriter.rewriteIfNeeded(
            data: original,
            aliasHost: "nori-loopback.localtest.me"
        )
        ...
        XCTAssertFalse(text.contains("nori-loopback.localtest.me"))
    }

    func testRewritesAbsoluteFormRequestLineForLoopbackAlias() {
        let original = Data(
            (
                "GET http://nori-loopback.localtest.me:3000/demo HTTP/1.1\r\n" +
                "Host: nori-loopback.localtest.me:3000\r\n" +
                "\r\n"
            ).utf8
        )
    ...
    func testRewritesLoopbackResponseHeadersBackToAlias() {
        ...
        let text = String(decoding: rewritten, as: UTF8.self)
        XCTAssertTrue(text.contains("Location: http://nori-loopback.localtest.me:3000/login"))
        XCTAssertTrue(text.contains("Access-Control-Allow-Origin: http://nori-loopback.localtest.me:3000"))
        XCTAssertTrue(text.contains("Set-Cookie: sid=1; Domain=nori-loopback.localtest.me; Path=/"))
    }
}
...
final class GhosttyTerminalStartupEnvironmentTests: XCTestCase {
    ...
    func testMergedStartupEnvironmentAllowsSessionReplayAndInitialEnvNORIKeys() {
        let replayPath = "/tmp/nori-replay-\(UUID().uuidString)"
        let merged = TerminalSurface.mergedStartupEnvironment(
            base: [
                "PATH": "/usr/bin",
                "NORI_SURFACE_ID": "managed-surface"
            ],
            protectedKeys: ["PATH", "NORI_SURFACE_ID"],
            additionalEnvironment: [
                SessionScrollbackReplayStore.environmentKey: replayPath
            ],
            initialEnvironmentOverrides: [
                "NORI_INITIAL_ENV_TOKEN": "token-123"
            ]
        )

        XCTAssertEqual(merged[SessionScrollbackReplayStore.environmentKey], replayPath)
        XCTAssertEqual(merged["NORI_INITIAL_ENV_TOKEN"], "token-123")
    }
    ...
    func testMergedStartupEnvironmentProtectsManagedKeysOnly() {
        let merged = TerminalSurface.mergedStartupEnvironment(
            ...
            protectedKeys: ["PATH", "NORI_SURFACE_ID"],
            additionalEnvironment: [
                "NORI_SURFACE_ID": "user-surface",
                "CUSTOM_FLAG": "1"
            ],
            initialEnvironmentOverrides: [
                "PATH": "/tmp/bin",
                "NORI_SURFACE_ID": "override-surface"
            ]
        )

        XCTAssertEqual(merged["PATH"], "/usr/bin")
        XCTAssertEqual(merged["NORI_SURFACE_ID"], "managed-surface")
...
final class BrowserPanelRemoteStoreTests: XCTestCase {
    ...
    func testRemoteWorkspaceDefersInitialNavigationUntilProxyEndpointIsReady() {
        ...
        XCTAssertEqual(panel.preferredURLStringForOmnibar(), url.absoluteString)
        XCTAssertEqual(panel.webView.url?.host, "nori-loopback.localtest.me")
    }
    ...
    func testBrowserMoveIntoRemoteWorkspaceRebuildsWebsiteDataStoreScope() throws {
        ...
        destination.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/nori-store-dest.sock",
    ...
    func testBrowserMoveOutOfRemoteWorkspaceRestoresDefaultWebsiteDataStore() throws {
        ...
        source.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/nori-store-source.sock",
                terminalStartupCommand: "ssh nori-macmini"
            ),
            autoConnect: false
        )
    ...
    func testProxyBrokerTransportKeyIgnoresControlPath() {
        let first = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "Compression=yes",
                "ControlMaster=auto",
                "ControlPath=/tmp/nori-ssh-501-64000-%C",
            ],
            localProxyPort: 9000,
            relayPort: 64000,
            relayID: "relay-a",
            relayToken: "token-a",
            localSocketPath: "/tmp/nori-a.sock",
            terminalStartupCommand: "ssh nori-macmini"
        )
        let second = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "Compression=yes",
                "ControlMaster=auto",
                "ControlPath=/tmp/nori-ssh-501-64001-%C",
            ],
            localProxyPort: 9000,
            relayPort: 64001,
            relayID: "relay-b",
            relayToken: "token-b",
            localSocketPath: "/tmp/nori-b.sock",
            terminalStartupCommand: "ssh nori-macmini"
        )
...
final class WorkspaceRemoteSSHCleanupTests: XCTestCase {
    func testOrphanedNORIRemoteSSHPIDsMatchesOnlyParentOneRelayAndDaemonTransports() {
        let psOutput = """
          101 1 /usr/bin/ssh -N -T -S none -o ControlPath=/tmp/nori-ssh-501-56080-%C -R 127.0.0.1:56080:127.0.0.1:64048 nori-macmini
          102 1 /usr/bin/ssh -T -S none -o RequestTTY=no nori-macmini sh -c 'exec .nori/bin/norid-remote/0.63.1/darwin-arm64/norid-remote serve --stdio'
          103 999 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56081:127.0.0.1:64049 nori-macmini
          104 1 /usr/bin/ssh -tt nori-macmini
          105 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56082:127.0.0.1:64050 other-host
          106 1 /usr/bin/ssh -T -S none nori-macmini /bin/sh
        """

        XCTAssertEqual(
            WorkspaceRemoteSessionController.orphanedNORIRemoteSSHPIDs(
                psOutput: psOutput,
                destination: "nori-macmini"
            ),
            [101, 102]
    ...
    func testOrphanedNORIRemoteSSHPIDsCanRestrictCleanupToSpecificRelayPort() {
        let psOutput = """
          201 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56080:127.0.0.1:64048 nori-macmini
          202 1 /usr/bin/ssh -N -T -S none -R 127.0.0.1:56081:127.0.0.1:64049 nori-macmini
          203 1 /usr/bin/ssh -T -S none -o RequestTTY=no nori-macmini sh -c 'exec .nori/bin/norid-remote/0.63.1/darwin-arm64/norid-remote serve --stdio'
        """
...
final class SocketControlSettingsTests: XCTestCase {
    func testMigrateModeSupportsExpandedSocketModes() {
        ...
        XCTAssertEqual(SocketControlSettings.migrateMode("noriOnly"), .noriOnly)
    ...
    func testSocketModePermissions() {
        ...
        XCTAssertEqual(SocketControlMode.noriOnly.socketFilePermissions, 0o600)
    ...
    func testInvalidEnvSocketModeDoesNotOverrideUserMode() {
        XCTAssertNil(
            SocketControlSettings.envOverrideMode(
                environment: ["NORI_SOCKET_MODE": "definitely-not-a-mode"]
            )
        )
        XCTAssertEqual(
    ...
    func testStableReleaseIgnoresAmbientSocketOverrideByDefault() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "NORI_SOCKET_PATH": "/tmp/nori-debug-issue-153-tmux-compat.sock",
            ],
            bundleIdentifier: "com.nori.app",
            isDebugBuild: false,
    ...
    func testNightlyReleaseUsesDedicatedDefaultAndIgnoresAmbientSocketOverride() {
        let path = SocketControlSettings.socketPath(
            ...
            bundleIdentifier: "com.nori.app.nightly",
            isDebugBuild: false,
            probeStableDefaultPathEntry: { _ in .missing }
        )

        XCTAssertEqual(path, "/tmp/nori-nightly.sock")
    }

    func testDebugBundleHonorsSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "NORI_SOCKET_PATH": "/tmp/nori-debug-my-tag.sock",
            ],
            bundleIdentifier: "com.nori.app.debug.my-tag",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/nori-debug-my-tag.sock")
    }

    func testStagingBundleHonorsSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "NORI_SOCKET_PATH": "/tmp/nori-staging-my-tag.sock",
            ],
            bundleIdentifier: "com.nori.app.staging.my-tag",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/nori-staging-my-tag.sock")
    }

    func testStableReleaseCanOptInToSocketOverride() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "NORI_SOCKET_PATH": "/tmp/nori-debug-forced.sock",
                "NORI_ALLOW_SOCKET_OVERRIDE": "1",
            ],
            bundleIdentifier: "com.nori.app",
            isDebugBuild: false,
            probeStableDefaultPathEntry: { _ in .missing }
        ...
        XCTAssertEqual(path, "/tmp/nori-debug-forced.sock")
    }
    ...
    func testDefaultSocketPathByChannel() {
        ...
        XCTAssertEqual(
            ...
            "/tmp/nori-nightly.sock"
        )
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "com.nori.app.debug.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/nori-debug.sock"
        ...
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "com.nori.app.staging.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/nori-staging.sock"
        )
    }
...
final class UITestLaunchManifestTests: XCTestCase {
    func testManifestPathReadsArgumentValue() {
        XCTAssertEqual(
            UITestLaunchManifest.manifestPath(
                from: ["nori", "-noriUITestLaunchManifest", "/tmp/nori-ui-test-launch.json"]
            ),
            "/tmp/nori-ui-test-launch.json"
        )
    }
    ...
    func testManifestPathReturnsNilWithoutValue() {
        XCTAssertNil(
            UITestLaunchManifest.manifestPath(
                from: ["nori", "-noriUITestLaunchManifest"]
            )
        )
    }
    ...
    func testApplyIfPresentDecodesEnvironmentPayload() {
        let payload = """
        {"environment":{"NORI_TAG":"ui-tests-display","NORI_SOCKET_PATH":"/tmp/nori-ui-tests.sock"}}
        """.data(using: .utf8)!
        var applied: [String: String] = [:]

        UITestLaunchManifest.applyIfPresent(
            arguments: ["nori", UITestLaunchManifest.argumentName, "/tmp/nori-ui-test-launch.json"],
        ...
        XCTAssertEqual(applied["NORI_TAG"], "ui-tests-display")
        XCTAssertEqual(applied["NORI_SOCKET_PATH"], "/tmp/nori-ui-tests.sock")
    }
}
...
final class PostHogAnalyticsPropertiesTests: XCTestCase {
    ...
    func testSuperPropertiesIncludePlatformVersionAndBuild() {
        ...
        XCTAssertEqual(properties["platform"] as? String, "noriterm")
        XCTAssertEqual(properties["app_version"] as? String, "0.31.0")
        XCTAssertEqual(properties["app_build"] as? String, "230")
    }
    ...
    func testPropertiesOmitVersionFieldsWhenUnavailable() {
        ...
        XCTAssertEqual(superProperties["platform"] as? String, "noriterm")
    ...
    func testFlushPolicyIncludesDailyAndHourlyActiveEvents() {
        XCTAssertTrue(PostHogAnalytics.shouldFlushAfterCapture(event: "nori_daily_active"))
        XCTAssertTrue(PostHogAnalytics.shouldFlushAfterCapture(event: "nori_hourly_active"))
        XCTAssertFalse(PostHogAnalytics.shouldFlushAfterCapture(event: "nori_other_event"))
    }
}
...
final class GhosttyMouseFocusTests: XCTestCase {
    ...
    private func withTempConfig(
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-test-cjk-\(UUID().uuidString)")
    ...
    func testUserConfigContainsCJKCodepointMapReturnsFalseForMissingFiles() {
        let path = NSTemporaryDirectory() + "nori-nonexistent-\(UUID().uuidString)/config"
        XCTAssertFalse(
    ...
    func testUserConfigContainsCJKCodepointMapFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-test-cjk-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapFollowsRelativeIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-test-cjk-rel-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapHandlesOptionalInclude() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-test-cjk-opt-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapHandlesCyclicIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-test-cjk-cycle-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-test-cjk-font-family-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainIgnoresDuplicateFamilies() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-test-cjk-font-family-duplicate-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainMatchesGhosttyIncludeLoadOrder() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-test-cjk-font-family-order-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigHasExplicitFontFamilyFallbackChainRespectsConfigFileReset() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-test-cjk-font-family-config-file-reset-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testLoadedCJKScanPathsSkipsReleaseAppSupportWhenTaggedConfigExists() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-test-cjk-app-support-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }

        let taggedDir = appSupport.appendingPathComponent("com.example.nori-dev", isDirectory: true)
        ...
        let paths = GhosttyApp.loadedCJKScanPaths(
            currentBundleIdentifier: "com.example.nori-dev",
...
final class ZshShellIntegrationHandoffTests: XCTestCase {
    func testGhosttyPromptHooksLoadWhenNoriRequestsZshIntegration() throws {
        let output = try runInteractiveZsh(noriLoadGhosttyIntegration: true)
    ...
    func testGhosttyPromptHooksDoNotLoadWithoutNoriHandoffFlag() throws {
        let output = try runInteractiveZsh(noriLoadGhosttyIntegration: false)
    ...
    func testGhosttySemanticPatchRetriesAfterDeferredInitCreatesLiveHooks() throws {
        let output = try runInteractiveZsh(
            noriLoadGhosttyIntegration: true,
            noriLoadShellIntegration: true,
            command: """
            _nori_patch_ghostty_semantic_redraw
    ...
    func testShellIntegrationWinchGuardDoesNotPrintSpacerLineOnResize() throws {
        let output = try runInteractiveZsh(
            noriLoadGhosttyIntegration: false,
            noriLoadShellIntegration: true,
            command: """
            print -r -- BEFORE
            TRAPWINCH
    ...
    func testShellIntegrationPreservesStartupTermForThemeSelectionBeforeRestoringManagedTerm() throws {
        let output = try runPromptInteractiveZsh(
            ...
            noriLoadShellIntegration: true,
            command: """
            print -r -- "CMD=$TERM|${NORI_ZSH_RESTORE_TERM-unset}" >> "$NORI_TEST_OUTPUT"
            """,
            userZshRCContents: """
            export NORI_STARTUP_THEME_TERM="$TERM"
            if [[ $TERM = (*256color|*rxvt*) ]]; then
              export NORI_STARTUP_THEME_BRANCH=extended
            else
              export NORI_STARTUP_THEME_BRANCH=basic
            fi

            nori_test_ready() {
              print -r -- "PRE=$NORI_STARTUP_THEME_TERM|$NORI_STARTUP_THEME_BRANCH|$TERM|${NORI_ZSH_RESTORE_TERM-unset}" > "$NORI_TEST_OUTPUT"
              : > "$NORI_TEST_READY"
            }
            precmd_functions+=(nori_test_ready)
            """
        )
    ...
    func testShellIntegrationDoesNotSpoofManagedTermForInteractiveCommandMode() throws {
        let output = try runInteractiveZsh(
            ...
            noriLoadShellIntegration: true,
            command: """
            print -r -- "$NORI_STARTUP_TERM|$TERM|${NORI_ZSH_RESTORE_TERM-unset}"
            """,
            userZshRCContents: """
            export NORI_STARTUP_TERM="$TERM"
            """
        )
    ...
    func testShellIntegrationDoesNotSpoofManagedTermWhenIntegrationDisabled() throws {
        let output = try runInteractiveZsh(
            noriLoadGhosttyIntegration: false,
            noriLoadShellIntegration: false,
            command: """
            print -r -- "$NORI_STARTUP_TERM|$TERM|${NORI_ZSH_RESTORE_TERM-unset}"
            """,
            userZshRCContents: """
    ...
    func testShellIntegrationDoesNotSpoofManagedTermWhenUserZshEnvDisablesIntegration() throws {
        let output = try runInteractiveZsh(
            ...
            noriLoadShellIntegration: true,
            command: """
            print -r -- "$NORI_STARTUP_TERM|$TERM|${NORI_ZSH_RESTORE_TERM-unset}|${NORI_SHELL_INTEGRATION:-unset}"
            """,
            userZshEnvContents: """
            export NORI_SHELL_INTEGRATION=0
            """,
            userZshRCContents: """
            export NORI_STARTUP_TERM="$TERM"
            """
    ...
    func testShellIntegrationRestoresManagedTermDuringPreexec() throws {
        let output = try runInteractiveZsh(
            ...
            noriLoadShellIntegration: true,
            command: """
            _nori_preexec 'echo $TERM'
            print -r -- "$TERM|${NORI_ZSH_RESTORE_TERM-unset}"
            """,
            extraEnvironment: [
                "TERM": "xterm-ghostty",
                "NORI_ZSH_RESTORE_TERM": "xterm-256color",
            ]
        )
    ...
    func testShellIntegrationPublishesOnlyWorkspaceScopedNoriEnvironmentToTmuxServerAutomatically() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("nori-zsh-tmux-publish-\(UUID().uuidString)")
        ...
        _ = try runInteractiveZsh(
            noriLoadGhosttyIntegration: false,
            noriLoadShellIntegration: true,
            command: "_nori_preexec tmux; print -r -- READY",
            extraEnvironment: [
                "PATH": "\(binDir.path):/usr/bin:/bin:/usr/sbin:/sbin",
                "NORI_SOCKET_PATH": "/tmp/nori-current.sock",
                "NORI_TAG": "feat-tmux-notification-attention-state",
                "NORI_WORKSPACE_ID": "11111111-1111-1111-1111-111111111111",
                "NORI_SURFACE_ID": "22222222-2222-2222-2222-222222222222",
                "NORI_TAB_ID": "11111111-1111-1111-1111-111111111111",
                "NORI_PANEL_ID": "22222222-2222-2222-2222-222222222222",
            ]
        )

        let log = (try? String(contentsOf: logPath, encoding: .utf8)) ?? ""
        XCTAssertTrue(log.contains("set-environment -g NORI_TAG feat-tmux-notification-attention-state"), log)
        XCTAssertTrue(log.contains("set-environment -g NORI_SOCKET_PATH /tmp/nori-current.sock"), log)
        XCTAssertTrue(log.contains("set-environment -g NORI_WORKSPACE_ID 11111111-1111-1111-1111-111111111111"), log)
        XCTAssertFalse(log.contains("set-environment -g NORI_SURFACE_ID"), log)
        XCTAssertFalse(log.contains("set-environment -g NORI_PANEL_ID"), log)
    }

    func testShellIntegrationClearsStaleSurfaceScopedTmuxEnvironmentAutomatically() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("nori-zsh-tmux-clear-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("tmux.log", isDirectory: false)
        ...
        try writeExecutableScript(
            ...
            #!/bin/sh
            if [ "$1" = "show-environment" ] && [ "$2" = "-g" ]; then
              printf '%s\\n' 'NORI_SURFACE_ID=99999999-9999-9999-9999-999999999999'
              printf '%s\\n' 'NORI_PANEL_ID=99999999-9999-9999-9999-999999999999'
              exit 0
            fi
            printf '%s\\n' "$*" >> "\(logPath.path)"
            exit 0
        ...
        let log = (try? String(contentsOf: logPath, encoding: .utf8)) ?? ""
        XCTAssertTrue(log.contains("set-environment -gu NORI_SURFACE_ID"), log)
        XCTAssertTrue(log.contains("set-environment -gu NORI_PANEL_ID"), log)
    }

    func testShellIntegrationRefreshesWorkspaceScopedNoriEnvironmentFromTmuxWithoutOverwritingSurfaceScope() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("nori-zsh-tmux-refresh-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        ...
        let output = try runInteractiveZsh(
            noriLoadGhosttyIntegration: false,
            noriLoadShellIntegration: true,
            command: "_nori_precmd; print -r -- \"$NORI_TAG|$NORI_SOCKET_PATH|$NORI_WORKSPACE_ID|$NORI_SURFACE_ID|$NORI_PANEL_ID\"",
            extraEnvironment: [
                "PATH": "\(binDir.path):/usr/bin:/bin:/usr/sbin:/sbin",
                "TMUX": "/tmp/tmux-stale,123,0",
                "NORI_SOCKET_PATH": "/tmp/nori-stale.sock",
                "NORI_TAG": "feat-tmux-integration-experiments",
                "NORI_WORKSPACE_ID": "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA",
                "NORI_SURFACE_ID": "22222222-2222-2222-2222-222222222222",
                "NORI_TAB_ID": "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA",
                "NORI_PANEL_ID": "22222222-2222-2222-2222-222222222222",
            ]
        )
        ...
        XCTAssertEqual(
            output,
            "feat-tmux-notification-attention-state|/tmp/nori-current.sock|11111111-1111-1111-1111-111111111111|22222222-2222-2222-2222-222222222222|22222222-2222-2222-2222-222222222222"
        )
    }
    ...
    func testShellIntegrationReportsTTYFromTmuxWithoutUsingPanelScope() throws {
        let output = try runInteractiveZsh(
            ...
            noriLoadShellIntegration: true,
            command: """
            _NORI_TTY_NAME=ttys999
            print -r -- "$(_nori_report_tty_payload)"
            """,
            extraEnvironment: [
    ...
    func testShellIntegrationRelayReportTTYUsesWorkspaceIDInZsh() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("nori-zsh-relay-report-tty-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        ...
        try writeExecutableScript(
            at: binDir.appendingPathComponent("nori", isDirectory: false),
            contents: """
            #!/bin/sh
            printf '%s\\n' "$*" >> "\(logPath.path)"
            exit 0
        ...
        let output = try runInteractiveZsh(
            ...
            command: """
            : > "\(logPath.path)"
            _NORI_TTY_NAME=ttys777
            _nori_report_tty_via_relay
            cat "\(logPath.path)"
            """,
            extraEnvironment: [
                "PATH": "\(binDir.path):/usr/bin:/bin:/usr/sbin:/sbin",
    ...
    func testShellIntegrationRelayPortsKickOmitsSurfaceIDUntilAvailableInZsh() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("nori-zsh-relay-kick-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        let output = try runInteractiveZsh(
            ...
            command: """
            : > "\(logPath.path)"
            _nori_ports_kick_via_relay refresh
            repeat 20; do
    ...
    func testShellIntegrationRelayPromptRefreshUsesRefreshReasonInZsh() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("nori-zsh-relay-precmd-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        let output = try runInteractiveZsh(
            ...
            command: """
            : > "\(logPath.path)"
            _NORI_TTY_REPORTED=1
            _NORI_PORTS_LAST_RUN=-999
            _nori_precmd
            repeat 20; do
              [[ -s "\(logPath.path)" ]] && break
              sleep 0.05
    ...
    func testShellIntegrationRelayReportTTYUsesWorkspaceIDInBash() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("nori-bash-relay-report-tty-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
    ...
    func testShellIntegrationRelayPreexecWorksBeforeSurfaceIDExistsInBash() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("nori-bash-relay-preexec-no-surface-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        let result = try runInteractiveBash(
            ...
            command: """
            : > "\(logPath.path)"
            _NORI_TTY_NAME=ttys889
            _NORI_TTY_REPORTED=0
            _nori_preexec_command "python3 -m http.server 8899"
            for _nori_i in $(seq 1 20); do
              [ -s "\(logPath.path)" ] && break
              sleep 0.05
            done
            cat "\(logPath.path)"
    ...
    func testShellIntegrationRelayPromptRefreshUsesRefreshReasonInBashWithoutPromptNoise() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("nori-bash-relay-prompt-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("relay.log", isDirectory: false)
        ...
        XCTAssertFalse(result.stderr.contains("_nori_report_tmux_state"), result.stderr)
        XCTAssertTrue(
    ...
    private func runInteractiveZsh(noriLoadGhosttyIntegration: Bool) throws -> String {
        try runInteractiveZsh(
            noriLoadGhosttyIntegration: noriLoadGhosttyIntegration,
            noriLoadShellIntegration: false,
    ...
    private func runInteractiveZsh(
        noriLoadGhosttyIntegration: Bool,
        noriLoadShellIntegration: Bool,
        command: String,
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("nori-zsh-shell-integration-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let repoRoot = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
        let noriZdotdir = repoRoot.appendingPathComponent("Resources/shell-integration")
        ...
        process.environment = [
            ...
            "SHELL": "/bin/zsh",
            "USER": NSUserName(),
            "ZDOTDIR": noriZdotdir.path,
            "NORI_ZSH_ZDOTDIR": userZdotdir.path,
            "NORI_SHELL_INTEGRATION": "0",
            "GHOSTTY_RESOURCES_DIR": ghosttyResources.path,
        ]
        if noriLoadGhosttyIntegration {
            process.environment?["NORI_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        }
        if noriLoadShellIntegration {
            process.environment?["NORI_SHELL_INTEGRATION"] = "1"
            process.environment?["NORI_SHELL_INTEGRATION_DIR"] = noriZdotdir.path
            process.environment?["NORI_SOCKET_PATH"] = root.appendingPathComponent("nori-test.sock").path
            process.environment?["NORI_TAB_ID"] = "tab-test"
            process.environment?["NORI_PANEL_ID"] = "panel-test"
        }
    ...
    private func runPromptInteractiveZsh(
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("nori-zsh-prompt-integration-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        process.environment = [
            ...
            "NORI_SHELL_INTEGRATION": "0",
            "GHOSTTY_RESOURCES_DIR": ghosttyResources.path,
            "NORI_TEST_READY": readyPath.path,
            "NORI_TEST_OUTPUT": outputPath.path,
        ]
        if noriLoadGhosttyIntegration {
            process.environment?["NORI_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        }
    ...
    private func runInteractiveBash(
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("nori-bash-shell-integration-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let repoRoot = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let integrationPath = repoRoot.appendingPathComponent("Resources/shell-integration/nori-bash-integration.bash")
        let rcfilePath = root.appendingPathComponent(".bashrc")
        let rcfileContents: String = {
            guard noriLoadShellIntegration else { return ":\n" }
            return """
...
final class BrowserInstallDetectorTests: XCTestCase {
    ...
    private func makeTemporaryHome() -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent("nori-browser-detect-\(UUID().uuidString)")
    }
