...
final class GhosttyPasteboardHelperTests: XCTestCase {
    ...
    func testHTMLOnlyPasteboardExtractsPlainText() {
        let pasteboard = NSPasteboard(name: .init("nori-test-html-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString("<p>Hello <strong>world</strong></p>", forType: .html)

        XCTAssertEqual(noriPasteboardStringContentsForTesting(pasteboard), "Hello world")
        XCTAssertNil(noriPasteboardImagePathForTesting(pasteboard))
    }

    func testAlternatePlainTextUTIExtractsPlainText() {
        let pasteboard = NSPasteboard(name: .init("nori-test-plain-text-uti-\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        XCTAssertEqual(
            noriPasteboardStringContentsForTesting(pasteboard),
            "hello from public.plain-text"
        )
    }

    func testEmptyPlainTextFallsBackToRichTextPayload() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-empty-plain-rich-fallback-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testXHTMLTypeFallsBackToRenderedHTMLText() {
        let pasteboard = NSPasteboard(name: .init("nori-test-xhtml-html-fallback-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString(
    ...
    func testImageClipboardWithPlainTextFallbackStillFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-image-plain-text-fallback-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString(
        ...
        XCTAssertNil(noriPasteboardStringContentsForTesting(pasteboard))

        let imagePath = try XCTUnwrap(noriPasteboardImagePathForTesting(pasteboard))
    ...
    func testImageHTMLClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-image-html-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testImageHTMLClipboardWithGenericPlainTextStillFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-image-html-generic-text-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString("<meta charset='utf-8'><img src=\"https://example.com/keyboard.png\">", forType: .html)
        pasteboard.setString(
            "https://example.com/keyboard.png",
    ...
    func testImageHTMLClipboardWithVisibleTextPrefersText() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-image-html-text-\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        XCTAssertEqual(noriPasteboardStringContentsForTesting(pasteboard), "Hello")
        XCTAssertNil(noriPasteboardImagePathForTesting(pasteboard))
    }

    func testJPEGClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-jpeg-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testAttachmentOnlyRTFDClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-rtfd-attachment-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testAttachmentOnlyRTFDNonImageClipboardDoesNotFallBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-rtfd-non-image-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRTFDClipboardWithVisibleTextPrefersText() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-rtfd-text-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testImageOnlyPasteboardProducesTempFileURL() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-drop-image-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setData(try make1x1PNG(color: .red), forType: .png)

        let fileURL = try XCTUnwrap(noriPasteboardImageFileURLForTesting(pasteboard))
    ...
    func testRemoteImageDropPlanUploadsMaterializedFile() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-remote-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testLocalImageDropPlanInsertsEscapedLocalPath() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-local-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteImagePastePlanUploadsMaterializedFile() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-remote-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteFileURLPastePlanUploadsReadableFile() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("nori-test-remote-file-url-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteDirectoryPastePlanFallsBackToEscapedPathInsertion() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("nori-test-remote-directory-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testLazyPastePlanSkipsTargetResolutionForPlainText() {
        let pasteboard = NSPasteboard(name: .init("nori-test-lazy-text-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testPastePlanFallsBackToAlternatePlainTextWhenImageTypeIsUnusable() {
        let pasteboard = NSPasteboard(name: .init("nori-test-raycast-fallback-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString(
            "hello from Raycast",
            forType: NSPasteboard.PasteboardType(UTType.plainText.identifier)
    ...
    func testLazyPastePlanResolvesTargetForFileURLPaste() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("nori-test-lazy-file-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
        XCTAssertTrue(pasteboard.writeObjects([fileURL as NSURL]))
    ...
    func testLocalImagePastePlanInsertsEscapedLocalPath() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-local-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteImagePasteExecutionUploadsAndCompletesWithRemotePath() throws {
        ...
        TerminalImageTransferPlanner.executeForTesting(
            ...
            uploadWorkspaceRemote: { _, _, finish in finish(.success(["/tmp/nori-drop-123.png"])) },
        ...
        XCTAssertEqual(completedText, "/tmp/nori-drop-123.png")
    }
    ...
    func testCancelledRemoteImagePasteExecutionSuppressesCompletionHandlers() throws {
        ...
        completion?(.success(["/tmp/nori-drop-cancelled.png"]))
    ...
    func testCancelledOperationSuppressesLateLocalInsert() {
        ...
        let returnedOperation = TerminalImageTransferPlanner.executeForTesting(
            plan: .insertText("/tmp/nori-drop-local.png"),
            operation: operation,
    ...
    func testRemoteImageDropHandlerUploadsAndSendsRemotePath() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-remote-handler-\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        let handled = GhosttyNSView.handleDropForTesting(
            ...
            uploadRemote: { urls, finish in
                uploadedURLs = urls
                finish(.success(["/tmp/nori-drop-abc123.png"]))
            },
        ...
        XCTAssertEqual(sentText, ["/tmp/nori-drop-abc123.png"])
        XCTAssertEqual(failureCount, 0)
    }

    func testRemoteImageDropHandlerCleansUpMaterializedTemporaryImageAfterSuccess() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-remote-handler-cleanup-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setData(try make1x1PNG(color: .orange), forType: .png)
    ...
    func testRemoteDropUploadFailureTriggersFailureHandler() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-remote-handler-fail-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteImageDropHandlerCleansUpMaterializedTemporaryImageAfterFailure() throws {
        let pasteboard = NSPasteboard(name: .init("nori-test-remote-handler-failure-cleanup-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setData(try make1x1PNG(color: .cyan), forType: .png)
...
final class GhosttyResponderResolutionTests: XCTestCase {
    ...
    func testResolvesGhosttyViewFromDescendantResponder() {
        ...
        XCTAssertTrue(noriOwningGhosttyView(for: descendant) === ghosttyView)
    }

    func testResolvesGhosttyViewFromGhosttyResponder() {
        let ghosttyView = GhosttyNSView(frame: NSRect(x: 0, y: 0, width: 200, height: 120))
        XCTAssertTrue(noriOwningGhosttyView(for: ghosttyView) === ghosttyView)
    }

    func testReturnsNilForUnrelatedResponder() {
        let view = FocusProbeView(frame: NSRect(x: 0, y: 0, width: 40, height: 40))
        XCTAssertNil(noriOwningGhosttyView(for: view))
    }
}
...
final class TerminalOpenURLTargetResolutionTests: XCTestCase {
    ...
    func testResolvesFileSchemeAsExternal() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("file:///tmp/nori.txt"))
        switch target {
        case let .external(url):
            XCTAssertTrue(url.isFileURL)
            XCTAssertEqual(url.path, "/tmp/nori.txt")
        default:
    ...
    func testResolvesAbsolutePathAsExternalFileURL() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("/tmp/nori-path.txt"))
        switch target {
        case let .external(url):
            XCTAssertTrue(url.isFileURL)
            XCTAssertEqual(url.path, "/tmp/nori-path.txt")
        default:
    ...
    func testResolvesHostlessHTTPSAsExternal() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("https:///tmp/nori.txt"))
        switch target {
        case let .external(url):
            XCTAssertEqual(url.scheme, "https")
...
final class TerminalCmdClickPathPunctuationTrimmingTests: XCTestCase {
    func testTrimsTrailingPeriodAfterMarkdownFile() {
        XCTAssertEqual(
            noriTrimTerminalPathTrailingPunctuationForTesting(
    ...
    func testResolveQuicklookFallsBackToStrippedPathWhenLiteralPathIsMissing() {
        let strippedPath = "/tmp/nori-cmdclick-path.md"

        XCTAssertEqual(
            noriResolveQuicklookPathForTesting(
                "\(strippedPath).",
                cwd: "/tmp",
    ...
    func testResolveQuicklookPrefersLiteralPathThatReallyEndsWithDot() {
        let literalPath = "/tmp/nori-cmdclick-literal-dot.md."
        let strippedPath = "/tmp/nori-cmdclick-literal-dot.md"
    ...
    func testResolveQuicklookPrefersLiteralPathThatReallyEndsWithParen() {
        let literalPath = "/tmp/nori-cmdclick-literal-paren)"
        let strippedPath = "/tmp/nori-cmdclick-literal-paren"
...
final class TerminalControllerSocketListenerHealthTests: XCTestCase {
    ...
    func testNonStableSocketBindFailureDoesNotFallback() {
        XCTAssertNil(
            TerminalController.fallbackSocketPathAfterBindFailure(
                requestedPath: "/tmp/nori-debug.sock",
                stage: "bind",
                errnoCode: EACCES,
                currentUserID: 501
            )
    ...
    private func makeTempSocketPath() -> String {
        "/tmp/nori-socket-health-\(UUID().uuidString).sock"
    }
