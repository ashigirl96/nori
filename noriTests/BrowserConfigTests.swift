...
var noriUnitTestInspectorAssociationKey: UInt8 = 0
var noriUnitTestInspectorOverrideInstalled = false
var noriUnitTestWKWebViewPerformKeyEquivalentOverrideInstalled = false
var noriUnitTestWKWebViewPerformKeyEquivalentHook: ((WKWebView, NSEvent) -> Bool?)?

extension NoriWebView {
    @objc func noriUnitTestInspector() -> NSObject? {
        objc_getAssociatedObject(self, &noriUnitTestInspectorAssociationKey) as? NSObject
    }
}

extension WKWebView {
    @objc func noriUnitTest_performKeyEquivalent(with event: NSEvent) -> Bool {
        if let hook = noriUnitTestWKWebViewPerformKeyEquivalentHook,
           let result = hook(self, event) {
            return result
        }
        return noriUnitTest_performKeyEquivalent(with: event)
    }

    func noriSetUnitTestInspector(_ inspector: NSObject?) {
        objc_setAssociatedObject(
            self,
            &noriUnitTestInspectorAssociationKey,
            inspector,
...
func installNoriUnitTestInspectorOverride() {
    guard !noriUnitTestInspectorOverrideInstalled else { return }

    guard let replacementMethod = class_getInstanceMethod(
        NoriWebView.self,
        #selector(NoriWebView.noriUnitTestInspector)
    ) else {
    ...
    guard added else {
        fatalError("Unable to install NoriWebView _inspector test override")
    }

    noriUnitTestInspectorOverrideInstalled = true
}

func installNoriUnitTestWKWebViewPerformKeyEquivalentOverride() {
    guard !noriUnitTestWKWebViewPerformKeyEquivalentOverrideInstalled else { return }

    let originalSelector = #selector(NSResponder.performKeyEquivalent(with:))
    let swizzledSelector = #selector(WKWebView.noriUnitTest_performKeyEquivalent(with:))
    ...
    noriUnitTestWKWebViewPerformKeyEquivalentOverrideInstalled = true
}
...
final class NoriWebViewKeyEquivalentTests: XCTestCase {
    ...
    func testCmdNRoutesToMainMenuWhenWebViewIsFirstResponder() {
        ...
        let webView = NoriWebView(frame: .zero, configuration: WKWebViewConfiguration())
    ...
    func testCanBlockFirstResponderAcquisitionWhenPaneIsUnfocused() {
        ...
        let webView = NoriWebView(frame: container.bounds, configuration: WKWebViewConfiguration())
    ...
    func testWindowFirstResponderGuardAllowsPointerInitiatedClickFocusFromPortalHostedInspectorSibling() {
        ...
        let webView = NoriWebView(frame: slot.bounds, configuration: WKWebViewConfiguration())
        webView.autoresizingMask = [.width, .height]
    ...
    func testCmdBacktickDoesNotRouteDirectlyToMainMenuWhenWebViewIsFirstResponder() {
        ...
        XCTAssertFalse(
            spy.invoked,
            "NoriWebView should not route Cmd+` directly to the menu when WebKit is first responder"
        )
    }
    ...
    func testCmdFDoesNotPreflightIntoPageWhenWebInspectorResponderIsFocused() {
        _ = NSApplication.shared
        installNoriUnitTestWKWebViewPerformKeyEquivalentOverride()
        ...
        noriUnitTestWKWebViewPerformKeyEquivalentHook = { currentWebView, event in
        ...
        window.makeKeyAndOrderFront(nil)
        defer {
            noriUnitTestWKWebViewPerformKeyEquivalentHook = nil
            window.orderOut(nil)
        }
        ...
        XCTAssertEqual(
            ...
            0,
            "Did not expect NoriWebView to preflight Cmd+F into page content while Web Inspector is focused"
        )
    }
...
@MainActor
final class NoriWebViewContextMenuTests: XCTestCase {
    ...
    func testWillOpenMenuAddsOpenLinkInDefaultBrowserAndRoutesSelectionToDefaultBrowserOpener() {
        _ = NSApplication.shared
        let webView = NoriWebView(frame: NSRect(x: 0, y: 0, width: 800, height: 600), configuration: WKWebViewConfiguration())
        let menu = NSMenu()
...
final class BrowserSessionHistoryRestoreTests: XCTestCase {
    ...
    func testGoBackPrefersLiveWKWebViewHistoryBeforeRestoredFallback() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("nori-browser-history-\(UUID().uuidString)", isDirectory: true)
...
final class BrowserDeveloperToolsVisibilityPersistenceTests: XCTestCase {
    ...
    override class func setUp() {
        super.setUp()
        installNoriUnitTestInspectorOverride()
    }
    ...
    private func makePanelWithInspector(
        ...
        panel.webView.noriSetUnitTestInspector(inspector)
...
@MainActor
final class NoriWebViewDragRoutingTests: XCTestCase {
    func testRejectsInternalPaneDragEvenWhenFilePromiseTypesArePresent() {
        XCTAssertTrue(
            NoriWebView.shouldRejectInternalPaneDrag([
    ...
    func testAllowsRegularExternalFileDrops() {
        XCTAssertFalse(NoriWebView.shouldRejectInternalPaneDrag([.fileURL]))
    }
}
...
final class BrowserLinkOpenSettingsTests: XCTestCase {
    ...
    func testTerminalLinksDefaultToNoriBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.openTerminalLinksInNoriBrowser(defaults: defaults))
    }

    func testTerminalLinksPreferenceUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openTerminalLinksInNoriBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.openTerminalLinksInNoriBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openTerminalLinksInNoriBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.openTerminalLinksInNoriBrowser(defaults: defaults))
    }

    func testSidebarPullRequestLinksDefaultToNoriBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.openSidebarPullRequestLinksInNoriBrowser(defaults: defaults))
    }

    func testSidebarPullRequestLinksPreferenceUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openSidebarPullRequestLinksInNoriBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.openSidebarPullRequestLinksInNoriBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openSidebarPullRequestLinksInNoriBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.openSidebarPullRequestLinksInNoriBrowser(defaults: defaults))
    }

    func testOpenCommandInterceptionDefaultsToNoriBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.interceptTerminalOpenCommandInNoriBrowser(defaults: defaults))
    }

    func testOpenCommandInterceptionUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.interceptTerminalOpenCommandInNoriBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.interceptTerminalOpenCommandInNoriBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.interceptTerminalOpenCommandInNoriBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.interceptTerminalOpenCommandInNoriBrowser(defaults: defaults))
    }
    ...
    func testSettingsInitialOpenCommandInterceptionValueFallsBackToLegacyLinkToggleWhenUnset() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openTerminalLinksInNoriBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInNoriBrowserValue(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openTerminalLinksInNoriBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInNoriBrowserValue(defaults: defaults))
    }
...
final class BrowserNavigableURLResolutionTests: XCTestCase {
    func testResolvesFileSchemeAsNavigableURL() throws {
        let resolved = try XCTUnwrap(resolveBrowserNavigableURL("file:///tmp/nori-local-test.html"))
        XCTAssertTrue(resolved.isFileURL)
        XCTAssertEqual(resolved.path, "/tmp/nori-local-test.html")
    }
...
final class BrowserExternalNavigationSchemeTests: XCTestCase {
    ...
    func testEmbeddedBrowserSchemesStayInWebView() throws {
        ...
        let file = try XCTUnwrap(URL(string: "file:///tmp/nori-local-test.html"))
