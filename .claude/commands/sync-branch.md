...
1. **Update submodules to latest**
   - For each submodule (ghostty, homebrew-nori, vendor/bonsplit):
     - `cd <submodule>`
     - `git fetch origin`
...
2. **Commit submodule updates on main**
   ...
   - If changed, stage and commit: `git add ghostty homebrew-nori vendor/bonsplit && git commit -m "Update submodules: <brief description>"`
