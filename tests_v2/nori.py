#!/usr/bin/env python3
"""nori v2 Python Client

A client library for programmatically controlling nori via the Unix socket.
...
def _default_socket_path() -> str:
    # Backwards/forward compatibility: some scripts export NORI_SOCKET,
    # while the client historically used NORI_SOCKET_PATH.
    override = os.environ.get("NORI_SOCKET_PATH") or os.environ.get("NORI_SOCKET")
    if override:
    ...
    discovered = glob.glob("/tmp/nori-debug-*.sock")
    discovered.extend(glob.glob(os.path.join(_APP_SUPPORT_DIR, "nori*.sock")))
...
class nori:
    """Client for controlling nori via the v2 JSON Unix socket."""
    ...
    def connect(self) -> None:
        ...
        while not os.path.exists(self.socket_path):
            if time.time() - start >= 10.0:
                raise noriError(
                    f"Socket not found at {self.socket_path}. Is nori running?"
                )
            time.sleep(0.1)
    ...
    def _recv_line(self, timeout_s: float = 20.0) -> str:
        ...
        raise noriError("Timed out waiting for response")
    ...
    def _call(self, method: str, params: Optional[Dict[str, Any]] = None, timeout_s: float = 20.0) -> Any:
        ...
        try:
            ...
            raise noriError(f"Invalid JSON response: {e}: {resp_line[:200]}")

        if not isinstance(resp, dict):
            raise noriError(f"Invalid response type: {type(resp).__name__}")

        if resp.get("id") != req_id:
            raise noriError(f"Mismatched response id: expected {req_id}, got {resp.get('id')}")
        ...
        data = err.get("data")
        if data is not None:
            raise noriError(f"{code}: {msg} ({data})")
        raise noriError(f"{code}: {msg}")
    ...
    def _resolve_workspace_id(self, workspace: Union[str, int, None]) -> Optional[str]:
        if workspace is None:
            ...
            if not wsid:
                raise noriError("No workspace selected")
            return str(wsid)
        ...
        if isinstance(workspace, int):
            ...
            raise noriError(f"Workspace index not found: {workspace}")
        ...
        if not _looks_like_uuid(s):
            raise noriError(f"Invalid workspace id: {s}")
        return s
    ...
    def _resolve_surface_id(self, surface: Union[str, int, None], workspace_id: Optional[str] = None) -> Optional[str]:
        ...
        if isinstance(surface, int):
            ...
            for row in items:
                if int(row.get("index", -1)) == surface:
                    return str(row.get("id"))
            raise noriError(f"Surface index not found: {surface}")
        ...
        if _looks_like_ref(s, "surface"):
            return s
        if not _looks_like_uuid(s):
            raise noriError(f"Invalid surface id: {s}")
        return s
    ...
    def _resolve_pane_id(self, pane: Union[str, int, None], workspace_id: Optional[str] = None) -> Optional[str]:
        ...
        if isinstance(pane, int):
            ...
            for row in items:
                if int(row.get("index", -1)) == pane:
                    return str(row.get("id"))
            raise noriError(f"Pane index not found: {pane}")
        ...
        if _looks_like_ref(s, "pane"):
            return s
        if not _looks_like_uuid(s):
            raise noriError(f"Invalid pane id: {s}")
        return s
    ...
    def current_window(self) -> str:
        ...
        if not wid:
            raise noriError(f"window.current returned no window_id: {res}")
        return str(wid)
    ...
    def new_window(self) -> str:
        ...
        wid = res.get("window_id")
        if not wid:
            raise noriError(f"window.create returned no window_id: {res}")
        return str(wid)
    ...
    def new_workspace(self, window_id: Optional[str] = None) -> str:
        ...
        if not wsid:
            raise noriError(f"workspace.create returned no workspace_id: {res}")
        return str(wsid)
    ...
    def rename_workspace(self, title: str, workspace: Union[str, int, None] = None) -> None:
        ...
        if not renamed:
            raise noriError("rename_workspace requires a non-empty title")
        wsid = self._resolve_workspace_id(workspace)
    ...
    def current_workspace(self) -> str:
        ...
        if not wsid:
            raise noriError("No current workspace")
        return wsid
    ...
    def next_workspace(self) -> str:
        ...
        wsid = res.get("workspace_id")
        if not wsid:
            raise noriError(f"workspace.next returned no workspace_id: {res}")
        return str(wsid)
    ...
    def previous_workspace(self) -> str:
        ...
        wsid = res.get("workspace_id")
        if not wsid:
            raise noriError(f"workspace.previous returned no workspace_id: {res}")
        return str(wsid)
    ...
    def last_workspace(self) -> str:
        ...
        wsid = res.get("workspace_id")
        if not wsid:
            raise noriError(f"workspace.last returned no workspace_id: {res}")
        return str(wsid)
    ...
    def reorder_workspace(
        ...
        *,
        ...
        if after_workspace is not None:
            ...
            targets += 1
        if targets != 1:
            raise noriError("reorder_workspace requires exactly one target: index|before_workspace|after_workspace")
    ...
    def focus_surface(self, surface: Union[str, int]) -> None:
        ...
        if not sid:
            raise noriError(f"Invalid surface: {surface!r}")
    ...
    def new_split(self, direction: str) -> str:
        ...
        if not sid:
            raise noriError(f"surface.split returned no surface_id: {res}")
        return str(sid)
    ...
    def new_pane(self, direction: str = "right", panel_type: str = "terminal", url: str = None) -> str:
        ...
        sid = res.get("surface_id")
        if not sid:
            raise noriError(f"pane.create returned no surface_id: {res}")
        return str(sid)
    ...
    def new_surface(self, pane: Union[str, int, None] = None, panel_type: str = "terminal", url: str = None) -> str:
        ...
        if pane is not None:
            ...
            if not pid:
                raise noriError(f"Invalid pane: {pane!r}")
            params["pane_id"] = pid
        if url:
            params["url"] = url
        ...
        sid = res.get("surface_id")
        if not sid:
            raise noriError(f"surface.create returned no surface_id: {res}")
        return str(sid)
    ...
    def move_surface(
        ...
        *,
        ...
        if workspace is not None:
            wsid = self._resolve_workspace_id(workspace)
            if not wsid:
                raise noriError(f"Invalid workspace: {workspace!r}")
            params["workspace_id"] = wsid
        if window_id is not None:
            params["window_id"] = str(window_id)
        if before_surface is not None:
            ...
            if not before_id:
                raise noriError(f"Invalid before_surface: {before_surface!r}")
            params["before_surface_id"] = before_id
        if after_surface is not None:
            after_id = self._resolve_surface_id(after_surface)
            if not after_id:
                raise noriError(f"Invalid after_surface: {after_surface!r}")
    ...
    def reorder_surface(
        ...
        *,
        ...
        if after_surface is not None:
            ...
            targets += 1
        if targets != 1:
            raise noriError("reorder_surface requires exactly one target: index|before_surface|after_surface")
    ...
    def swap_pane(self, pane: Union[str, int], target_pane: Union[str, int], focus: bool = True) -> None:
        ...
        if not source or not target:
            raise noriError(f"Invalid panes: pane={pane!r}, target_pane={target_pane!r}")
    ...
    def break_pane(self, pane: Union[str, int, None] = None, surface: Union[str, int, None] = None, focus: bool = True) -> str:
        ...
        wsid = res.get("workspace_id")
        if not wsid:
            raise noriError(f"pane.break returned no workspace_id: {res}")
        return str(wsid)
    ...
    def join_pane(
        ...
        target = self._resolve_pane_id(target_pane)
        if not target:
            raise noriError(f"Invalid target_pane: {target_pane!r}")
    ...
    def last_pane(self) -> str:
        ...
        pid = res.get("pane_id")
        if not pid:
            raise noriError(f"pane.last returned no pane_id: {res}")
        return str(pid)
    ...
    def open_browser(self, url: str = None) -> str:
        ...
        sid = res.get("surface_id")
        if not sid:
            raise noriError(f"browser.open_split returned no surface_id: {res}")
        return str(sid)

    def navigate(self, panel_id: str, url: str) -> None:
        sid = self._resolve_surface_id(panel_id)
        if not sid:
            raise noriError(f"Invalid surface: {panel_id!r}")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="nori v2 socket client")
    parser.add_argument("-s", "--socket", default=nori.DEFAULT_SOCKET_PATH, help="Socket path")
    parser.add_argument("--method", help="v2 method name")
    parser.add_argument("--params", default="{}", help="JSON params")

    args = parser.parse_args()

    with nori(args.socket) as c:
        if not args.method:
            # Minimal smoke.
