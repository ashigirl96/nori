...
def _run_cli_json(cli: str, args: list[str], retries: int = 4) -> dict:
    ...
    for attempt in range(1, retries + 1):
        ...
        if proc.returncode == 0:
            try:
                ...
                raise noriError(f"Invalid CLI JSON output for {' '.join(args)}: {proc.stdout!r} ({exc})")
    ...
    raise noriError(f"CLI failed ({' '.join(args)}): {last_merged}")
...
def _run_cli_expect_failure(cli: str, args: list[str], needles: list[str]) -> None:
    ...
    if proc.returncode == 0:
        raise noriError(f"Expected CLI failure for {' '.join(args)}, but it succeeded: {proc.stdout}")
    merged = f"{proc.stdout}\n{proc.stderr}"
    if not any(needle in merged for needle in needles):
        raise noriError(f"Expected CLI failure containing one of {needles!r} for {' '.join(args)}, got: {merged}")


@contextmanager
def _local_test_server() -> str:
    with tempfile.TemporaryDirectory(prefix="nori-browser-cli-") as root:
        root_path = Path(root)
...
def main() -> int:
    ...
    with _local_test_server() as page_url:
        ...
        os.environ["NORI_WORKSPACE_ID"] = workspace
        ...
        state_file = tempfile.NamedTemporaryFile(delete=False, prefix="nori-cli-state-", suffix=".json").name
