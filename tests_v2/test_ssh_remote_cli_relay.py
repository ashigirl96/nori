#!/usr/bin/env python3
"""Docker integration: verify nori CLI commands work over SSH via reverse socket forwarding."""
...
REMOTE_HTTP_PORT = int(os.environ.get("NORI_SSH_TEST_REMOTE_HTTP_PORT", "81"))
...
def _wait_for_remote_ready(client, workspace_id: str, timeout: float = 45.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        if state == "connected" and daemon_state == "ready":
            return last_status
        time.sleep(0.5)
    raise noriError(f"Remote daemon did not become ready: {last_status}")
...
def _assert_remote_ping(host: str, host_port: int, key_path: Path, remote_socket_addr: str, *, label: str) -> None:
    ping_result = _ssh_run(
        ...
        f"NORI_SOCKET_PATH={remote_socket_addr} $HOME/.nori/bin/nori ping",
        check=False,
    )
    _must(
        ping_result.returncode == 0 and "pong" in ping_result.stdout.lower(),
        f"{label} nori ping failed: rc={ping_result.returncode} stdout={ping_result.stdout!r} stderr={ping_result.stderr!r}",
    )
...
def main() -> int:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="nori-ssh-cli-relay-"))
    image_tag = f"nori-ssh-test:{secrets.token_hex(4)}"
    container_name = f"nori-ssh-cli-relay-{secrets.token_hex(4)}"
    workspace_id = ""
    workspace_id_2 = ""
    ...
    try:
        ...
        with nori(SOCKET_PATH) as client:
            ...
            _must(
                'PATH="$HOME/.nori/bin:$PATH"' in startup_cmd,
                f"ssh startup command should prepend ~/.nori/bin for remote nori CLI: {startup_cmd!r}",
            )
            _must(
                f"NORI_SOCKET_PATH={remote_socket_addr}" in startup_cmd,
                f"ssh startup command should pin NORI_SOCKET_PATH to workspace relay: {startup_cmd!r}",
            )
            ...
            _must(
                ...
                f"nori ssh should focus created workspace: current={current_workspace_id!r} created={workspace_id!r}",
            )
            ...
            # Verify remote nori wrapper + relay-specific daemon mapping were installed.
            wrapper_check = None
            wrapper_deadline = time.time() + 10.0
            while time.time() < wrapper_deadline:
                wrapper_check = _ssh_run(
                    host, host_ssh_port, key_path,
                    f"test -x \"$HOME/.nori/bin/nori\" && test -f \"$HOME/.nori/bin/nori\" && "
                    f"map=\"$HOME/.nori/relay/{remote_relay_port}.daemon_path\" && "
            ...
            _must(
                ...
                f"Expected remote nori wrapper+relay mapping to exist: {wrapper_check.stdout if wrapper_check else ''} {wrapper_check.stderr if wrapper_check else ''}",
            )
            ...
            if not workspace_id_2 and workspace_ref_2.startswith("workspace:"):
                ...
                for row in listed_2.get("workspaces") or []:
                    if str(row.get("ref") or "") == workspace_ref_2:
                        ...
                        break
            _must(bool(workspace_id_2), f"second nori ssh output missing workspace_id: {payload_2}")

            remote_relay_port_2 = payload_2.get("remote_relay_port")
            _must(remote_relay_port_2 is not None, f"second nori ssh output missing remote_relay_port: {payload_2}")
            ...
            _must(
                f"NORI_SOCKET_PATH={remote_socket_addr_2}" in startup_cmd_2,
                f"second ssh startup command should pin NORI_SOCKET_PATH to second relay: {startup_cmd_2!r}",
            )
            ...
            # Test 1: nori ping (v1)
            _assert_remote_ping(host, host_ssh_port, key_path, remote_socket_addr, label="nori")

            # Test 2: nori list-workspaces --json (v2)
            list_ws_result = _ssh_run(
                host, host_ssh_port, key_path,
                f"NORI_SOCKET_PATH={remote_socket_addr} $HOME/.nori/bin/nori --json list-workspaces",
                check=False,
            )
            _must(
                list_ws_result.returncode == 0,
                f"nori list-workspaces failed: rc={list_ws_result.returncode} stderr={list_ws_result.stderr!r}",
            )
            try:
                ...
                raise noriError(f"list-workspaces returned invalid JSON: {list_ws_result.stdout!r}")

            # Test 3: nori new-window (v1)
            new_win_result = _ssh_run(
                host, host_ssh_port, key_path,
                f"NORI_SOCKET_PATH={remote_socket_addr} $HOME/.nori/bin/nori new-window",
                check=False,
            )
            _must(
                new_win_result.returncode == 0,
                f"nori new-window failed: rc={new_win_result.returncode} stderr={new_win_result.stderr!r}",
            )

            # Test 4: nori rpc system.capabilities (v2 passthrough)
            rpc_result = _ssh_run(
                host, host_ssh_port, key_path,
                f"NORI_SOCKET_PATH={remote_socket_addr} $HOME/.nori/bin/nori rpc system.capabilities",
                check=False,
            )
            _must(
                rpc_result.returncode == 0,
                f"nori rpc system.capabilities failed: rc={rpc_result.returncode} stderr={rpc_result.stderr!r}",
            )
            try:
                ...
                raise noriError(f"rpc system.capabilities returned invalid JSON: {rpc_result.stdout!r}")
        ...
        print("PASS: nori CLI commands relay correctly over SSH reverse socket forwarding")
        return 0
