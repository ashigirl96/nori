...
def _focused_pane_id(client):
    ...
    if not focused:
        raise noriError(f"no focused pane: {panes}")
...
def _selected_surface_title(client, pane_id):
    ...
    if not selected:
        raise noriError(f"no selected surface in pane {pane_id}: {rows}")
...
def main():
    with nori(SOCKET_PATH) as client:
        ...
        if new_title != rename_to:
            raise noriError(f"rename not applied: expected '{rename_to}', got '{new_title}'")
