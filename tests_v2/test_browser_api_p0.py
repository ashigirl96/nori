...
<!doctype html>
<html>
  <head><title>nori-browser-p0</title></head>
  <body>
    <input id='name' value=''>
...
""".strip()
        ...
        _must("nori" in str(out.get("value", "")), f"Expected #out text to include 'nori': {out}")
        ...
        _must(str(eval_res.get("value", "")) == "nori", f"Expected eval value 'nori': {eval_res}")

        snap = c._call("browser.snapshot", {"surface_id": target}) or {}
        snapshot_text = str(snap.get("snapshot") or "")
        _must("nori-browser-p0" in snapshot_text, f"Expected snapshot text to include page title: {snap}")
    ...
    print("PASS: browser.* P0 methods work on nori webview with ref handles")
    return 0
