...
FUZZ_SEED = int(os.environ.get("NORI_SPLIT_2PANE_SEED", "20260223"))
WORKSPACES = int(os.environ.get("NORI_SPLIT_2PANE_WORKSPACES", "3"))
CYCLES_PER_WORKSPACE = int(os.environ.get("NORI_SPLIT_2PANE_CYCLES", "220"))
TRANSITION_TIMEOUT_S = float(os.environ.get("NORI_SPLIT_2PANE_TIMEOUT_S", "2.0"))
HOLD_MIN_S = float(os.environ.get("NORI_SPLIT_2PANE_HOLD_MIN_S", "0.003"))
HOLD_MAX_S = float(os.environ.get("NORI_SPLIT_2PANE_HOLD_MAX_S", "0.018"))
PRE_ACTION_JITTER_MAX_S = float(os.environ.get("NORI_SPLIT_2PANE_ACTION_JITTER_MAX_S", "0.002"))
EPSILON = float(os.environ.get("NORI_SPLIT_2PANE_EPSILON", "0.0"))
TRACE_TAIL = int(os.environ.get("NORI_SPLIT_2PANE_TRACE_TAIL", "64"))
LAYOUT_POLL_SLEEP_S = float(os.environ.get("NORI_SPLIT_2PANE_POLL_SLEEP_S", "0.0008"))
LAYOUT_TIMEOUT_RETRIES = int(os.environ.get("NORI_SPLIT_2PANE_LAYOUT_TIMEOUT_RETRIES", "4"))
LAYOUT_TIMEOUT_RETRY_SLEEP_S = float(os.environ.get("NORI_SPLIT_2PANE_LAYOUT_TIMEOUT_RETRY_SLEEP_S", "0.0015"))
MAX_LAYOUT_TIMEOUTS = int(os.environ.get("NORI_SPLIT_2PANE_MAX_LAYOUT_TIMEOUTS", "80"))
CTRL_D_RETRY_INTERVAL_S = float(os.environ.get("NORI_SPLIT_2PANE_CTRL_D_RETRY_INTERVAL_S", "0.18"))
CTRL_D_MAX_EXTRA = int(os.environ.get("NORI_SPLIT_2PANE_CTRL_D_MAX_EXTRA", "6"))
...
def _largest_split_frame(layout_payload: dict) -> dict:
    ...
    if best is None:
        raise noriError(f"layout_debug contains no usable split-view frame: {layout_payload}")
    return best
...
def _rightmost_pane_id(layout_payload: dict) -> str:
    panes = _pane_frames_sorted_x(layout_payload)
    if len(panes) < 2:
        raise noriError(f"Expected at least 2 panes to resolve rightmost pane: {panes}")
    pane_id = str(panes[-1].get("pane_id") or "")
    if not pane_id:
        raise noriError(f"Rightmost pane is missing pane_id: {panes[-1]}")
    return pane_id
...
def _rightmost_panel_id(layout_payload: dict) -> str:
    ...
    if not panel_id:
        raise noriError(f"Missing selected panel for rightmost pane: pane_id={pane_id}, selected={selected}")
    return panel_id


def _safe_layout_debug(c: nori, *, timeout_state: dict[str, int], context: str) -> dict:
    ...
    raise noriError(f"layout_debug retry loop exhausted unexpectedly (context={context})")
...
def main() -> int:
    ...
    with nori(SOCKET_PATH) as c:
        ...
        for workspace_index in range(1, WORKSPACES + 1):
            ...
            if start_panes != 1:
                raise noriError(f"New workspace did not start as single pane (workspace={workspace_index}, panes={start_panes})")
