...
def _wait_for_file_text(path: Path, needle: str, timeout_s: float = 8.0) -> str:
    ...
    while time.time() < deadline:
        ...
        if needle in last_text:
            return last_text
        time.sleep(0.1)
    raise noriError(f"Timed out waiting for {needle!r} in background workspace file: {last_text!r}")
...
def main() -> int:
    with nori(SOCKET_PATH) as c:
        ...
        created_workspace = ""
        marker_path = Path(tempfile.gettempdir()) / f"nori-bg-start-{int(time.time() * 1000)}.txt"
        try:
            token = f"NORI_BG_START_{int(time.time() * 1000)}"
            initial_command = (
                "python3 -c " +
                shlex.quote(
