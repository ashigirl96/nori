...
def _resolve_socket_path() -> str:
    socket_path = os.environ.get("NORI_SOCKET", "").strip()
    if not socket_path:
        raise noriError("NORI_SOCKET is required (expected /tmp/nori-debug-<tag>.sock)")
    if not re.fullmatch(r"/tmp/nori-debug-[^/]+\.sock", socket_path):
        raise noriError(f"NORI_SOCKET must be a tagged debug socket, got: {socket_path!r}")
    return socket_path
...
def _find_cli_binary() -> str:
    ...
    candidates = glob.glob(
        os.path.expanduser("~/Library/Developer/Xcode/DerivedData/**/Build/Products/Debug/nori"),
        recursive=True,
    )
...
def _create_git_repo(root: Path) -> Path:
    ...
    subprocess.run(
        ["git", "config", "user.name", "nori-test"],
        cwd=repo,
        check=True,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    ...
    subprocess.run(
        ["git", "config", "user.email", "nori-test@example.com"],
        cwd=repo,
        check=True,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
...
def main() -> int:
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="nori_issue_915_external_git_"))
