#!/usr/bin/env python3
"""Regression: `nori ssh` creates a remote-tagged workspace with remote metadata."""
...
from nori import nori, noriError


SOCKET_PATH = os.environ.get("NORI_SOCKET", "/tmp/nori-debug.sock")
...
def _find_cli_binary() -> str:
    env_cli = os.environ.get("NORITERM_CLI")
    if env_cli and os.path.isfile(env_cli) and os.access(env_cli, os.X_OK):
        return env_cli

    fixed = os.path.expanduser("~/Library/Developer/Xcode/DerivedData/nori-tests-v2/Build/Products/Debug/nori")
    if os.path.isfile(fixed) and os.access(fixed, os.X_OK):
        return fixed

    candidates = glob.glob(os.path.expanduser("~/Library/Developer/Xcode/DerivedData/**/Build/Products/Debug/nori"), recursive=True)
    candidates += glob.glob("/tmp/nori-*/Build/Products/Debug/nori")
    candidates = [p for p in candidates if os.path.isfile(p) and os.access(p, os.X_OK)]
    if not candidates:
        raise noriError("Could not locate nori CLI binary; set NORITERM_CLI")
...
def _run_cli(cli: str, args: list[str], *, json_output: bool, extra_env: dict[str, str] | None = None) -> str:
    env = dict(os.environ)
    env.pop("NORI_WORKSPACE_ID", None)
    env.pop("NORI_SURFACE_ID", None)
    env.pop("NORI_TAB_ID", None)
    if extra_env:
    ...
    if proc.returncode != 0:
        ...
        raise noriError(f"CLI failed ({' '.join(cmd)}): {merged}")
    return proc.stdout
...
def _run_cli_json(cli: str, args: list[str], *, extra_env: dict[str, str] | None = None) -> dict:
    ...
    try:
        ...
        raise noriError(f"Invalid JSON output for {' '.join(args)}: {output!r} ({exc})")
...
def _read_any_terminal_text(client: nori, workspace_id: str, timeout: float = 8.0) -> str | None:
...
def _resolve_workspace_id_from_payload(client: nori, payload: dict) -> str:
...
def _find_workspace_row(client: nori, workspace_id: str) -> dict | None:
    listed = client._call("workspace.list", {}) or {}
    for row in listed.get("workspaces") or []:
...
def main() -> int:
    ...
    _must("nori ssh" in help_text, "ssh --help output should include command header")
    ...
    with nori(SOCKET_PATH) as client:
        try:
            ...
            _must(
                ...
                f"nori ssh should select the new remote workspace: selected={selected_workspace_id} row={listed_row} payload={payload}",
            )
            ...
            _must(remote_relay_port is not None, f"nori ssh output missing remote_relay_port: {payload}")
            remote_socket_addr = f"127.0.0.1:{int(remote_relay_port)}"
            ssh_command = str(payload.get("ssh_command") or "")
            _must(bool(ssh_command), f"nori ssh output missing ssh_command: {payload}")
            _must(
                ssh_command.startswith("ssh "),
                f"nori ssh should emit plain ssh command text (env is passed via workspace.create initial_env): {ssh_command!r}",
            )
            ssh_terminal_command = str(payload.get("ssh_terminal_command") or "")
            _must(bool(ssh_terminal_command), f"nori ssh output missing ssh_terminal_command: {payload}")
            _must(
                ssh_terminal_command.startswith("ssh "),
                f"nori ssh should emit a terminal command that launches the remote bootstrap over ssh: {ssh_terminal_command!r}",
            )
            ssh_startup_command = str(payload.get("ssh_startup_command") or "")
            _must(
                "nori-ssh-startup-" in ssh_startup_command and ssh_startup_command.rstrip("'").endswith(".sh"),
                f"nori ssh should launch a generated startup script that preserves shell integration and cleanup: {ssh_startup_command!r}",
            )
            _must(os.path.isfile(ssh_startup_command), f"nori ssh startup script should exist on disk: {ssh_startup_command!r}")
            ssh_startup_script = Path(ssh_startup_command).read_text()
            _must(
                f"nori_bootstrap_path=\"$HOME/.nori/relay/{int(remote_relay_port)}.bootstrap.sh\"" in ssh_startup_script,
                f"nori ssh startup script should stage the remote bootstrap payload under ~/.nori/relay: {ssh_startup_command!r}",
            )
            _must(
                "cat > \"$nori_bootstrap_path\"" in ssh_startup_script,
                f"nori ssh startup script should upload the remote bootstrap over stdin before opening the interactive ssh session: {ssh_startup_command!r}",
            )
            _must(
                "/bin/sh -c " in ssh_startup_script and "/bin/sh -lc " not in ssh_startup_script,
                f"nori ssh startup script should install the bootstrap through a non-login POSIX shell: {ssh_startup_command!r}",
            )
            _must(
                f"/bin/sh \"$HOME/.nori/relay/{int(remote_relay_port)}.bootstrap.sh\"" in ssh_startup_script,
                f"nori ssh startup script should execute the staged remote bootstrap file through /bin/sh: {ssh_startup_command!r}",
            )
            _must(
                "export NORI_BOOTSTRAP_TTY=\"$nori_bootstrap_tty\"" in ssh_startup_script,
                f"nori ssh startup script should preserve the bootstrap tty for relay warmup: {ssh_startup_command!r}",
            )
            bootstrap_b64_match = re.search(r"^nori_remote_bootstrap_b64=([A-Za-z0-9+/=]+)$", ssh_startup_script, re.MULTILINE)
            _must(
                bootstrap_b64_match is not None,
                f"nori ssh startup script should embed the remote bootstrap payload: {ssh_startup_command!r}",
            )
            ...
            _must(
                ...
                f"nori ssh should pass shell niceties via ssh_env_overrides: {payload}",
            )
            ...
            _must("ControlPath=/tmp/nori-ssh-" in ssh_command, f"ssh command should use shared control path template: {ssh_command!r}")
            _must(
                "RemoteCommand=" not in ssh_command,
                f"nori ssh should keep the plain ssh_command separate from the terminal bootstrap wrapper: {ssh_command!r}",
            )
            _must(
                "nori_tmp=$(mktemp " in ssh_terminal_command,
                f"nori ssh should stage a temp startup script through ssh_terminal_command: {ssh_terminal_command!r}",
            )
            _must(
                "export NORI_BOOTSTRAP_TTY=\"$nori_bootstrap_tty\"" in ssh_terminal_command,
                f"nori ssh should capture the bootstrap tty before handing off to the temp startup script: {ssh_terminal_command!r}",
            )
            _must(
                f"\"$HOME/.nori/relay/{int(remote_relay_port)}.tty\"" in ssh_terminal_command,
                f"nori ssh should persist the bootstrap tty beside the relay metadata: {ssh_terminal_command!r}",
            )
            _must(
                f"export PATH=\"$HOME/.nori/bin:$PATH\"" in remote_bootstrap,
                f"nori ssh should still prepend the remote nori wrapper path in the remote bootstrap: {remote_bootstrap!r}",
            )
            _must(
                f"export NORI_SOCKET_PATH=127.0.0.1:{int(remote_relay_port)}" in remote_bootstrap,
                f"nori ssh should still pin the relay socket path in the remote bootstrap: {remote_bootstrap!r}",
            )
            _must(
                "export NORI_WORKSPACE_ID='__NORI_WORKSPACE_ID__'" in remote_bootstrap,
                f"nori ssh should export the remote workspace id into the bootstrap shell: {remote_bootstrap!r}",
            )
            _must(
                "export NORI_TAB_ID='__NORI_WORKSPACE_ID__'" in remote_bootstrap,
                f"nori ssh should keep NORI_TAB_ID aligned with the workspace id for shell integration: {remote_bootstrap!r}",
            )
            _must(
                "export NORI_SURFACE_ID='__NORI_SURFACE_ID__'" in remote_bootstrap,
                f"nori ssh should export the remote surface id into the bootstrap shell: {remote_bootstrap!r}",
            )
            _must(
                "export NORI_PANEL_ID='__NORI_SURFACE_ID__'" in remote_bootstrap,
                f"nori ssh should keep NORI_PANEL_ID aligned with the surface id for shell integration: {remote_bootstrap!r}",
            )
            _must(
                "case \"${NORI_LOGIN_SHELL##*/}\" in" in remote_bootstrap,
                f"nori ssh should still branch on the user's login shell when possible: {remote_bootstrap!r}",
            )
            _must(
                "cat > \"$nori_shell_dir/.zshrc\"" in remote_bootstrap,
                f"nori ssh should install a post-rc zsh wrapper so the remote nori wrapper stays first on PATH: {remote_bootstrap!r}",
            )
            _must(
                '"$nori_relay_cli" rpc surface.report_tty "$nori_relay_report_tty"' in remote_bootstrap,
                f"nori ssh should synchronously report the relay TTY during bootstrap: {remote_bootstrap!r}",
            )
            _must(
                'nori_relay_tty="${NORI_BOOTSTRAP_TTY:-}"' in remote_bootstrap,
                f"nori ssh should reuse the bootstrap tty when warming the relay-backed shell integration: {remote_bootstrap!r}",
            )
            _must(
                '"$nori_relay_cli" rpc surface.ports_kick "$nori_relay_ports_kick"' in remote_bootstrap,
                f"nori ssh should trigger an immediate relay-backed port scan during bootstrap: {remote_bootstrap!r}",
            )
            _must(
                "exec \"$NORI_LOGIN_SHELL\" --rcfile \"$nori_shell_dir/.bashrc\" -i" in remote_bootstrap,
                f"nori ssh should still support bash login shells with a post-rc wrapper file: {remote_bootstrap!r}",
            )
            _must(
                "exec \"$NORI_LOGIN_SHELL\" -i" in remote_bootstrap,
                f"nori ssh should still hand off to the user's interactive login shell when possible: {remote_bootstrap!r}",
            )
            ...
            # Regression: nori ssh should launch through initial_command, not visibly type a giant command into the shell.
            terminal_text = _read_any_terminal_text(client, workspace_id)
            if terminal_text is not None:
                _must("ControlPersist=600" not in terminal_text, f"nori ssh should not inject raw ssh command text: {terminal_text!r}")
                _must("GHOSTTY_SHELL_FEATURES=" not in terminal_text, f"nori ssh should not inject env assignment text: {terminal_text!r}")
                _must("BASH_EXECUTION_STRING=set" not in terminal_text, f"nori ssh should not print the remote shell environment dump on connect: {terminal_text!r}")
            ...
            while time.time() < deadline_daemon:
                ...
                raise noriError(f"unreachable host should fail fast instead of hanging in connecting: {last_status}")
            ...
            try:
                ...
                raise noriError("workspace.remote.reconnect should fail when remote config was cleared")
            except noriError as exc:
                text = str(exc).lower()
            ...
            _must(bool(workspace_id_without_name), f"nori ssh without --name should still create workspace: {payload2}")
            _must(
                "ControlPath=/tmp/nori-ssh-" in ssh_command_without_name,
                f"nori ssh without --name should still include control path defaults: {ssh_command_without_name!r}",
            )
            _must(
                _extract_control_path(ssh_command) != _extract_control_path(ssh_command_without_name),
                f"distinct nori ssh workspaces should get distinct control paths: {ssh_command!r} vs {ssh_command_without_name!r}",
            )
            row2 = None
            ...
            _must(
                ...
                f"nori ssh with StrictHostKeyChecking override should create workspace: {payload_strict_override}",
            )
            ...
            payload_case_override = _run_cli_json(
                ...
                [
                    ...
                    "controlpersist=0",
                    "--ssh-option",
                    "controlpath=/tmp/nori-ssh-%C-custom",
                ],
            )
            ...
            _must(
                ...
                f"nori ssh with lowercase SSH option overrides should create workspace: {payload_case_override}",
            )
            ...
            _must(
                "controlpath=/tmp/nori-ssh-%c-custom" in ssh_command_case_override_lower,
            ...
            _must(
                ...
                f"nori ssh should merge existing shell features when present: {payload3!r}",
            )
    ...
    print("PASS: nori ssh marks workspace as remote, exposes remote metadata, and does not require --name")
    return 0
