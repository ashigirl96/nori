#!/usr/bin/env python3
"""Docker integration: remote SSH proxy endpoint via `nori ssh`."""
...
REMOTE_HTTP_PORT = int(os.environ.get("NORI_SSH_TEST_REMOTE_HTTP_PORT", "43173"))
REMOTE_WS_PORT = int(os.environ.get("NORI_SSH_TEST_REMOTE_WS_PORT", "43174"))
MAX_REMOTE_DAEMON_SIZE_BYTES = int(os.environ.get("NORI_SSH_TEST_MAX_DAEMON_SIZE_BYTES", "15000000"))
...
def _curl_via_socks(proxy_port: int, target_url: str) -> str:
    if shutil.which("curl") is None:
        raise noriError("curl is required for SOCKS proxy verification")
    proc = _run(
        [
            "curl",
            "--silent",
    ...
    if proc.returncode != 0:
        merged = f"{proc.stdout}\n{proc.stderr}".strip()
        raise noriError(f"curl via SOCKS proxy failed: {merged}")
    return proc.stdout
...
def _recv_exact(sock: socket.socket, n: int) -> bytes:
    ...
    while len(out) < n:
        ...
        if not chunk:
            raise noriError("unexpected EOF while reading socket")
        out.extend(chunk)
    return bytes(out)
...
def _recv_until(sock: socket.socket, marker: bytes, limit: int = 16384) -> bytes:
    ...
    while marker not in out:
        ...
        if not chunk:
            raise noriError("unexpected EOF while reading response headers")
        out.extend(chunk)
        if len(out) > limit:
            raise noriError("response headers too large")
    return bytes(out)
...
def _read_socks5_connect_reply(sock: socket.socket) -> None:
    ...
    if len(head) != 4 or head[0] != 0x05:
        raise noriError(f"invalid SOCKS5 reply: {head!r}")
    if head[1] != 0x00:
        raise noriError(f"SOCKS5 connect failed with status=0x{head[1]:02x}")
    ...
    if atyp == 0x01:
        ...
        raise noriError(f"invalid SOCKS5 atyp in reply: 0x{atyp:02x}")
...
def _read_http_response_from_connected_socket(sock: socket.socket) -> str:
    ...
    if "200" not in status_line:
        raise noriError(f"HTTP over SOCKS tunnel failed: {status_line!r}")
...
def _socks5_connect(proxy_host: str, proxy_port: int, target_host: str, target_port: int) -> socket.socket:
    ...
    if greeting != b"\x05\x00":
        sock.close()
        raise noriError(f"SOCKS5 greeting failed: {greeting!r}")
    ...
    try:
        ...
        if len(host_encoded) > 255:
            sock.close()
            raise noriError("target host too long for SOCKS5 domain form")
        atyp = b"\x03"  # domain
...
def _socks5_http_get_pipelined(proxy_host: str, proxy_port: int, target_host: str, target_port: int) -> str:
    ...
    try:
        ...
        if greeting_reply != b"\x05\x00":
            raise noriError(f"SOCKS5 greeting failed: {greeting_reply!r}")
        _read_socks5_connect_reply(sock)
        return _read_http_response_from_connected_socket(sock)
    finally:
        try:
...
def _http_connect_tunnel(proxy_host: str, proxy_port: int, target_host: str, target_port: int) -> socket.socket:
    ...
    if "200" not in status_line:
        sock.close()
        raise noriError(f"HTTP CONNECT tunnel failed: {status_line!r}")
    return sock
...
def _read_server_text_frame(sock: socket.socket) -> str:
    ...
    if opcode != 0x1:
        raise noriError(f"Expected websocket text frame opcode=0x1, got opcode=0x{opcode:x}")
    try:
        return payload.decode("utf-8")
    except Exception as exc:  # noqa: BLE001
        raise noriError(f"WebSocket response payload is not valid UTF-8: {exc}")
...
def _websocket_echo_on_connected_socket(sock: socket.socket, ws_host: str, ws_port: int, message: str, path_label: str) -> str:
    ...
    if "101" not in status_line:
        raise noriError(f"WebSocket handshake failed over {path_label}: {status_line!r}")
    ...
    if lowered_headers.get("sec-websocket-accept", "") != expected_accept:
        raise noriError(f"WebSocket handshake over {path_label} returned invalid Sec-WebSocket-Accept")
...
def _extract_daemon_version_platform(remote_path: str) -> tuple[str, str]:
    ...
    try:
        marker_index = parts.index("norid-remote")
    except ValueError as exc:
        raise noriError(f"remote daemon path missing norid-remote marker: {remote_path!r}") from exc
    ...
    _must(binary_name == "norid-remote", f"unexpected daemon binary name in remote path: {remote_path!r}")
...
def _local_cached_daemon_binary(version: str, platform: str) -> Path:
    return Path(tempfile.gettempdir()) / "nori-remote-daemon-build" / version / platform / "norid-remote"
...
def _wait_connected_proxy_port(client: nori, workspace_id: str, timeout: float = 45.0) -> tuple[dict, int]:
    deadline = time.time() + timeout
    last_status = {}
    ...
    while time.time() < deadline:
        ...
        time.sleep(0.5)
    raise noriError(f"Remote proxy did not converge to connected state: {last_status}")
...
def main() -> int:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="nori-ssh-docker-"))
    image_tag = f"nori-ssh-test:{secrets.token_hex(4)}"
    container_name = f"nori-ssh-test-{secrets.token_hex(4)}"
    workspace_id = ""
    workspace_id_shared = ""
    ...
    try:
        ...
        fresh_check = _ssh_run(
            ...
            host_ssh_port,
            key_path,
            "test ! -e \"$HOME/.nori/bin/norid-remote\" && echo fresh",
            check=True,
        )
        _must("fresh" in fresh_check.stdout, "Fresh container should not have preinstalled norid-remote")
        ...
        with nori(SOCKET_PATH) as client:
            ...
            while time.time() < deadline_http:
                try:
                    ...
                    time.sleep(0.5)
                    continue
                if "nori-ssh-forward-ok" in body:
                    break
                time.sleep(0.3)

            _must("nori-ssh-forward-ok" in body, f"Forwarded HTTP endpoint returned unexpected body: {body[:120]!r}")
            pipelined_body = _socks5_http_get_pipelined("127.0.0.1", proxy_port, "127.0.0.1", REMOTE_HTTP_PORT)
            _must(
                "nori-ssh-forward-ok" in pipelined_body,
                f"SOCKS pipelined greeting/connect+payload path returned unexpected body: {pipelined_body[:120]!r}",
            )

            ws_message = "nori-ws-over-socks-ok"
            ...
            ws_connect_message = "nori-ws-over-connect-ok"
            ...
            if not workspace_id_shared and workspace_ref_shared.startswith("workspace:"):
                ...
                for row in listed_shared.get("workspaces") or []:
                    if str(row.get("ref") or "") == workspace_ref_shared:
                        ...
                        break
            _must(bool(workspace_id_shared), f"nori ssh output missing workspace_id for shared transport test: {payload_shared}")
        ...
        print(
            ...
            f"uploaded norid-remote size={binary_size_bytes} bytes, version={daemon_version}, platform={daemon_platform}"
        )
        return 0
