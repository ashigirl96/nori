...
def _run_cli_json(cli: str, args: list[str]) -> dict:
    ...
    try:
        ...
        raise noriError(f"Invalid JSON output for {' '.join(args)}: {proc.stdout!r} ({exc})") from exc
...
def _wait_remote_ready(client: nori, workspace_id: str, timeout: float = 45.0) -> dict:
    deadline = time.time() + timeout
    last_status = {}
    while time.time() < deadline:
...
def _workspace_row(client: nori, workspace_id: str) -> dict:
    ...
    for row in payload.get("workspaces") or []:
        if str(row.get("id") or "") == workspace_id:
            return row
    raise noriError(f"workspace {workspace_id} missing from workspace.list payload: {payload}")
...
def _wait_surface_tty(client: nori, workspace_id: str, surface_id: str, timeout: float = 20.0) -> str:
    ...
    if last_error is not None:
        raise noriError(f"Timed out waiting for surface tty after terminal lookup retries: {last_error}")
...
def _wait_for_remote_port(client: nori, workspace_id: str, port: int, timeout: float = 15.0) -> tuple[dict, dict]:
    deadline = time.time() + timeout
    last_status = {}
    last_row = {}
    while time.time() < deadline:
...
def main() -> int:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="nori-ssh-port-detection-"))
    image_tag = f"nori-ssh-test:{secrets.token_hex(4)}"
    container_name = f"nori-ssh-port-detect-{secrets.token_hex(4)}"
    workspace_id = ""
    surface_id = ""
