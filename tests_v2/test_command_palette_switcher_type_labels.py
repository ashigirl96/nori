...
def _open_switcher(client: nori, window_id: str) -> None:
...
def main() -> int:
    with nori(SOCKET_PATH) as client:
        ...
        if not rows:
            raise noriError("switcher returned no rows for token query")
        ...
        if not workspace_rows:
            raise noriError(f"expected workspace rows for switcher query: rows={rows}")
        if not surface_rows:
            raise noriError(f"expected surface rows for switcher query: rows={rows}")

        bad_workspace = [row for row in workspace_rows if str((row or {}).get("trailing_label") or "") != "Workspace"]
        if bad_workspace:
            raise noriError(f"workspace rows missing 'Workspace' trailing label: {bad_workspace}")

        bad_surface = [row for row in surface_rows if str((row or {}).get("trailing_label") or "") != "Surface"]
        if bad_surface:
            raise noriError(f"surface rows missing 'Surface' trailing label: {bad_surface}")
