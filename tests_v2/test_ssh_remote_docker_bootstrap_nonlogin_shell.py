...
def _wait_for_remote_connected(client: nori, workspace_id: str, timeout: float = 45.0) -> dict:
    deadline = time.time() + timeout
    last_status: dict = {}
    while time.time() < deadline:
        ...
        if (
            ...
            return last_status
        time.sleep(0.5)
    raise noriError(f"Remote did not converge to connected/ready under slow login profile: {last_status}")
...
def _wait_for_heartbeat_advance(client: nori, workspace_id: str, minimum_count: int, timeout: float = 20.0) -> dict:
    deadline = time.time() + timeout
    last_status: dict = {}
    while time.time() < deadline:
        last_status = client._call("workspace.remote.status", {"workspace_id": workspace_id}) or {}
...
def main() -> int:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="nori-ssh-bootstrap-nonlogin-"))
    image_tag = f"nori-ssh-test:{secrets.token_hex(4)}"
    container_name = f"nori-ssh-bootstrap-nonlogin-{secrets.token_hex(4)}"
    workspace_id = ""
