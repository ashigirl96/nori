...
def _wait_for_text(c: nori, workspace_id: str, needle: str, timeout_s: float = 8.0) -> str:
    ...
    while time.time() < deadline:
        ...
        if needle in last_text:
            return last_text
        time.sleep(0.1)
    raise noriError(f"Timed out waiting for {needle!r} in panel text: {last_text!r}")
    ...
    with nori(SOCKET_PATH) as c:
        ...
        try:
            ...
            payload = c._call(
                "workspace.create",
                {
                    "initial_env": {"NORI_INITIAL_ENV_TOKEN": token},
                },
            ) or {}
            ...
            c.send("printf 'NORI_ENV_CHECK=%s\\n' \"$NORI_INITIAL_ENV_TOKEN\"\\n")
            text = _wait_for_text(c, created_workspace, f"NORI_ENV_CHECK={token}")
            _must(
                f"NORI_ENV_CHECK={token}" in text,
