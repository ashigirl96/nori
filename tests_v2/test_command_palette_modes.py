...
def _palette_input_selection(client: nori, window_id: str) -> dict:
...
def main() -> int:
    with nori(SOCKET_PATH) as client:
        ...
        if not switched_rows:
            raise noriError("switcher returned no rows for workspace query")
        top_id = str((switched_rows[0] or {}).get("command_id") or "")
        if not top_id.startswith("switcher."):
            raise noriError(f"expected switcher row on top for cmd+p query, got: {switched_rows[0]}")
        ...
        if not command_rows:
            raise noriError("commands mode returned no rows")
        top_command_id = str((command_rows[0] or {}).get("command_id") or "")
        if not top_command_id.startswith("palette."):
            raise noriError(f"expected command row in commands mode, got: {command_rows[0]}")
