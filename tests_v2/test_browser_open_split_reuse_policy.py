...
def _pane_count(c: nori, workspace_id: str) -> int:
    panes_payload = c._call("pane.list", {"workspace_id": workspace_id}) or {}
    panes = panes_payload.get("panes") or []
    return len(panes)


def _pane_for_surface(c: nori, workspace_id: str, surface_id: str) -> str:
    ...
    for row in payload.get("surfaces") or []:
        if str(row.get("id") or "") == surface_id:
            ...
            if pane:
                return pane
    raise noriError(f"Surface {surface_id} not found in workspace {workspace_id}: {payload}")
