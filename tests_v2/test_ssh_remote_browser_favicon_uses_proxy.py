...
def _resolve_workspace_id(client: nori, payload: dict, *, before_workspace_ids: set[str]) -> str:
    ...
    if workspace_ref.startswith("workspace:"):
        with nori(SOCKET_PATH) as lookup_client:
...
def _wait_remote_ready(client: nori, workspace_id: str, timeout_s: float = 65.0) -> dict:
    ...
    while time.time() < deadline:
        ...
        if (
            ...
            return last
        time.sleep(0.25)
    raise noriError(f"Remote did not reach connected+ready+proxy-ready state: {last}")
...
def _wait_surface_contains(client: nori, workspace_id: str, surface_id: str, token: str, timeout_s: float = 20.0) -> None:
    deadline = time.time() + timeout_s
    while time.time() < deadline:
...
def _browser_body_text(client: nori, surface_id: str) -> str:
    payload = client._call(
        "browser.eval",
        {
...
def _wait_browser_contains(client: nori, surface_id: str, token: str, timeout_s: float = 20.0) -> None:
    deadline = time.time() + timeout_s
    last_text = ""
    while time.time() < deadline:
        try:
        ...
        if token in last_text:
            return
        time.sleep(0.2)
    raise noriError(f"Timed out waiting for browser content token {token!r}; last body sample={last_text[:240]!r}")


def _browser_favicon_state(client: nori, surface_id: str) -> dict:
    return dict(client._call("debug.browser.favicon", {"surface_id": surface_id}) or {})


def _wait_browser_favicon(client: nori, surface_id: str, timeout_s: float = 20.0) -> dict:
    deadline = time.time() + timeout_s
    last = {}
    while time.time() < deadline:
        try:
        ...
        if bool(last.get("has_favicon")) and bool(str(last.get("png_base64") or "")):
            return last
        time.sleep(0.2)
    raise noriError(f"Timed out waiting for browser favicon state on {surface_id}: {last}")


def main() -> int:
    if not SSH_HOST:
        print("SKIP: set NORI_SSH_TEST_HOST to run remote favicon proxy regression")
        return 0
    ...
    page_token = f"NORI_REMOTE_FAVICON_PAGE_{stamp}"
    server_ready_token = f"NORI_REMOTE_FAVICON_READY_{stamp}"
    default_web_port = 23000 + (os.getpid() % 4000)
    ssh_web_port = int(os.environ.get("NORI_SSH_TEST_WEB_PORT", str(default_web_port)))
    ...
    server_script_path = f"/tmp/nori_remote_favicon_server_{stamp}.py"
    server_log_path = f"/tmp/nori_remote_favicon_server_{stamp}.log"
    hit_file_path = f"/tmp/nori_remote_favicon_hit_{stamp}"

    try:
        with nori(SOCKET_PATH) as setup_client:
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        body = (
            ...
            "<link rel=\\"icon\\" href=\\"/favicon.ico?via=nori\\">"
