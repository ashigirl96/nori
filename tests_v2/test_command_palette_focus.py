...
def main() -> int:
    token = "NORI_PALETTE_FOCUS_PROBE_9412"
    restore_token = "NORI_PALETTE_RESTORE_PROBE_7731"
    ...
    with nori(SOCKET_PATH) as client:
        ...
        if token in post_text and token not in pre_text:
            raise noriError("typed probe text leaked into terminal while palette is open")
        ...
        if restore_token not in restore_text:
            raise noriError("terminal did not receive typing after closing command palette")
