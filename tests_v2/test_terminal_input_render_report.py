...
Environment:
  NORI_SOCKET or NORI_SOCKET_PATH can override the socket path.
"""
...
SOCKET_PATH = os.environ.get("NORI_SOCKET") or os.environ.get("NORI_SOCKET_PATH") or "/tmp/nori-debug.sock"
...
def _focused_panel_id(c: nori) -> str:
    surfaces = c.list_surfaces()
    if not surfaces:
        raise noriError("Expected at least 1 surface")
    return next((sid for _i, sid, focused in surfaces if focused), surfaces[0][1])


def _snap_panel(c: nori, panel_id: str, label: str) -> Shot:
...
def _panel_sequence_blink_and_type(c: nori, panel_id: str, prefix: str, typed_char: str = "x") -> tuple[list[Shot], dict]:
    shots: list[Shot] = []
...
<head>
  <meta charset="utf-8" />
  <title>nori terminal input render report</title>
  <style>
    :root {{
      --bg: #0b0f14;
      --panel: #111826;
...
<body>
  <h1>nori terminal input render report</h1>
