#!/usr/bin/env python3
"""Regression: CLI commands are workspace-relative via NORI_WORKSPACE_ID.

Tests that when NORI_WORKSPACE_ID is set, CLI commands target that workspace
...
def _run_cli_json(cli: str, args: List[str], env_overrides: Optional[Dict[str, str]] = None) -> Any:
    ...
    try:
        ...
        raise noriError(f"Invalid JSON output: {output!r} ({exc})")


def test_list_panels_workspace_relative(c: nori, cli: str) -> None:
    ...
    payload_env = _run_cli_json(
        cli, ["list-panels"],
        env_overrides={"NORI_WORKSPACE_ID": ws_a["id"]}
    )
...
def test_list_panes_workspace_relative(c: nori, cli: str) -> None:
...
def test_send_workspace_relative(c: nori, cli: str) -> None:
    """send with NORI_WORKSPACE_ID env var targets that workspace's surface."""
    ws_result = c._call("workspace.list")
    workspaces = ws_result.get("workspaces", [])
    _must(len(workspaces) >= 1, "Need at least 1 workspace")
    ...
    output = _run_cli(
        cli, ["send", " "],
        env_overrides={"NORI_WORKSPACE_ID": ws["id"]}
    )
...
def test_send_with_explicit_workspace(c: nori, cli: str) -> None:
...
def test_v2_migrated_commands_output_refs(c: nori, cli: str) -> None:
...
def test_surface_health_workspace_relative(c: nori, cli: str) -> None:
...
def test_non_json_output_uses_refs(c: nori, cli: str) -> None:
