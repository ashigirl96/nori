...
def main() -> int:
    ...
    with nori(SOCKET_PATH) as c:
        ...
        token = f"NORI_READ_SCREEN_{int(time.time() * 1000)}"
        ...
        _must(token in cli_text, f"nori read-screen output missing token {token!r}: {cli_text!r}")

        cli_ws_only = _run_cli(cli, ["read-screen", "--workspace", ws_target])
        _must(token in cli_ws_only, f"nori read-screen --workspace output missing token {token!r}: {cli_ws_only!r}")

        cli_text_scrollback = _run_cli(cli, ["read-screen", "--workspace", ws_target, "--surface", surface_target, "--scrollback", "--lines", "80"])
        _must(token in cli_text_scrollback, f"nori read-screen --scrollback output missing token {token!r}: {cli_text_scrollback!r}")

        cli_json = _run_cli(cli, ["--json", "read-screen", "--workspace", ws_target, "--surface", surface_target])
        payload = json.loads(cli_json or "{}")
        _must(token in str(payload.get("text") or ""), f"nori --json read-screen missing token {token!r}: {payload}")
