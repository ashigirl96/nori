...
def _rename_surface(client: nori, surface_id: str, title: str) -> None:
    client._call(
        "surface.action",
        {
...
def _current_surface_id(client: nori, workspace_id: str) -> str:
...
def main() -> int:
    with nori(SOCKET_PATH) as client:
        ...
        if len(rows) < 2:
            raise noriError(f"expected at least two surfaces after split: {payload}")
        ...
        if not left_surface_id:
            raise noriError(f"failed to resolve left surface id: {payload}")
        ...
        if not result_rows:
            raise noriError("switcher returned no rows for renamed surface query")
