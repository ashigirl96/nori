...
def _find_pane_for_surface(c: nori, surface_id: str) -> str:
    for _pidx, pane_id, _count, _focused in c.list_panes():
        ids = _pane_surface_ids(c, pane_id)
        if surface_id in ids:
            return pane_id
    raise noriError(f"Surface not found in any pane: {surface_id}")
...
def main() -> int:
    with nori(SOCKET_PATH) as c:
        ...
        if len(panes) < 2:
            raise noriError(f"Expected >=2 panes, got {len(panes)}")
        dst_pane = next((pid for pid in panes if pid != src_pane), None)
        if not dst_pane:
            raise noriError("Failed to find destination pane")
        ...
        if s_move in after_src:
            raise noriError(f"Expected moved surface to leave source pane (src={src_pane}, ids={after_src})")
        if s_move not in after_dst:
            raise noriError(f"Expected moved surface in destination pane (dst={dst_pane}, ids={after_dst})")
        ...
        if len(after_dst) < 2:
            ...
            if extra not in after_dst:
                raise noriError("Failed to create extra destination surface for reorder test")
        ...
        if s_move not in reordered:
            raise noriError(f"Expected moved surface to remain in destination pane after reorder (ids={reordered})")
        if reordered[0] != s_move:
            raise noriError(f"Expected moved surface at front after reorder (ids={reordered})")
        ...
        if not ordered_ws:
            raise noriError("workspace.list returned empty after reorder")
        if ordered_ws[0] != ws2:
            raise noriError(f"Expected ws2 first after reorder (ordered={ordered_ws}, ws2={ws2})")
