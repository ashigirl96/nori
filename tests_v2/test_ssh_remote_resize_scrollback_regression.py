...
SOCKET_PATH = os.environ.get("NORI_SOCKET", "/tmp/nori.sock")
SSH_HOST = os.environ.get("NORI_SSH_TEST_HOST", "").strip()
SSH_PORT = os.environ.get("NORI_SSH_TEST_PORT", "").strip()
SSH_IDENTITY = os.environ.get("NORI_SSH_TEST_IDENTITY", "").strip()
SSH_OPTIONS_RAW = os.environ.get("NORI_SSH_TEST_OPTIONS", "").strip()
LS_ENTRY_COUNT = int(os.environ.get("NORI_SSH_TEST_LS_COUNT", "320"))
RESIZE_ITERATIONS = int(os.environ.get("NORI_SSH_TEST_RESIZE_ITERATIONS", "48"))
...
def _wait_remote_connected(client: nori, workspace_id: str, timeout_s: float = 45.0) -> None:
    deadline = time.time() + timeout_s
    last = {}
    while time.time() < deadline:
        ...
        if str(remote.get("state") or "") == "connected" and str(daemon.get("state") or "") == "ready":
            return
        time.sleep(0.25)
    raise noriError(f"Remote did not reach connected+ready state: {last}")


def _resolve_workspace_id(client: nori, payload: dict, *, before_workspace_ids: set[str]) -> str:
    ...
    raise noriError(f"Unable to resolve workspace_id from payload: {payload}")
...
def _surface_scrollback_text(client: nori, workspace_id: str, surface_id: str) -> str:
...
def _surface_scrollback_lines(client: nori, workspace_id: str, surface_id: str) -> list[str]:
...
def _valid_resize_directions(client: nori, workspace_id: str, pane_id: str) -> list[str]:
    valid: list[str] = []
...
def _choose_resize_pair(client: nori, workspace_id: str, pane_ids: list[str]) -> list[tuple[str, str]]:
    ...
    raise noriError(f"Could not find oscillating resize pair across panes: {by_pane}")


def main() -> int:
    if not SSH_HOST:
        print("SKIP: set NORI_SSH_TEST_HOST to run remote resize scrollback regression")
        return 0
    if LS_ENTRY_COUNT < 64:
        print("SKIP: NORI_SSH_TEST_LS_COUNT must be >= 64 for meaningful scrollback coverage")
        return 0
    ...
    try:
        with nori(SOCKET_PATH) as client:
            ...
            ls_entries = [f"NORI_REMOTE_RESIZE_LS_{stamp}_{index:04d}.txt" for index in range(1, LS_ENTRY_COUNT + 1)]
            ls_start = f"NORI_REMOTE_RESIZE_LS_START_{stamp}"
            ls_end = f"NORI_REMOTE_RESIZE_LS_END_{stamp}"

            ls_prefix = f"NORI_REMOTE_RESIZE_LS_{stamp}_"
            ls_script = (
                "tmpdir=$(mktemp -d); "
                f"echo {ls_start}; "
            ...
            post_token = f"NORI_REMOTE_RESIZE_POST_{secrets.token_hex(6)}"
        ...
        print(
            "PASS: nori ssh split+resize churn preserved large pre-resize scrollback "
