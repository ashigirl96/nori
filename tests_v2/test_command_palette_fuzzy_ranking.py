...
def _rename_input_selection(client: nori, window_id: str) -> dict:
    return client._call("debug.command_palette.rename_input.selection", {"window_id": window_id}) or {}


def _palette_results(client: nori, window_id: str, limit: int = 10) -> dict:
...
def main() -> int:
    with nori(SOCKET_PATH) as client:
        ...
        if not rows:
            raise noriError(f"palette returned no results for rename query: {payload}")
        ...
        if not retab_rows:
            raise noriError(f"palette returned no results for retab query: {retab_payload}")
