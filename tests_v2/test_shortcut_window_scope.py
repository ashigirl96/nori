...
def _sidebar_visible(client: nori, window_id: str) -> bool:
    payload = client._call("debug.sidebar.visible", {"window_id": window_id}) or {}
    return bool(payload.get("visible"))


def _surface_count(client: nori, workspace_id: str) -> int:
...
def main() -> int:
    with nori(SOCKET_PATH) as client:
        ...
        if b_after != b_before:
            raise noriError("Cmd+B in window A incorrectly toggled sidebar in window B")
        ...
        if _sidebar_visible(client, window_a) != a_after:
            raise noriError("Cmd+B in window B incorrectly toggled sidebar in window A")
        ...
        if count_b_after != count_b_before:
            raise noriError("Cmd+T in window A incorrectly created a surface in window B")
