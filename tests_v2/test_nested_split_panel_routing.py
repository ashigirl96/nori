...
def _wait_for_terminal_focus(c: nori, panel_id: str, timeout_s: float = 4.0) -> None:
...
def _baseline_all(c: nori, panel_ids: list[str], label: str) -> None:
    for pid in panel_ids:
        c.panel_snapshot(pid, label=f"{label}_base_{pid[:6]}")


def _after_all(c: nori, panel_ids: list[str], label: str) -> dict[str, int]:
    diffs: dict[str, int] = {}
    for pid in panel_ids:
...
def _assert_routing(diffs: dict[str, int], target: str, *, min_changed: int = 250, ratio: float = 3.0) -> None:
    ...
    if tgt is None:
        raise noriError(f"missing diff for target {target}")
    # -1 means first diff or size mismatch; treat as failure here.
    if tgt < min_changed:
        raise noriError(f"target panel did not change enough (changed_pixels={tgt}): diffs={diffs}")
...
def main() -> int:
    with nori(SOCKET_PATH) as c:
        ...
        if not surfaces0:
            raise noriError("expected initial surface")
        ...
        for pid in panel_ids:
            c.send_surface(pid, f"echo NORI_READY_{pid[:6]}\n")
        time.sleep(0.6)
        ...
        for i, target in enumerate(panel_ids):
            marker = f"NORI_ROUTE_{i}_{target[:6]}"
            ...
            if marker not in text:
                raise noriError(f"marker missing from read_terminal_text for {target}: {marker}")
