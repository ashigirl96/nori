...
def _wait_remote_ready(client: nori, workspace_id: str, timeout_s: float = 60.0) -> dict:
    deadline = time.time() + timeout_s
    last = {}
    while time.time() < deadline:
...
def _wait_surface_contains(client: nori, workspace_id: str, surface_id: str, token: str, timeout_s: float = 20.0) -> None:
    ...
    while time.time() < deadline:
        if token in _surface_scrollback_text(client, workspace_id, surface_id):
            return
        time.sleep(0.2)
    raise noriError(f"Timed out waiting for remote terminal token: {token}")
...
def _assert_browser_does_not_contain(client: nori, surface_id: str, token: str, sample_window_s: float = 6.0) -> str:
...
def main() -> int:
    if not SSH_HOST:
        print("SKIP: set NORI_SSH_TEST_HOST to run remote browser move/proxy regression")
        return 0
    ...
    marker_file = f"NORI_REMOTE_PROXY_MOVE_{stamp}.txt"
    marker_body = f"NORI_REMOTE_PROXY_BODY_{stamp}"
    ready_token = f"NORI_HTTP_READY_{stamp}"
    ...
    try:
        with nori(SOCKET_PATH) as client:
            ...
            server_script = (
                ...
                f"python3 -m http.server {ssh_web_port} --directory /tmp >/tmp/nori-remote-browser-proxy-{stamp}.log 2>&1 & "
