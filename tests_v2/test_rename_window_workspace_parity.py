...
def _run_cli(cli: str, args: List[str], env_overrides: Optional[Dict[str, str]] = None) -> str:
    env = dict(os.environ)
    # Keep this test deterministic when running from inside another nori shell.
...
def _workspace_title(c: nori, workspace_id: str) -> str:
    payload = c._call("workspace.list") or {}
    for row in payload.get("workspaces") or []:
        if str(row.get("id") or "") == workspace_id:
            return str(row.get("title") or "")
    raise noriError(f"workspace.list missing workspace {workspace_id}: {payload}")
...
def main() -> int:
    ...
    with nori(SOCKET_PATH) as c:
        ...
        _must(_workspace_title(c, ws_id) == cli_title, "nori rename-workspace did not update workspace title")

        alias_title = f"tmux alias {stamp}"
        _run_cli(cli, ["rename-window", "--workspace", ws_id, alias_title])
        _must(_workspace_title(c, ws_id) == alias_title, "nori rename-window did not update workspace title")

        current_title = f"tmux current {stamp}"
        _run_cli(cli, ["rename-window", current_title])
        _must(
            _workspace_title(c, ws_id) == current_title,
            "nori rename-window without --workspace should target current workspace",
        )
        ...
        _run_cli(
            ...
            env_overrides={"NORI_WORKSPACE_ID": ws_id},
        )
        _must(
            _workspace_title(c, ws_id) == env_title,
            "nori rename-workspace should default to NORI_WORKSPACE_ID",
        )
