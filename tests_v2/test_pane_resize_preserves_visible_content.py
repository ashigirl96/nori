...
def _run_once(socket_path: str) -> int:
    ...
    try:
        with nori(socket_path) as client:
            ...
            resize_lines = [f"NORI_LOCAL_RESIZE_LINE_{stamp}_{index:02d}" for index in range(1, 33)]
            clear_and_draw = (
                "clear; "
                f"for i in $(seq 1 {len(resize_lines)}); do "
                "n=$(printf '%02d' \"$i\"); "
                f"echo NORI_LOCAL_RESIZE_LINE_{stamp}_$n; "
                "done"
            )
            ...
            post_token = f"NORI_LOCAL_RESIZE_POST_{stamp}"
