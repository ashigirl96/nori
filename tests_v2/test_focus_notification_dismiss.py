...
def wait_for_notification(client: nori, surface_id: str, is_read: bool, timeout: float = 2.0) -> bool:
...
def surface_id_for_index(client: nori, index: int) -> str:
...
def ensure_two_surfaces(client: nori) -> None:
    surfaces = client.list_surfaces()
    if len(surfaces) < 2:
        client.new_split("right")
        time.sleep(0.2)

def first_two_terminal_indices(client: nori) -> tuple[int, int]:
