...
def _wait_focused_index(client: nori, index: int, timeout: float = 4.0) -> bool:
    start = time.time()
...
def _ensure_surfaces(client: nori, count: int) -> None:
    surfaces = client.list_surfaces()
...
def test_close_middle_keeps_index(client: nori) -> TestResult:
...
def test_close_last_selects_previous(client: nori) -> TestResult:
