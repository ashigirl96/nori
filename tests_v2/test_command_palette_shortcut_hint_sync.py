...
def _palette_results(client: nori, window_id: str, limit=12) -> dict:
    return client.command_palette_results(window_id=window_id, limit=limit)


def _open_palette_and_rows(client: nori, window_id: str, limit: int = 80) -> list:
    ...
    if not rows:
        raise noriError(f"command palette returned no rows: {payload}")
    return rows
...
def _assert_shortcut_hint(rows: list, command_id: str, expected_hint: str) -> None:
    ...
    if row is None:
        raise noriError(f"missing command palette row for {command_id!r}; rows={rows}")
