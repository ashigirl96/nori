...
def _expect_not_supported(c: nori, method: str, params: dict) -> None:
    try:
        ...
        if "not_supported" in text:
            return
        raise noriError(f"Expected not_supported for {method}, got: {text}")
    raise noriError(f"Expected not_supported for {method}, but call succeeded")
