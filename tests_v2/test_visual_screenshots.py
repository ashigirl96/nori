#!/usr/bin/env python3
"""
Visual Screenshot Tests for nori
...
def take_screenshot(client: nori, label: str) -> Optional[Screenshot]:
...
def capture_state(client: nori) -> str:
    try:
...
def stamp_terminals(client: nori, label: str) -> None:
    ...
    for h in terminal_surfaces:
        ...
        try:
            ...
            client.send_surface(idx, f"echo NORI_VIS {safe} surf={idx}\n")
        except Exception:
            pass


def capture(client: nori, label: str):
...
def reset_workspace(client: nori) -> nori:
...
def surface_count(client: nori) -> int:
    return len(client.list_surfaces())

def pane_count(client: nori) -> int:
...
def wait_surface_count(client: nori, expected: int, timeout: float = 3.0) -> bool:
    start = time.time()
...
def wait_url_contains(client: nori, panel_id: str, needle: str, timeout: float = 8.0) -> bool:
...
def cleanup_workspaces(client: nori):
...
def _verify_surface_responsive(client: nori, surface_idx: int, marker: Path,
                                retries: int = 3) -> bool:
...
def verify_views_in_window(client: nori, label: str = "", timeout: float = 5.0) -> Optional[str]:
...
def verify_all_responsive(client: nori, label: str = "") -> Optional[str]:
    ...
    for idx, h in enumerate(terminal_surfaces):
        surface_idx = h["index"]
        marker = Path(tempfile.gettempdir()) / f"nori_vis_{os.getpid()}_{idx}"
        try:
...
def test_a1_initial_state(client: nori) -> StateChange:
...
def test_a2_split_right(client: nori) -> StateChange:
...
def test_a3_split_down(client: nori) -> StateChange:
...
def _close_and_verify(client: nori, change: StateChange, close_idx: int,
...
def test_b4_close_right(client: nori) -> StateChange:
...
def test_b5_close_left(client: nori) -> StateChange:
...
def test_b6_close_bottom(client: nori) -> StateChange:
...
def test_b7_close_top(client: nori) -> StateChange:
...
def test_c8_3way_close_middle(client: nori) -> StateChange:
...
def test_c9_grid_close_topleft(client: nori) -> StateChange:
...
def test_c10_grid_close_bottomright(client: nori) -> StateChange:
...
def test_d11_nested_close_bottomright(client: nori) -> StateChange:
...
def test_d12_nested_close_top(client: nori) -> StateChange:
...
def test_d13_4pane_close_second(client: nori) -> StateChange:
...
def test_e14_browser_close_terminal(client: nori) -> StateChange:
...
def test_e15_browser_close_browser(client: nori) -> StateChange:
...
def test_f16_nested_tabs_close_first(client: nori) -> StateChange:
...
def test_g17_rapid_down_close_top(client: nori) -> StateChange:
...
def test_g18_rapid_right_close_left(client: nori) -> StateChange:
...
def test_g19_alternating_close_reverse(client: nori) -> StateChange:
...
def test_h20_workspace_switch_back(client: nori) -> StateChange:
...
def _create_browser_surface(client: nori, url: Optional[str] = None) -> str:
    return client.new_surface(panel_type="browser", url=url)


def test_i21_browser_drag_split_right_wait_load(client: nori) -> StateChange:
...
def test_i22_browser_drag_split_right_immediate(client: nori) -> StateChange:
...
def test_i23_browser_drag_split_right_webview_focused(client: nori) -> StateChange:
...
def test_i24_browser_drag_split_right_focus_bounce(client: nori) -> StateChange:
...
def test_i25_browser_drag_split_right_then_switch_panes(client: nori) -> StateChange:
...
def test_i26_browser_drag_split_right_initial_url(client: nori) -> StateChange:
...
def test_i27_browser_drag_split_right_after_reload(client: nori) -> StateChange:
...
def test_i28_browser_drag_split_right_double_drag(client: nori) -> StateChange:
...
<html>
<head>
    <title>nori Visual Test Report</title>
    <style>
        body {
...
def run_visual_tests():
    ...
    print("=" * 60)
    print(f"nori Visual Screenshot Tests ({len(test_fns)} scenarios)")
    print("=" * 60)
    print()
