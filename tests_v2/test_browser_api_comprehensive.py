...
def _wait_until(pred, timeout_s: float, label: str) -> None:
    ...
    if last_exc is not None:
        raise noriError(f"Timed out waiting for {label}: {last_exc}")
    raise noriError(f"Timed out waiting for {label}")
...
def _expect_error(label: str, fn, code_substr: str) -> None:
    try:
        ...
        if code_substr in text:
            return
        raise noriError(f"{label}: expected error containing {code_substr!r}, got: {text}")
    raise noriError(f"{label}: expected error containing {code_substr!r}, but call succeeded")
...
def _expect_error_contains(label: str, fn, *needles: str) -> None:
    try:
        ...
        if missing:
            raise noriError(f"{label}: missing expected substrings {missing!r} in error: {text}")
        return
    raise noriError(f"{label}: expected failure, but call succeeded")
...
def _wait_with_fallback(c: nori, surface_id: str, params: dict, pred, timeout_s: float, label: str) -> None:
...
<html>
  <head>
    <title>nori-browser-comprehensive-1</title>
    <style>
...
<!doctype html>
<html>
  <head><title>nori-browser-comprehensive-2</title></head>
  <body>
...
def main() -> int:
    ...
    with nori(SOCKET_PATH) as c:
        ...
        _wait_until(
            lambda: "nori-browser-comprehensive-1"
        ...
        c._call("browser.fill", {"surface_id": target, "selector": "#name", "text": "nori"})
        ...
        _must(str(_value(out_text)) == "nori", f"Expected status text to be nori: {out_text}")
        ...
        _must(str(_value(name_val)) == "nori-v2", f"Expected typed suffix in input value: {name_val}")
        ...
        _must("nori-browser-comprehensive-1" in snapshot_text, f"Expected snapshot text for page1: {snap}")
        ...
        _wait_until(
            lambda: "nori-browser-comprehensive-2"
            in str((c._call("browser.get.title", {"surface_id": target}) or {}).get("title") or ""),
            timeout_s=3.0,
        ...
        _wait_with_fallback(
            c,
            target,
            {"url_contains": "nori-browser-comprehensive-1", "timeout_ms": 4000},
            lambda: "nori-browser-comprehensive-1" in str((c._call("browser.url.get", {"surface_id": target}) or {}).get("url") or ""),
            timeout_s=5.0,
        ...
        _wait_with_fallback(
            c,
            target,
            {"url_contains": "nori-browser-comprehensive-2", "timeout_ms": 4000},
            lambda: "nori-browser-comprehensive-2" in str((c._call("browser.url.get", {"surface_id": target}) or {}).get("url") or ""),
            timeout_s=5.0,
