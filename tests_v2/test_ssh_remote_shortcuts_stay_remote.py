...
def _wait_remote_ready(client: nori, workspace_id: str, timeout_s: float = 45.0) -> None:
    deadline = time.time() + timeout_s
    last_status = {}
    while time.time() < deadline:
...
def _focused_surface_id(client: nori) -> str:
    ident = client.identify()
    focused = ident.get("focused") or {}
    surface_id = str(focused.get("surface_id") or "")
    if not surface_id:
        raise noriError(f"Missing focused surface in identify payload: {ident}")
    return surface_id


def _run_remote_shell_probe(client: nori, surface_id: str, probe_label: str) -> str:
    token = f"__NORI_REMOTE_SOCKET_{probe_label}_{secrets.token_hex(4)}__"
    client.send_surface(
        surface_id,
        (
            f"__nori_socket_path=\"${{NORI_SOCKET_PATH:-}}\"; "
            f"printf '{token}:%s:__NORI_REMOTE_SOCKET_END__\\n' \"$__nori_socket_path\"\n"
        ),
    )
    deadline = time.time() + 15.0
    last = ""
    pattern = re.compile(re.escape(token) + r":(.*?):__NORI_REMOTE_SOCKET_END__")
    while time.time() < deadline:
        ...
        if matches:
            for candidate in reversed(matches):
                ...
                if cleaned and cleaned != "%s":
                    return cleaned
        time.sleep(0.15)
    raise noriError(f"Timed out waiting for socket token {token!r}: {last[-1200:]!r}")


def _assert_remote_socket_path(client: nori, surface_id: str, shortcut_name: str) -> None:
    socket_path = _run_remote_shell_probe(client, surface_id, shortcut_name)
    _must(
        socket_path.startswith("127.0.0.1:"),
        f"{shortcut_name} should keep the new terminal on the ssh relay, got NORI_SOCKET_PATH={socket_path!r}",
    )


def _open_ssh_workspace(client: nori, cli: str, *, name: str) -> str:
...
def main() -> int:
    if not SSH_HOST:
        print("SKIP: set NORI_SSH_TEST_HOST to run ssh shortcut inheritance regression")
        return 0
