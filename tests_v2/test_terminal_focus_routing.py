...
This test validates:
  ...
  2) Text insertion via debug socket (`simulate_type`) lands in the expected terminal by writing
     $NORI_SURFACE_ID to a temp file.
"""
...
FOCUS_FILE = Path("/tmp/nori_focus_routing.txt")


def _focused_surface_id(c: nori) -> str:
    surfaces = c.list_surfaces()
    for _, sid, focused in surfaces:
        if focused:
            return sid
    raise noriError(f"No focused surface in list_surfaces: {surfaces}")
    ...
    while time.time() - start < timeout_s:
        if path.exists():
            ...
            if data:
                return data
        time.sleep(0.05)
    raise noriError(f"Timed out waiting for file content: {path}")


def _wait_for_terminal_focus(c: nori, panel_id: str, timeout_s: float = 6.0) -> None:
    start = time.time()
    while time.time() - start < timeout_s:
        if c.is_terminal_focused(panel_id):
            return
        time.sleep(0.05)
    raise noriError(f"Timed out waiting for terminal focus: {panel_id}")


def _focus_and_wait(c: nori, panel_id: str, *, total_timeout_s: float = 8.0) -> None:
    """
    ...
    raise noriError(f"Failed to focus terminal surface (panel_id={panel_id}): {last_err}")


def _assert_routed_to_surface(c: nori, expected_surface_id: str, panel_id: str) -> None:
    last_actual = "<empty>"
    for attempt in range(4):
        ...
        c.simulate_type(f"echo $NORI_SURFACE_ID > {FOCUS_FILE}")
...
def main() -> int:
    with nori(SOCKET_PATH) as c:
        ...
        surfaces = c.list_surfaces()
        if not surfaces:
            raise noriError("Expected at least one surface after new_workspace")
