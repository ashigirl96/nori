...
def main():
    with nori(SOCKET_PATH) as client:
        ...
        if not before_rows:
            raise noriError(f"no results for 'open': {before}")
        if str(before_rows[0].get("command_id") or "") != "palette.terminalOpenDirectory":
            raise noriError(f"unexpected top command for 'open': {before_rows[0]}")
        ...
        if not after_rows:
            raise noriError(f"no results for 'rename' after replacement: {after}")
        top_after = str(after_rows[0].get("command_id") or "")
        if top_after not in {"palette.renameWorkspace", "palette.renameTab"}:
            raise noriError(f"top result did not update to rename command after replacement: {after_rows[0]}")
