...
def _workspace_row(client: nori, workspace_id: str) -> dict:
    ...
    for row in rows:
        if str(row.get("id") or "") == workspace_id:
            return row
    raise noriError(f"workspace.list missing {workspace_id}: {rows}")


def _remote_session_count(client: nori, workspace_id: str) -> int:
    row = _workspace_row(client, workspace_id)
    remote = row.get("remote") or {}
    return int(remote.get("active_terminal_sessions") or 0)


def _run_surface_probe(client: nori, surface_id: str, command: str, token_prefix: str, timeout_s: float = 12.0) -> str:
    token = f"__NORI_{token_prefix}_{int(time.time() * 1000)}__"
    client.send_surface(
        surface_id,
        (
    ...
    while time.time() < deadline:
        ...
        if matches:
            return matches[-1]
        time.sleep(0.15)
    raise noriError(f"Timed out waiting for probe {token!r}: {last[-1200:]!r}")
...
def main() -> int:
    if not SSH_HOST:
        print("SKIP: set NORI_SSH_TEST_HOST to run ssh last-surface remote state regression")
        return 0
        ...
        with nori(SOCKET_PATH) as client:
            ...
            for idx, surface_id in enumerate(local_surface_ids):
                socket_output = _run_surface_probe(
                    client,
                    surface_id,
                    r'''printf '%s' "${NORI_SOCKET_PATH:-}"''',
                    f"SSH_LAST_SURFACE_SOCKET_{idx}",
                ).strip()
                _must(
                    not socket_output.startswith("127.0.0.1:"),
                    f"surface {surface_id} should be local after clearing remote state, got NORI_SOCKET_PATH={socket_output!r}",
                )
    finally:
        if workspace_id:
            try:
