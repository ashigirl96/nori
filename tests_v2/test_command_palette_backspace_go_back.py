    ...
    with nori(SOCKET_PATH) as client:
        ...
        try:
            ...
            if str(after_first.get("mode") or "") != "rename_input":
                raise noriError(f"palette exited rename mode too early after first backspace: {after_first}")
            ...
            if not _palette_visible(client, window_id):
                raise noriError("palette closed unexpectedly instead of navigating back to command list")
