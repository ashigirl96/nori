...
FUZZ_SEED = int(os.environ.get("NORI_SPLIT_FUZZ_SEED", "424242"))
FUZZ_STEPS = int(os.environ.get("NORI_SPLIT_FUZZ_STEPS", "1400"))
SAMPLES_PER_STEP = int(os.environ.get("NORI_SPLIT_FUZZ_SAMPLES", "4"))
SAMPLE_INTERVAL_S = float(os.environ.get("NORI_SPLIT_FUZZ_SAMPLE_INTERVAL_S", "0.0015"))
ACTION_JITTER_MAX_S = float(os.environ.get("NORI_SPLIT_FUZZ_ACTION_JITTER_MAX_S", "0.0035"))
BURST_MAX = int(os.environ.get("NORI_SPLIT_FUZZ_BURST_MAX", "3"))
MAX_PANES = int(os.environ.get("NORI_SPLIT_FUZZ_MAX_PANES", "10"))
EPSILON = float(os.environ.get("NORI_SPLIT_FUZZ_EPSILON", "0.0"))
TRACE_TAIL = int(os.environ.get("NORI_SPLIT_FUZZ_TRACE_TAIL", "40"))
ASSERT_NO_UNDERFLOW = os.environ.get("NORI_SPLIT_FUZZ_ASSERT_NO_UNDERFLOW", "0") == "1"
ASSERT_NO_EMPTY_PANEL = os.environ.get("NORI_SPLIT_FUZZ_ASSERT_NO_EMPTY_PANEL", "0") == "1"
...
def _warm_start_split(c: nori) -> dict:
    ...
    while time.time() < deadline:
        ...
        if _pane_count(payload) >= 2:
            return payload
        time.sleep(0.02)
    raise noriError(f"Timed out waiting for first split to appear: {last}")
...
def main() -> int:
    ...
    with nori(SOCKET_PATH) as c:
        ...
        if _pane_count(initial) < 2:
            raise noriError("Expected at least 2 panes after warm start split")
        ...
        if ASSERT_NO_UNDERFLOW and underflows != 0:
            raise noriError(f"bonsplit arranged-subview underflow observed during fuzz run: {underflows}")

        flashes = c.empty_panel_count()
        if ASSERT_NO_EMPTY_PANEL and flashes != 0:
            raise noriError(f"EmptyPanelView appeared during fuzz run (count={flashes})")
