...
def workspace_panes(client: nori, workspace_id: str) -> list[tuple[str, bool, int]]:
...
def focused_pane_id(client: nori, workspace_id: str) -> str:
    for pane_id, focused, _surface_count in workspace_panes(client, workspace_id):
        if focused:
            return pane_id
    raise noriError("No focused pane found")


def surface_scrollback_text(client: nori, workspace_id: str, surface_id: str) -> str:
...
def surface_scrollback_lines(client: nori, workspace_id: str, surface_id: str) -> list[str]:
    text = surface_scrollback_text(client, workspace_id, surface_id)
    return [clean_line(raw) for raw in text.splitlines()]


def scrollback_has_exact_line(client: nori, workspace_id: str, surface_id: str, token: str) -> bool:
    return token in surface_scrollback_lines(client, workspace_id, surface_id)


def wait_for_surface_command_roundtrip(client: nori, workspace_id: str, surface_id: str) -> None:
    for _attempt in range(1, 5):
        token = f"NORI_READY_{secrets.token_hex(4)}"
        ...
        try:
            ...
            time.sleep(0.1)
    raise noriError("Timed out waiting for surface command roundtrip")


def pick_resize_direction_for_pane(client: nori, pane_ids: list[str], target_pane: str) -> tuple[str, str]:
