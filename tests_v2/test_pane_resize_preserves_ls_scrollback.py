...
DEFAULT_SOCKET_PATHS = ["/tmp/nori-debug.sock", "/tmp/nori.sock"]
...
def _extract_segment_lines(
    ...
    *,
    ...
    if not saw_start:
        raise noriError(f"start marker not found in scrollback: {start_marker}")
    if require_end and not saw_end:
        raise noriError(f"end marker not found in scrollback: {end_marker}")
    return out


def _run_once(socket_path: str) -> int:
    workspace_id = ""
    fixture_dir = Path(tempfile.mkdtemp(prefix="nori-ls-resize-regression-"))
    try:
        with nori(socket_path) as client:
            ...
            start_marker = f"NORI_LS_SCROLLBACK_START_{secrets.token_hex(4)}"
            end_marker = f"NORI_LS_SCROLLBACK_END_{secrets.token_hex(4)}"
...
def main() -> int:
    env_socket = os.environ.get("NORI_SOCKET")
    if env_socket:
    ...
    if last_error is not None:
        raise last_error
    raise noriError("No socket candidates configured")
