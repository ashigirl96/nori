...
def _surface_row(c: nori, workspace_id: str, surface_id: str) -> dict:
    payload = c._call("surface.list", {"workspace_id": workspace_id}) or {}
    for row in payload.get("surfaces") or []:
        if str(row.get("id") or "") == surface_id:
            return row
    raise noriError(f"surface.list missing surface {surface_id} in workspace {workspace_id}: {payload}")


def _devtools_visible(c: nori, workspace_id: str, surface_id: str) -> bool:
    row = _surface_row(c, workspace_id, surface_id)
    return bool(row.get("developer_tools_visible"))


def _focus_browser_webview(c: nori, surface_id: str, timeout_s: float = 2.0) -> None:
    deadline = time.time() + timeout_s
    last_exc = None
    while time.time() < deadline:
        try:
            ...
            last_exc = exc
        time.sleep(0.05)
    raise noriError(f"Timed out waiting for browser webview focus: {last_exc}")
