...
def _focused_window_id(c: nori) -> str:
    ident = c.identify()
...
def main() -> int:
    with nori(SOCKET_PATH) as c:
        ...
        if not windows0:
            raise noriError("Expected at least one window from window.list")
        ...
        if w1 not in ids1:
            raise noriError(f"Expected original window id in window.list (w1={w1}, ids={sorted(ids1)})")
        if w2 not in ids1:
            raise noriError(f"Expected new window id in window.list (w2={w2}, ids={sorted(ids1)})")
        ...
        if len(before_ids) < 2:
            raise noriError(f"Expected >=2 surfaces before move, got {len(before_ids)} ({before_ids})")
        ...
        while time.time() - start < 6.0:
            ...
            raise noriError(f"Expected all moved surfaces to be in_window=true (health={health})")
        ...
        if ws not in w2_ids:
            raise noriError("Expected moved workspace to be present in destination window")
        ...
        if not isinstance(focused2, dict) or str(focused2.get("workspace_id")) != ws:
            raise noriError(f"Expected moved workspace to be selectable after move (focused={focused2})")

        after = c.list_surfaces(ws)
        after_ids = [sid for _, sid, _focused in after]
        if set(after_ids) != set(before_ids):
            raise noriError(f"Expected surface IDs to remain stable after move (before={before_ids}, after={after_ids})")
        ...
        if ws in w1_ids:
            raise noriError("Expected moved workspace to no longer be present in source window")
