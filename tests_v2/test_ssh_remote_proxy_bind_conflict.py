...
def _wait_for_proxy_conflict_status(client: nori, workspace_id: str, expected_local_proxy_port: int, timeout: float = 30.0) -> dict:
    deadline = time.time() + timeout
    last_status = {}
    while time.time() < deadline:
    ...
    raise noriError(f"Remote did not reach structured proxy_unavailable status for bind conflict: {last_status}")
...
def main() -> int:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="nori-ssh-proxy-conflict-"))
    image_tag = f"nori-ssh-test:{secrets.token_hex(4)}"
    container_name = f"nori-ssh-proxy-conflict-{secrets.token_hex(4)}"
    workspace_id = ""
