...
def _assert_stable_layout(payload: dict, *, expected_panes: int, min_wh: float = 80.0) -> None:
    ...
    if len(panes) != expected_panes:
        raise noriError(f"expected {expected_panes} panes, got {len(panes)}")

    selected_by_pane = _selected_panels_by_pane(payload)
    if len(selected_by_pane) < expected_panes:
        raise noriError(f"layout_debug missing selectedPanels (got {len(selected_by_pane)} for {expected_panes} panes)")
    ...
    for p in panes:
        ...
        if w < min_wh or h < min_wh:
            raise noriError(f"pane collapsed: paneId={pid} frame={frame}")

        row = selected_by_pane.get(pid)
        if not row:
            raise noriError(f"missing selectedPanels entry for paneId={pid}")

        panel_id = row.get("panelId")
        if not panel_id:
            raise noriError(f"missing panelId for paneId={pid}")

        if row.get("inWindow") is not True:
            raise noriError(f"panel not in window: paneId={pid} panelId={panel_id} inWindow={row.get('inWindow')}")

        if row.get("hidden") is True:
            raise noriError(f"panel hidden: paneId={pid} panelId={panel_id}")
        ...
        if vw < min_wh or vh < min_wh:
            raise noriError(f"panel viewFrame collapsed: paneId={pid} panelId={panel_id} viewFrame={view_frame}")


def _take_screenshot(c: nori, label: str) -> str:
...
def main() -> int:
    with nori(SOCKET_PATH) as c:
        ...
        if len(panes) < 2:
            raise noriError(f"expected >=2 panes after first split, got {len(panes)}")

        left_pane_id = str(panes[0].get("paneId"))
        right_pane_id = str(panes[-1].get("paneId"))

        if not left_pane_id or not right_pane_id:
            raise noriError(f"missing pane IDs: left={left_pane_id} right={right_pane_id}")
        ...
        raise noriError(f"nested split layout never stabilized: {last_err}; screenshot: {shot}; payload_keys={list((last_payload or {}).keys())}")
