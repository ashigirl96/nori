...
These tests use the nori socket interface to:
...
Requirements:
    - nori must be running with the socket controller enabled
"""
...
# Add the directory containing nori.py to the path
...
def ensure_focused_terminal(client: nori) -> None:
    """
...
def wait_for_terminal_in_window(client: nori, surface_idx: int, timeout: float = 5.0) -> bool:
...
def verify_terminal_responsive(client: nori, marker: Path, surface_idx: int = None, retries: int = 3) -> bool:
    """
...
def test_connection(client: nori) -> TestResult:
...
def test_initial_terminal_responsive(client: nori) -> TestResult:
    """Test that the initial terminal is responsive."""
    result = TestResult("Initial Terminal Responsive")
    marker = Path(tempfile.gettempdir()) / f"nori_init_{os.getpid()}"
...
def test_split_right_responsive(client: nori) -> TestResult:
    """Test that both terminals remain responsive after horizontal split."""
    result = TestResult("Split Right - Both Responsive")
    marker0 = Path(tempfile.gettempdir()) / f"nori_split0_{os.getpid()}"
    marker1 = Path(tempfile.gettempdir()) / f"nori_split1_{os.getpid()}"
...
def test_split_down_responsive(client: nori) -> TestResult:
    """Test that both terminals remain responsive after vertical split."""
    result = TestResult("Split Down - Both Responsive")
    marker0 = Path(tempfile.gettempdir()) / f"nori_splitv0_{os.getpid()}"
    marker1 = Path(tempfile.gettempdir()) / f"nori_splitv1_{os.getpid()}"
...
def test_focus_switching(client: nori) -> TestResult:
    """Test that focus switching between panes works correctly."""
    result = TestResult("Focus Switching")
    markers = [
        Path(tempfile.gettempdir()) / f"nori_focus{i}_{os.getpid()}"
        for i in range(3)
    ]
...
def test_split_ratio_50_50(client: nori) -> TestResult:
    """Test that splits create 50/50 pane ratios."""
    result = TestResult("Split Ratio 50/50")
    cols_file_0 = Path(tempfile.gettempdir()) / f"nori_cols0_{os.getpid()}"
    cols_file_1 = Path(tempfile.gettempdir()) / f"nori_cols1_{os.getpid()}"
...
def test_new_surfaces(client: nori) -> TestResult:
    """Test creating new surfaces in a pane."""
    result = TestResult("New Surfaces")
    markers = [
        Path(tempfile.gettempdir()) / f"nori_bonsplit{i}_{os.getpid()}"
        for i in range(3)
    ]
...
def test_pane_commands(client: nori) -> TestResult:
    """Test the new pane commands (list_panes, focus_pane)."""
    result = TestResult("Pane Commands")
    marker = Path(tempfile.gettempdir()) / f"nori_pane_{os.getpid()}"
...
def test_close_horizontal_split(client: nori) -> TestResult:
    """Test that closing one side of a horizontal split preserves the other terminal."""
    result = TestResult("Close Horizontal Split")
    marker0 = Path(tempfile.gettempdir()) / f"nori_close_h0_{os.getpid()}"
    marker1 = Path(tempfile.gettempdir()) / f"nori_close_h1_{os.getpid()}"
...
def test_close_vertical_split(client: nori) -> TestResult:
    """Test that closing one side of a vertical split preserves the other terminal."""
    result = TestResult("Close Vertical Split")
    marker0 = Path(tempfile.gettempdir()) / f"nori_close_v0_{os.getpid()}"
    marker1 = Path(tempfile.gettempdir()) / f"nori_close_v1_{os.getpid()}"
...
def test_close_first_pane_vertical_split(client: nori) -> TestResult:
    ...
    marker0 = Path(tempfile.gettempdir()) / f"nori_close_fv0_{os.getpid()}"
    marker1 = Path(tempfile.gettempdir()) / f"nori_close_fv1_{os.getpid()}"
...
def test_close_nested_splits(client: nori) -> TestResult:
    """Test closing splits in a nested configuration."""
    result = TestResult("Close Nested Splits")
    markers = [
        Path(tempfile.gettempdir()) / f"nori_nested_{i}_{os.getpid()}"
        for i in range(4)
    ]
...
def test_rapid_split_close_vertical(client: nori) -> TestResult:
    ...
    marker = Path(tempfile.gettempdir()) / f"nori_rapid_{os.getpid()}"
...
def test_rapid_split_close_first_pane(client: nori) -> TestResult:
    ...
    marker = Path(tempfile.gettempdir()) / f"nori_rapid_first_{os.getpid()}"
...
def run_tests():
    """Run all tests."""
    print("=" * 60)
    print("nori Tab Dragging E2E Tests")
    print("=" * 60)
    print()
    ...
    socket_path = nori.DEFAULT_SOCKET_PATH
    if not os.path.exists(socket_path):
        print(f"Error: Socket not found at {socket_path}")
        print("Please make sure nori is running.")
        return 1
