...
def _read_socks5_connect_reply(sock: socket.socket) -> None:
    ...
    if reply_atyp == 0x01:
        ...
        raise noriError(f"invalid SOCKS5 atyp in reply: 0x{reply_atyp:02x}")
...
def _start_container(image_tag: str, container_name: str, pubkey: str, host_ssh_port: int) -> None:
    for _ in range(20):
        ...
        time.sleep(0.5)
    merged = f"{proc.stdout}\n{proc.stderr}".strip()
    raise noriError(f"Failed to start ssh test container on fixed port {host_ssh_port}: {merged}")
...
def _wait_remote_connected(client: nori, workspace_id: str, timeout: float) -> dict:
    ...
    while time.time() < deadline:
        ...
        if str(remote.get("state") or "") == "connected" and proxy_port is not None:
            return last_status
        time.sleep(0.5)
    raise noriError(f"Remote did not reach connected+proxy-ready state: {last_status}")


def _wait_remote_degraded(client: nori, workspace_id: str, timeout: float) -> dict:
    deadline = time.time() + timeout
    last_status = {}
    while time.time() < deadline:
        last_status = client._call("workspace.remote.status", {"workspace_id": workspace_id}) or {}
        ...
        if state in {"error", "connecting", "disconnected"}:
            return last_status
        time.sleep(0.5)
    raise noriError(f"Remote did not enter reconnecting/degraded state: {last_status}")
...
def main() -> int:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="nori-ssh-reconnect-"))
    image_tag = f"nori-ssh-test:{secrets.token_hex(4)}"
    container_name = f"nori-ssh-reconnect-{secrets.token_hex(4)}"
        ...
        with nori(SOCKET_PATH) as client:
            ...
            while time.time() < first_deadline_http:
                try:
                    ...
                    time.sleep(0.5)
                    continue
                if "nori-ssh-forward-ok" in first_body:
                    break
                time.sleep(0.3)
            _must("nori-ssh-forward-ok" in first_body, f"Forwarded HTTP endpoint failed before reconnect: {first_body[:120]!r}")
            first_pipelined_body = _socks5_http_get_pipelined("127.0.0.1", int(first_proxy_port), "127.0.0.1", REMOTE_HTTP_PORT)
            _must(
                "nori-ssh-forward-ok" in first_pipelined_body,
                f"SOCKS pipelined greeting/connect+payload failed before reconnect: {first_pipelined_body[:120]!r}",
            )

            first_ws_socks_message = "nori-reconnect-before-over-socks"
            ...
            first_ws_connect_message = "nori-reconnect-before-over-connect"
            ...
            while time.time() < deadline_http:
                try:
                    ...
                    time.sleep(0.5)
                    continue
                if "nori-ssh-forward-ok" in second_body:
                    break
                time.sleep(0.3)
            _must("nori-ssh-forward-ok" in second_body, f"Forwarded HTTP endpoint failed after reconnect: {second_body[:120]!r}")
            second_pipelined_body = _socks5_http_get_pipelined("127.0.0.1", int(second_proxy_port), "127.0.0.1", REMOTE_HTTP_PORT)
            _must(
                "nori-ssh-forward-ok" in second_pipelined_body,
                f"SOCKS pipelined greeting/connect+payload failed after reconnect: {second_pipelined_body[:120]!r}",
            )

            second_ws_socks_message = "nori-reconnect-after-over-socks"
            ...
            second_ws_connect_message = "nori-reconnect-after-over-connect"
