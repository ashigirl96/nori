#!/usr/bin/env python3
"""Regression: interactive `nori ssh` shells must resolve `nori` to the relay wrapper."""
...
SSH_HOST = os.environ.get("NORI_SSH_TEST_HOST", "").strip()
...
def _run_cli_json(cli: str, args: list[str]) -> dict:
    ...
    if proc.returncode != 0:
        raise noriError(f"CLI failed ({' '.join(args)}): {(proc.stdout + proc.stderr).strip()}")
    try:
...
def _workspace_id_from_payload(client: nori, payload: dict) -> str:
...
def _wait_remote_ready(client: nori, workspace_id: str, timeout: float = 25.0) -> None:
    ...
    while time.time() < deadline:
        ...
        if str(remote.get("state") or "") == "connected" and str(daemon.get("state") or "") == "ready":
            return
        time.sleep(0.25)
    raise noriError(f"Remote did not become ready for {workspace_id}: {last_status}")


def _wait_surface_id(client: nori, workspace_id: str, timeout: float = 10.0) -> str:
    deadline = time.time() + timeout
    while time.time() < deadline:
        ...
        time.sleep(0.1)
    raise noriError(f"No terminal surface appeared for workspace {workspace_id}")


def _wait_text(client: nori, surface_id: str, token: str, timeout: float = 12.0) -> str:
    deadline = time.time() + timeout
    last = ""
    while time.time() < deadline:
        ...
        if token in last:
            return last
        time.sleep(0.15)
    raise noriError(f"Timed out waiting for {token!r} in surface {surface_id}: {last[-1200:]!r}")


def _wait_shell_ready(client: nori, surface_id: str, timeout: float = 20.0) -> None:
    token = f"__NORI_SHELL_READY_{secrets.token_hex(6)}__"
...
def _assert_no_login_profile_noise(text: str) -> None:
    _must(
        "/Users/nori/.profile:" not in text,
...
def _run_remote_shell_command(client: nori, surface_id: str, command: str, timeout: float = 12.0) -> tuple[int, str, str]:
    token = f"__NORI_REMOTE_CMD_{secrets.token_hex(6)}__"
    ...
    client.send_surface(
        ...
        (
            ...
            f"{command}; "
            "__nori_status=$?; "
            f"printf '{status_marker}:%s' \"$__nori_status\"; echo; "
    ...
    if not matches:
        raise noriError(f"Missing command result token for {command!r}: {text[-1200:]!r}")
...
def main() -> int:
    if not SSH_HOST:
        print("SKIP: set NORI_SSH_TEST_HOST to run interactive ssh nori command regression")
        return 0
    ...
    try:
        with nori(SOCKET_PATH) as client:
            ...
            which_status, which_output, which_text = _run_remote_shell_command(client, surface_id, "command -v nori")
            _must(which_status == 0, f"`command -v nori` failed: output={which_output!r} tail={which_text[-1200:]!r}")
            _must(
                "/.nori/bin/nori" in which_output,
                f"interactive ssh shell should resolve nori to relay wrapper, got {which_output!r}",
            )

            ping_status, ping_output, ping_text = _run_remote_shell_command(client, surface_id, "nori ping")
            _must(ping_status == 0, f"`nori ping` failed in interactive shell: output={ping_output!r} tail={ping_text[-1200:]!r}")
            _must("pong" in ping_output.lower(), f"`nori ping` should return pong, got {ping_output!r}")
            _must(
                "Socket not found at 127.0.0.1:" not in ping_text,
                f"interactive ssh shell still routed nori to a unix-socket-only binary: {ping_text[-1200:]!r}",
            )
            _must(
                "waiting for relay on 127.0.0.1:" not in ping_text and "failed to connect to 127.0.0.1:" not in ping_text,
                f"`nori ping` hit a dead ssh relay instead of the local app socket: {ping_text[-1200:]!r}",
            )

            notify_status, notify_output, notify_text = _run_remote_shell_command(
                client,
                surface_id,
                "nori notify --body interactive-ssh-regression",
            )
            _must(
                notify_status == 0,
                f"`nori notify` failed in interactive shell: output={notify_output!r} tail={notify_text[-1200:]!r}",
            )
            _must(
                "Socket not found at 127.0.0.1:" not in notify_text,
                f"`nori notify` still failed via wrong nori binary: {notify_text[-1200:]!r}",
            )
            _must(
                "waiting for relay on 127.0.0.1:" not in notify_text and "failed to connect to 127.0.0.1:" not in notify_text,
                f"`nori notify` still failed because the ssh relay listener was not running: {notify_text[-1200:]!r}",
            )
    ...
    print("PASS: interactive ssh shell resolves nori to relay wrapper and remote nori commands succeed")
    return 0
