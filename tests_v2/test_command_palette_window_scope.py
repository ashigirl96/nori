...
def _wait_until(predicate, timeout_s: float = 5.0, interval_s: float = 0.05, message: str = "timeout") -> None:
    ...
    raise noriError(message)


def _palette_visible(client: nori, window_id: str) -> bool:
    res = client._call("debug.command_palette.visible", {"window_id": window_id}) or {}
    return bool(res.get("visible"))


def _palette_results(client: nori, window_id: str, limit: int = 20) -> dict:
    return client.command_palette_results(window_id=window_id, limit=limit)


def _set_palette_visible(client: nori, window_id: str, visible: bool) -> None:
...
def _assert_shortcut_window_scoped(client: nori, shortcut: str, w1: str, w2: str) -> None:
    ...
    if _palette_visible(client, w2):
        raise noriError(f"{shortcut} in window1 incorrectly opened palette in window2")
...
def _assert_cross_window_typing_after_mixed_shortcuts(client: nori, w1: str, w2: str) -> None:
    _set_palette_visible(client, w1, False)
    _set_palette_visible(client, w2, False)
...
def main() -> int:
    with nori(SOCKET_PATH) as client:
        ...
        if _palette_visible(client, w2):
            raise noriError("window2 palette became visible when toggling window1")
        ...
        if _palette_visible(client, w1):
            raise noriError("window1 palette became visible when toggling window2")
        client._call("debug.command_palette.toggle", {"window_id": w2})
        _wait_until(
            lambda: not _palette_visible(client, w2),
            timeout_s=3.0,
