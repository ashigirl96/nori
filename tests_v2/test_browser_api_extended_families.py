...
def _expect_error_contains(label: str, fn, needle: str) -> None:
    try:
        ...
        if needle in text:
            return
        raise noriError(f"{label}: expected error containing {needle!r}, got: {text}")
    raise noriError(f"{label}: expected error containing {needle!r}, but call succeeded")


def _wait_selector(c: nori, surface_id: str, selector: str, timeout_s: float = 6.0) -> None:
    ...
    while time.time() < deadline:
        ...
        if bool(probe.get("value")):
            return
        time.sleep(0.05)
    raise noriError(f"Timed out waiting for selector {selector}")


def _wait_function(c: nori, surface_id: str, expression: str, timeout_s: float = 6.0) -> None:
    timeout_ms = max(1, int(timeout_s * 1000.0))
    try:
    ...
    while time.time() < deadline:
        ...
        if bool(probe.get("value")):
            return
        time.sleep(0.05)
    raise noriError(f"Timed out waiting for function: {expression}")


@contextmanager
def _local_test_server() -> str:
    with tempfile.TemporaryDirectory(prefix="nori-browser-ext-") as root:
        root_path = Path(root)
...
<html>
  <head>
    <title>nori-browser-extended-second</title>
  </head>
  <body>
...
<html>
  <head>
    <title>nori-browser-extended</title>
    <style>
  ...
  <body>
    ...
    <script>
      ...
      window.emitConsoleAndError = function () {
        console.log('nori-console-entry');
        setTimeout(function () {
          throw new Error('nori-boom');
        }, 0);
        return true;
      };
    </script>
...
def main() -> int:
    with _local_test_server() as base_url:
        ...
        with nori(SOCKET_PATH) as c:
            ...
            download_path = tempfile.NamedTemporaryFile(delete=False, prefix="nori-download-", suffix=".txt").name
            ...
            got_cookie = c._call("browser.cookies.get", {"surface_id": sid, "name": "nori_cookie"}) or {}
            cookies = got_cookie.get("cookies") or []
            _must(any(str(row.get("name")) == "nori_cookie" for row in cookies), f"Expected nori_cookie in cookies.get: {got_cookie}")
            c._call("browser.cookies.clear", {"surface_id": sid, "name": "nori_cookie"})
            got_after_clear = c._call("browser.cookies.get", {"surface_id": sid, "name": "nori_cookie"}) or {}
            ...
            c._call("browser.addinitscript", {"surface_id": sid, "script": "window.__noriInitMarker = 'init-ok';"})
            c._call("browser.navigate", {"surface_id": sid, "url": second_url})
            _wait_selector(c, sid, "#second", timeout_s=7.0)
            init_value = c._call("browser.eval", {"surface_id": sid, "script": "window.__noriInitMarker || ''"}) or {}
            ...
            state_path = tempfile.NamedTemporaryFile(delete=False, prefix="nori-state-", suffix=".json").name
