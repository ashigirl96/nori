...
def _wait_for_terminal_focus(c: nori, panel_id: str, timeout_s: float = 8.0) -> None:
    start = time.time()
...
def _wait_for_text(c: nori, panel_id: str, needle: str, timeout_s: float = 2.5) -> None:
    start = time.time()
    last = ""
    while time.time() - start < timeout_s:
    ...
    raise noriError(f"Timed out waiting for token in terminal text: {needle}\nLast tail:\n{tail}")


def _type_and_wait_visible(c: nori, panel_id: str, cmd: str) -> bool:
...
def _wait_for_tmp_write(c: nori, panel_id: str, tmp: str, token: str) -> None:
...
def main() -> int:
    with nori(SOCKET_PATH) as c:
        ...
        panes = c.list_panes()
        if len(panes) < 2:
            raise noriError(f"expected multiple panes, got: {panes}")
        ...
        for i in range(6):
            ...
            token = f"NORI_NEW_TAB_OK_{i}_{uuid.uuid4().hex[:10]}"
            tmp = f"/tmp/nori_new_tab_{token}.txt"
