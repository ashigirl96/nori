#!/usr/bin/env python3
"""
Stability regression test: browser panels should not crash nori when:
...
def ensure_webview_focused(client: nori, panel_id: str, timeout_s: float = 2.0) -> None:
    """
...
def test_open_browser_then_new_surface_loop(client: nori) -> tuple[bool, str]:
...
def test_focus_panes_with_loaded_browser(client: nori) -> tuple[bool, str]:
    ws_id = client.new_workspace()
    client.select_workspace(ws_id)
    time.sleep(0.5)
...
def run_tests() -> int:
    print("=" * 60)
    print("nori Browser Panel Stability Test")
    print("=" * 60)
    print()
    ...
    try:
        with nori(socket_path=probe.socket_path) as client:
