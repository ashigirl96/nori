...
def _run_cli(cli: str, args: list[str]) -> str:
    ...
    if proc.returncode != 0:
        ...
        raise noriError(f"CLI failed ({' '.join(args)}): {merged}")
...
def _open_server() -> tuple[str, socketserver.TCPServer, threading.Thread, tempfile.TemporaryDirectory[str]]:
    root = tempfile.TemporaryDirectory(prefix="nori-browser-cli-logs-")
  ...
  <body>
    ...
    <script>
      window.emitLogs = function () {
        ...
        setTimeout(function () { throw new Error('nori-browser-boom'); }, 0);
        return true;
      };
    </script>
  </body>
...
def main() -> int:
    ...
    try:
        with nori(SOCKET_PATH) as c:
            ...
            _must("nori-console-entry" in console_output, f"browser console text mode should print entries: {console_output!r}")
            ...
            _must("nori-browser-boom" in errors_output, f"browser errors text mode should print entries: {errors_output!r}")
