#!/usr/bin/env python3
"""Process-level integration: norid-remote stdio session resize coordinator."""
...
def _rpc(
    ...
    *,
    ...
    if proc.stdin is None or proc.stdout is None:
        raise noriError("daemon subprocess stdio pipes are not available")
        ...
        if line == "":
            ...
            if proc.stderr is not None:
                try:
                    ...
                    stderr = ""
            raise noriError(f"norid-remote exited while waiting for {method} response: {stderr}")
        try:
            resp = json.loads(line)
        except Exception as exc:  # noqa: BLE001
            raise noriError(f"Invalid JSON response for {method}: {line!r} ({exc})")
        _must(resp.get("id") == req_id, f"Response id mismatch for {method}: {resp}")
        return resp

    raise noriError(f"Timed out waiting for norid-remote response: {method}")


def _as_int(value: object, field: str) -> int:
    if isinstance(value, bool):
        raise noriError(f"{field} should be numeric, got bool")
    if isinstance(value, int):
        return value
    if isinstance(value, float):
        if not value.is_integer():
            raise noriError(f"{field} should be an integer value, got float {value!r}")
        return int(value)
    raise noriError(f"{field} has unexpected type {type(value).__name__}: {value!r}")
...
def main() -> int:
    ...
    proc = subprocess.Popen(
        ["go", "run", "./cmd/norid-remote", "serve", "--stdio"],
        cwd=str(daemon_dir),
    ...
    try:
        ...
        print("PASS: norid-remote stdio session.resize coordinator enforces smallest-screen-wins semantics")
        return 0
    finally:
        try:
