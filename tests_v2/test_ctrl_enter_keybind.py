...
def infer_app_name_for_osascript(socket_path: str) -> str:
    ...
    Examples:
      - /tmp/nori-debug.sock          -> "nori DEV"
      - /tmp/nori-debug-foo.sock      -> "nori DEV foo"
      - ~/Library/Application Support/nori/nori.sock -> "nori"
      - /tmp/nori-foo.sock            -> "nori foo"
    """
    base = Path(socket_path).name
    if base.startswith("nori-debug") and base.endswith(".sock"):
        suffix = base[len("nori-debug") : -len(".sock")]
        if suffix.startswith("-") and suffix[1:]:
            return f"nori DEV {suffix[1:]}"
        return "nori DEV"
    if base.startswith("nori") and base.endswith(".sock"):
        suffix = base[len("nori") : -len(".sock")]
        if suffix.startswith("-") and suffix[1:]:
            return f"nori {suffix[1:]}"
        return "nori"
...
def test_ctrl_enter_keybind(client: nori) -> tuple[bool, str]:
...
def run_tests() -> int:
    print("=" * 60)
    print("nori Ctrl+Enter Keybind Test")
    print("=" * 60)
    print()
