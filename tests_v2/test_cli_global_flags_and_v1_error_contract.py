...
LAST_SOCKET_HINT_PATH = Path("/tmp/nori-last-socket-path")
...
def main() -> int:
    ...
    _must("nori" in version_out, f"--version output should mention nori: {version_out!r}")

    # Debug builds should auto-resolve the active debug socket via /tmp/nori-last-socket-path
    # when NORI_SOCKET_PATH is not set.
    ...
    try:
        ...
        auto_env.pop("NORI_SOCKET_PATH", None)
        auto_env.pop("NORI_SOCKET", None)
    ...
    ping_proc = _run([cli, "--socket", SOCKET_PATH, "--password", "ignored-in-norionly", "ping"])
