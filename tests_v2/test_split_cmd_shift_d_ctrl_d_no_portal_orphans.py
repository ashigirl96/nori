...
LOG_PATH_OVERRIDE = os.environ.get("NORI_DEBUG_LOG")
ITERATIONS = int(os.environ.get("NORI_PORTAL_ORPHAN_ITERS", "16"))
PANE_TIMEOUT_S = float(os.environ.get("NORI_PORTAL_ORPHAN_PANE_TIMEOUT_S", "3.0"))
INTEGRITY_TIMEOUT_S = float(os.environ.get("NORI_PORTAL_ORPHAN_INTEGRITY_TIMEOUT_S", "1.5"))
POLL_S = float(os.environ.get("NORI_PORTAL_ORPHAN_POLL_S", "0.02"))
CTRL_D_RETRY_INTERVAL_S = float(os.environ.get("NORI_PORTAL_ORPHAN_CTRL_D_RETRY_INTERVAL_S", "0.20"))
CTRL_D_MAX_EXTRA = int(os.environ.get("NORI_PORTAL_ORPHAN_CTRL_D_MAX_EXTRA", "3"))
POST_CLOSE_SETTLE_S = float(os.environ.get("NORI_PORTAL_ORPHAN_POST_CLOSE_SETTLE_S", "0.08"))
LOG_FLUSH_S = float(os.environ.get("NORI_PORTAL_ORPHAN_LOG_FLUSH_S", "0.15"))
...
def _derive_log_path(socket_path: str) -> str:
    ...
    if base.startswith("nori-debug-") and base.endswith(".sock"):
        slug = base[len("nori-debug-") : -len(".sock")]
        return f"/tmp/nori-debug-{slug}.log"
    return "/tmp/nori-debug.log"


def _read_new_lines(log_path: str, offset: int) -> tuple[list[str], int]:
    if not os.path.exists(log_path):
        raise noriError(f"debug log not found at {log_path}")
...
def _panel_for_pane(layout_payload: dict, pane: dict) -> str:
    ...
    if not panel_id:
        raise noriError(f"missing selected panel for pane: pane_id={pane_id} selected={selected}")
    return panel_id
...
def _rightmost_panel(layout_payload: dict) -> str:
    ...
    if len(panes) < 2:
        raise noriError(f"expected >=2 panes to find rightmost panel, got {len(panes)}")
...
def _bottom_right_panel(layout_payload: dict) -> str:
    panes = (layout_payload.get("layout") or {}).get("panes") or []
    if len(panes) < 3:
        raise noriError(f"expected >=3 panes to find bottom-right panel, got {len(panes)}")
    bottom_right = max(panes, key=_pane_sort_key)
    return _panel_for_pane(layout_payload, bottom_right)


def _wait_for_panes(c: nori, target_panes: int, *, timeout_s: float, context: str) -> dict:
...
def _portal_stats(c: nori, *, timeout_s: float) -> dict:
    stats = c._call("debug.portal.stats", timeout_s=timeout_s) or {}
    if not isinstance(stats, dict):
        raise noriError(f"debug.portal.stats returned non-dict payload: {stats!r}")
    return stats
...
def _wait_for_portal_integrity(c: nori, *, timeout_s: float, context: str) -> None:
    deadline = time.time() + timeout_s
    last = None
    error = None
    while time.time() < deadline:
        ...
        if error is None:
            return
        time.sleep(POLL_S)
    raise noriError(f"{context}: {error}; stats={last}")


def _close_bottom_right_via_ctrl_d(c: nori, *, bottom_right_panel_id: str, context: str) -> dict:
...
def main() -> int:
    ...
    if not os.path.exists(log_path):
        raise noriError(f"debug log not found at {log_path} for socket={SOCKET_PATH}")
    ...
    if close_count == 0:
        raise noriError("no surface.close.childExited events captured; test did not exercise close path")
    if violations:
        sample = "\n".join(violations[:5])
        raise noriError(
