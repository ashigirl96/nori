...
enum BrowserFindJavaScript {
    ...
    static func searchScript(query: String) -> String {
        ...
        return """
        (() => {
          const MARK_CLASS = '__nori-find';
          const CURRENT_CLASS = '__nori-find-current';
          ...
          window.__noriFindMatches = matches;
          window.__noriFindIndex = 0;
          ...
          if (!document.getElementById('__nori-find-style')) {
            const style = document.createElement('style');
            style.id = '__nori-find-style';
            style.textContent = `
              mark.__nori-find { background: #facc15; color: #000; border-radius: 2px; }
              mark.__nori-find.__nori-find-current { background: #f97316; color: #fff; }
            `;
    ...
    static func nextScript() -> String {
        """
        (() => {
          const matches = window.__noriFindMatches || [];
          if (matches.length === 0) return JSON.stringify({ total: 0, current: 0 });
          let idx = window.__noriFindIndex || 0;
          if (!matches[idx] || !matches[idx].isConnected) {
            window.__noriFindMatches = [];
            window.__noriFindIndex = 0;
            return JSON.stringify({ total: 0, current: 0 });
          }
          matches[idx].classList.remove('__nori-find-current');
          ...
          matches[idx].classList.add('__nori-find-current');
          matches[idx].scrollIntoView({ block: 'center', behavior: 'smooth' });
          window.__noriFindIndex = idx;
    ...
    static func clearScript() -> String {
        ...
        (() => {
          ...
          window.__noriFindMatches = [];
          window.__noriFindIndex = 0;
          const style = document.getElementById('__nori-find-style');
    ...
    document.querySelectorAll('mark.__nori-find').forEach(mark => {
