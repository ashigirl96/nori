...
/// Manages trusted directories for nori.json command execution.
/// When a directory (or its git repo root) is trusted, `confirm: true` commands
/// from that directory's nori.json skip the confirmation dialog.
/// Global config (~/.config/nori/nori.json) is always trusted.
final class NoriDirectoryTrust {
    static let shared = NoriDirectoryTrust()
    static let didChangeNotification = Notification.Name("nori.directoryTrustDidChange")
    ...
    private init() {
        ...
        ).first!.appendingPathComponent("nori")
    ...
    /// Check if a nori.json path is trusted.
    /// Global config is always trusted. For local configs, check the git repo root
    /// (or the nori.json parent directory if not in a git repo).
    ...
    /// Trust the directory containing a nori.json. If the nori.json is inside a git
    ...
    /// Resolve the trust key for a nori.json path: git repo root if inside a repo,
    /// otherwise the nori.json's parent directory.
