...
struct NoriConfigExecutor {
    ...
    static func execute(
        ...
        if let workspace = command.workspace {
            ...
            if needsConfirm, let sourcePath = configSourcePath {
                let trusted = NoriDirectoryTrust.shared.isTrusted(
    ...
    private static func showConfirmDialog(command: String, configPath: String) -> Bool {
        ...
        alert.messageText = String(
            localized: "dialog.noriConfig.confirmCommand.title",
            defaultValue: "Run Command"
        )
        let messageFormat = String(
            localized: "dialog.noriConfig.confirmCommand.messageWithCommand",
        ...
        alert.addButton(withTitle: String(
            localized: "dialog.noriConfig.confirmCommand.run",
            defaultValue: "Run"
        ))
        alert.addButton(withTitle: String(
            localized: "dialog.noriConfig.confirmCommand.cancel",
            defaultValue: "Cancel"
        ))

        let checkbox = NSButton(checkboxWithTitle: String(
            localized: "dialog.noriConfig.confirmCommand.trustDirectory",
        ...
        if checkbox.state == .on {
            NoriDirectoryTrust.shared.trust(configPath: configPath)
        }
    ...
    private static func executeWorkspaceCommand(
        ...
        if let existing = tabManager.tabs.first(where: { $0.customTitle == workspaceName }) {
            ...
            case .confirm:
                let alert = NSAlert()
                alert.messageText = String(
                    localized: "dialog.noriConfig.confirmRestart.title",
                    defaultValue: "Workspace Already Exists"
                )
                alert.informativeText = String(
                    localized: "dialog.noriConfig.confirmRestart.message",
