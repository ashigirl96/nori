...
extension WKWebView {
    var noriIsElementFullscreenActiveOrTransitioning: Bool {
    ...
    func noriIsManagedByExternalFullscreenWindow(relativeTo expectedWindow: NSWindow?) -> Bool {
        guard noriIsElementFullscreenActiveOrTransitioning else { return false }
...
/// family before nori falls back to its own browser find overlay.
final class NoriWebView: WKWebView {
    ...
    private static let pasteAsPlainTextFocusMessageHandlerName = "noriPasteAsPlainTextFocus"
    ...
    const __noriPasteAsPlainTextHelpers = (() => {
      const existing = window.__noriPasteAsPlainTextHelpers;
      ...
      window.__noriPasteAsPlainTextHelpers = helpers;
      return helpers;
    })();
    """
    ...
    (() => {
      try {
        if (window.__noriPasteAsPlainTextFocusTrackerInstalled) return true;
        window.__noriPasteAsPlainTextFocusTrackerInstalled = true;
        ...
        const publish = (canPaste) => {
          ...
          window.__noriPasteAsPlainTextTargetAvailable = canPaste;
          try {
            handler?.postMessage({ canPaste });
          } catch (_) {}
        };

        window.__noriCanPasteAsPlainTextIntoCurrentFocus = () => {
          return __noriPasteAsPlainTextHelpers.canPasteAsPlainTextInto(document.activeElement);
        };

        const publishForElement = (el) => {
          publish(__noriPasteAsPlainTextHelpers.canPasteAsPlainTextInto(el));
        };
        ...
        document.addEventListener("mousedown", (ev) => {
          ...
          if (!__noriPasteAsPlainTextHelpers.canPasteAsPlainTextInto(target)) {
            publish(false);
          }
        }, true);
    ...
    private final class PasteAsPlainTextFocusMessageHandler: NSObject, WKScriptMessageHandler {
        func userContentController(
            ...
            guard let webView = message.webView as? NoriWebView else {
                return
            }
    ...
    static func hasRecentMiddleClickIntent(for webView: WKWebView) -> Bool {
        guard let webView = webView as? NoriWebView else { return false }
    ...
    private static func recordMiddleClickIntent(for webView: NoriWebView) {
    ...
    var contextMenuLinkURLProvider: ((NoriWebView, NSPoint, @escaping (URL?) -> Void) -> Void)?
    ...
    private func pageCanAcceptPlainTextPaste() -> Bool {
        ...
        (() => {
            try {
                const fn = window.__noriCanPasteAsPlainTextIntoCurrentFocus;
