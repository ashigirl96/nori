...
/// Hosts a popup `NoriWebView` in a standalone `NSPanel`, created when a page
...
/// `nori_performKeyEquivalent` can dispatch it to the main menu's
...
final class BrowserPopupWindowController: NSObject, NSWindowDelegate {
    ...
    init(
        ...
        // overlaying the opener's browser context so OAuth popups keep nori's
        // shared cookie/storage scope and opener linkage.
        let webView = NoriWebView(frame: .zero, configuration: configuration)
        ...
        panel.identifier = NSUserInterfaceItemIdentifier("nori.browser-popup")
