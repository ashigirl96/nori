...
enum UITestLaunchManifest {
    static let argumentName = "-noriUITestLaunchManifest"
...
struct noriApp: App {
    ...
    @StateObject private var noriConfigStore = NoriConfigStore()
    ...
    init() {
        ...
        if let stored = defaults.string(forKey: SocketControlSettings.appStorageKey) {
            ...
            defaults.set(legacy ? SocketControlMode.noriOnly.rawValue : SocketControlMode.off.rawValue,
    ...
    var body: some Scene {
        ...
        .commands {
            CommandGroup(replacing: .appSettings) {
                ...
                Button(String(localized: "menu.app.openNoriSettingsFile", defaultValue: "Open settings.json")) {
                    openNoriSettingsFileInEditor()
                }
            ...
            CommandGroup(replacing: .appInfo) {
                Button(String(localized: "menu.app.about", defaultValue: "About nori")) {
                    showAboutPanel()
                }
            }

            CommandGroup(replacing: .appTermination) {
                splitCommandButton(title: String(localized: "menu.quitNori", defaultValue: "Quit nori"), shortcut: menuShortcut(for: .quit)) {
                    NSApp.terminate(nil)
                }
            }
    ...
    private func closePanelOrWindow() {
        if let window = NSApp.keyWindow ?? NSApp.mainWindow,
           noriWindowShouldOwnCloseShortcut(window) {
...
private let noriAuxiliaryWindowIdentifiers: Set<String> = [
    "nori.settings",
    "nori.about",
    "nori.licenses",
    "nori.browser-popup",
    "nori.settingsAboutTitlebarDebug",
    "nori.debugWindowControls",
    "nori.browserImportHintDebug",
    "nori.sidebarDebug",
    "nori.menubarDebug",
    "nori.backgroundDebug",
]

/// Returns whether the given window should handle the standard close shortcut
/// as a standalone auxiliary window instead of routing it through workspace or
/// panel-close behavior.
func noriWindowShouldOwnCloseShortcut(_ window: NSWindow?) -> Bool {
    guard let identifier = window?.identifier?.rawValue else { return false }
    return noriAuxiliaryWindowIdentifiers.contains(identifier)
}
...
private final class SettingsAboutTitlebarDebugStore: ObservableObject {
    ...
    private func ensureToolbar(on window: NSWindow, kind: SettingsAboutWindowKind) {
        ...
        let identifier = NSToolbar.Identifier("nori.debug.titlebar.\(kind.rawValue)")
...
private final class SettingsAboutTitlebarDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("nori.settingsAboutTitlebarDebug")
        window.center()
    ...
    private func editor(for kind: SettingsAboutWindowKind) -> some View {
        ...
        return GroupBox(kind.displayTitle) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text("When disabled, nori uses normal default titlebar behavior for this window.")
                    .font(.caption)
...
private final class DebugWindowControlsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("nori.debugWindowControls")
        window.center()
...
private final class BrowserImportHintDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("nori.browserImportHintDebug")
        window.center()
...
private final class BrowserProfilePopoverDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("nori.browserProfilePopoverDebug")
        window.center()
...
private final class AboutWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("nori.about")
        window.center()
...
private final class AcknowledgmentsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.identifier = NSUserInterfaceItemIdentifier("nori.licenses")
        window.center()
...
final class SettingsWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("nori.settings")
        window.center()
...
enum SettingsNavigationRequest {
    static let notificationName = Notification.Name("nori.settings.navigate")
...
private final class FileExplorerStyleDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("nori.fileExplorerStyleDebug")
        window.center()
...
private final class SidebarDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("nori.sidebarDebug")
        window.center()
...
private struct AboutPanelView: View {
    ...
    private var commit: String? {
        if let value = Bundle.main.infoDictionary?["NORICommit"] as? String, !value.isEmpty {
            return value
        }
        let env = ProcessInfo.processInfo.environment["NORI_COMMIT"] ?? ""
        ...
        VStack(alignment: .center) {
            ...
            VStack(alignment: .center, spacing: 32) {
                VStack(alignment: .center, spacing: 8) {
                    Text(String(localized: "about.appName", defaultValue: "nori"))
                        .bold()
                        .font(.title)
                ...
                VStack(spacing: 2) {
                    ...
                    let commitURL = commit.flatMap { hash in
                        URL(string: "https://github.com/manaflow-ai/nori/commit/\(hash)")
                    }
...
private final class MenuBarExtraDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("nori.menubarDebug")
        window.center()
...
private final class SplitButtonLayoutDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("nori.splitButtonLayoutDebug")
        window.center()
...
private final class BackgroundDebugWindowController: NSWindowController, NSWindowDelegate {
    ...
    private init() {
        ...
        window.isMovableByWindowBackground = true
        window.isReleasedWhenClosed = false
        window.identifier = NSUserInterfaceItemIdentifier("nori.backgroundDebug")
        window.center()
...
enum AppIconSettings {
    ...
    private static let dockTileIconDidChangeNotification = Notification.Name("com.nori.appIconDidChange")
...
enum WelcomeSettings {
    static let shownKey = "noriWelcomeShown"
}
...
enum PreferredEditorSettings {
    ...
    static func open(_ url: URL) {
        if NoriUITestCapture.appendLineIfConfigured(
            envKey: "NORI_UI_TEST_CAPTURE_OPEN_PATH",
            line: url.path
        ) {
            return
        }
...
enum NoriRuntimeDebugCapture {
    ...
    private static let configuration: Configuration? = {
        let env = ProcessInfo.processInfo.environment
        guard let baseURLString = env["NORI_RUNTIME_DEBUG_BASE_URL"]?.trimmingCharacters(in: .whitespacesAndNewlines),
              let baseURL = URL(string: baseURLString),
              let token = env["NORI_RUNTIME_DEBUG_TOKEN"]?.trimmingCharacters(in: .whitespacesAndNewlines),
              !token.isEmpty,
              let sessionID = env["NORI_RUNTIME_DEBUG_SESSION_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
...
struct SettingsView: View {
    ...
    private let shortcutChordsDocsURL = URL(string: "https://nori.com/docs/keyboard-shortcuts#shortcut-chords")!
    ...
    @AppStorage("noriPortBase") private var noriPortBase = 9100
    @AppStorage("noriPortRange") private var noriPortRange = 10
    ...
    @AppStorage(BrowserLinkOpenSettings.openTerminalLinksInNoriBrowserKey) private var openTerminalLinksInNoriBrowser = BrowserLinkOpenSettings.defaultOpenTerminalLinksInNoriBrowser
    @AppStorage(BrowserLinkOpenSettings.interceptTerminalOpenCommandInNoriBrowserKey)
    private var interceptTerminalOpenCommandInNoriBrowser = BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInNoriBrowserValue()
    ...
    @AppStorage(BrowserLinkOpenSettings.openSidebarPortLinksInNoriBrowserKey)
    private var openSidebarPortLinksInNoriBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPortLinksInNoriBrowser
    ...
    @State private var trustedDirectoriesDraft: String = NoriDirectoryTrust.shared.allTrustedPaths.joined(separator: "\n")
    ...
    private var paneFirstClickFocusSubtitle: String {
        if paneFirstClickFocusEnabled {
            return String(
                ...
                defaultValue: "When nori is inactive, clicking a pane activates the window and focuses that pane in one click."
            )
        }
        return String(
            localized: "settings.app.paneFirstClickFocus.subtitleOff",
            defaultValue: "When nori is inactive, the first click only activates the window. Click again to focus the pane."
        )
    }
    ...
    private func saveTrustedDirectories() {
        let paths = trustedDirectoriesDraft
            ...
            .filter { !$0.isEmpty }
        NoriDirectoryTrust.shared.replaceAll(with: paths)
    }
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: appLanguage != LanguageSettings.languageAtLaunch.rawValue
                                ? String(localized: "settings.app.language.restartSubtitle", defaultValue: "Restart nori to apply")
                                : nil,
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.app.showInMenuBar.subtitle", defaultValue: "Keep nori in the menu bar for unread notifications and quick actions.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: String(localized: "settings.notifications.paneFlash.subtitle", defaultValue: "Briefly flash a blue outline when nori highlights a pane.")
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: "Run a shell command when a notification arrives. $NORI_NOTIFICATION_TITLE, $NORI_NOTIFICATION_SUBTITLE, $NORI_NOTIFICATION_BODY are set."
                        ) {
                        ...
                        SettingsCardRow(
                            ...
                            subtitle: sendAnonymousTelemetry != telemetryValueAtLaunch
                                ...
                                : String(localized: "settings.app.telemetry.subtitle", defaultValue: "Share anonymized crash and usage data to help improve nori.")
                        ) {
                        ...
                        SettingsCardRow(
                            configurationReview: .json("sidebar.openPullRequestLinksInNoriBrowser"),
                            String(localized: "settings.app.openSidebarPRLinks", defaultValue: "Open Sidebar PR Links in nori Browser"),
                            subtitle: openSidebarPullRequestLinksInNoriBrowser
                                ? String(localized: "settings.app.openSidebarPRLinks.subtitleOn", defaultValue: "Clicks open inside nori browser.")
                                : String(localized: "settings.app.openSidebarPRLinks.subtitleOff", defaultValue: "Clicks open in your default browser.")
                        ) {
                            Toggle("", isOn: $openSidebarPullRequestLinksInNoriBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }
                        .disabled(sidebarHideAllDetails)
                        ...
                        SettingsCardRow(
                            configurationReview: .json("sidebar.openPortLinksInNoriBrowser"),
                            String(localized: "settings.app.openSidebarPortLinks", defaultValue: "Open Sidebar Port Links in nori Browser"),
                            subtitle: openSidebarPortLinksInNoriBrowser
                                ? String(localized: "settings.app.openSidebarPortLinks.subtitleOn", defaultValue: "Port clicks open inside nori browser.")
                                : String(localized: "settings.app.openSidebarPortLinks.subtitleOff", defaultValue: "Port clicks open in your default browser.")
                        ) {
                            Toggle("", isOn: $openSidebarPortLinksInNoriBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }
                        .disabled(sidebarHideAllDetails)
                    ...
                    SettingsCard {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.socketOverrides.note", defaultValue: "Overrides: NORI_SOCKET_ENABLE, NORI_SOCKET_MODE, and NORI_SOCKET_PATH (set NORI_ALLOW_SOCKET_OVERRIDE=1 for stable/nightly builds)."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: claudeCodeHooksEnabled
                                ...
                                : String(localized: "settings.automation.claudeCode.subtitleOff", defaultValue: "Claude Code runs without nori integration.")
                        ) {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.claudeCode.note", defaultValue: "When enabled, nori wraps the claude command to inject session tracking and notification hooks. Disable if you prefer to manage Claude Code hooks yourself."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: cursorHooksEnabled
                                ...
                                : String(localized: "settings.automation.cursor.subtitleOff", defaultValue: "Cursor runs without nori integration.")
                        ) {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.cursor.note", defaultValue: "Hooks must be installed with `nori cursor install-hooks`. They no-op outside nori terminals."))
                    }
                    ...
                    SettingsCard {
                        SettingsCardRow(
                            ...
                            subtitle: geminiHooksEnabled
                                ...
                                : String(localized: "settings.automation.gemini.subtitleOff", defaultValue: "Gemini runs without nori integration.")
                        ) {
                        ...
                        SettingsCardNote(String(localized: "settings.automation.gemini.note", defaultValue: "Hooks must be installed with `nori gemini install-hooks`. They no-op outside nori terminals."))
                    }

                    SettingsCard {
                        SettingsCardRow(configurationReview: .json("automation.portBase"), String(localized: "settings.automation.portBase", defaultValue: "Port Base"), subtitle: String(localized: "settings.automation.portBase.subtitle", defaultValue: "Starting port for NORI_PORT env var."), controlWidth: pickerColumnWidth) {
                            TextField("", value: $noriPortBase, format: .number)
                                .textFieldStyle(.roundedBorder)
                                .multilineTextAlignment(.trailing)
                        }
                        ...
                        SettingsCardRow(configurationReview: .json("automation.portRange"), String(localized: "settings.automation.portRange", defaultValue: "Port Range Size"), subtitle: String(localized: "settings.automation.portRange.subtitle", defaultValue: "Number of ports per workspace."), controlWidth: pickerColumnWidth) {
                            TextField("", value: $noriPortRange, format: .number)
                                .textFieldStyle(.roundedBorder)
                                .multilineTextAlignment(.trailing)
                        }
                        ...
                        SettingsCardNote(String(localized: "settings.automation.port.note", defaultValue: "Each workspace gets NORI_PORT and NORI_PORT_END env vars with a dedicated port range. New terminals inherit these values."))
                    }
                    ...
                    SettingsCard {
                        VStack(alignment: .leading, spacing: 6) {
                            SettingsCardRow(
                                ...
                                subtitle: String(localized: "settings.customCommands.trustedDirectories.subtitle", defaultValue: "Commands from nori.json in these directories run without confirmation. One path per line.")
                            ) {
                                EmptyView()
                            }
                        ...
                        SettingsCardDivider()
                        SettingsCardNote(String(localized: "settings.customCommands.trustedDirectories.note", defaultValue: "Place a nori.json in your project root to define custom commands. Trust a directory from the confirmation dialog, or add paths here. For git repos, trusting the root covers all subdirectories."))
                    }
                    ...
                    SettingsCard {
                        ...
                        SettingsCardRow(
                            configurationReview: .json("browser.openTerminalLinksInNoriBrowser"),
                            String(localized: "settings.browser.openTerminalLinks", defaultValue: "Open Terminal Links in nori Browser"),
                            ...
                            Toggle("", isOn: $openTerminalLinksInNoriBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }
                        ...
                        SettingsCardRow(
                            configurationReview: .json("browser.interceptTerminalOpenCommandInNoriBrowser"),
                            ...
                            Toggle("", isOn: $interceptTerminalOpenCommandInNoriBrowser)
                                .labelsHidden()
                                .controlSize(.small)
                        }

                        if openTerminalLinksInNoriBrowser || interceptTerminalOpenCommandInNoriBrowser {
                            SettingsCardDivider()
                            ...
                            VStack(alignment: .leading, spacing: 6) {
                                SettingsCardRow(
                                    ...
                                    subtitle: String(localized: "settings.browser.hostWhitelist.subtitle", defaultValue: "Applies to terminal link clicks and intercepted `open https://...` calls. Only these hosts open in nori. Others open in your default browser. One host or wildcard per line (for example: example.com, *.internal.example). Leave empty to open all hosts in nori.")
                                ) {
                                    EmptyView()
                                }
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text(String(localized: "settings.browser.httpAllowlist.description", defaultValue: "Controls which HTTP (non-HTTPS) hosts can open in nori without a warning prompt. Defaults include localhost, 127.0.0.1, ::1, 0.0.0.0, and *.localtest.me."))
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        ...
                        SettingsCardRow(
                            ...
                            HStack(spacing: 8) {
                                ...
                                Button(String(localized: "settings.app.settingsFile.openButton", defaultValue: "Open settings.json")) {
                                    openNoriSettingsFileInTextEdit()
                                }
                                .buttonStyle(.bordered)
                                .controlSize(.small)
                ...
                HStack {
                    ...
                    HStack(spacing: 6) {
                        SettingsHeaderActionButton(
                            ...
                            action: openNoriSettingsFileInTextEdit
                        )
                    }
                }
    ...
    private func resetAllSettings() {
        ...
        openSidebarPullRequestLinksInNoriBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPullRequestLinksInNoriBrowser
        openSidebarPortLinksInNoriBrowser = BrowserLinkOpenSettings.defaultOpenSidebarPortLinksInNoriBrowser
...
private enum SettingsConfigurationReview: Equatable {
    ...
    func validate(file: StaticString = #fileID, line: UInt = #line) {
        ...
        let unknownPaths = paths.filter { !NoriSettingsFileStore.supportedSettingsJSONPaths.contains($0) }
        precondition(
    ...
    private var enableSubtitle: String {
        if isEnabled {
            return String(
                ...
                defaultValue: "Press the shortcut from any app to show or hide all nori windows."
            )
        }
        return String(
            localized: "settings.globalHotkey.enable.subtitleOff",
            defaultValue: "Turn this on to show or hide all nori windows from any app."
        )
    }
...
private struct SettingsRootView: View {
    ...
    private func configureSettingsWindow(_ window: NSWindow) {
        ...
        for index in accessories.indices.reversed() {
            ...
            guard identifier.hasPrefix("nori.") else { continue }
