...
private final class GhosttySurfaceCallbackContext {
    ...
    /// Modifier flags from the most recent mouse event that is about to
    /// trigger a synchronous ghostty action callback (e.g. OPEN_URL).
    /// Set immediately before `ghostty_surface_mouse_button` and cleared
    /// right after so the action handler can branch on shift/option/etc.
    /// without needing the modifiers to be plumbed through the C ABI.
    var pendingClickModifiers: NSEvent.ModifierFlags = []
            ...
            if let mods = callbackContext?.pendingClickModifiers,
               mods.contains(.command), mods.contains(.shift) {
                let urlToOpen: URL = {
                    ...
                    case let .embeddedBrowser(url): return url
                    }
                }()
                #if DEBUG
                dlog("link.openURL cmd+shift force external url=\(urlToOpen)")
                #endif
                return performOnMain {
...
class GhosttyNSView: NSView, NSUserInterfaceValidations {
...
#endif
        // libghostty's link detection uses an exact-modifier match
        // (`mouse_mods.equal(ctrlOrSuper)`), so cmd+shift hover would otherwise
        // be filtered out. Strip shift when cmd is held so cmd+shift+click is
        // still recognized as a link gesture; the real shift bit stays in
        // NSEvent.modifierFlags for Swift-side branching (OPEN_URL handler).
        if effectiveFlags.contains(.command), effectiveFlags.contains(.shift) {
            effectiveFlags.remove(.shift)
        }
    ...
    /// Mouse-event modifiers sanitized for ghostty's link detection.
    /// See `hoverModsFromFlags` for the rationale: when cmd is held we strip
    /// shift so cmd+shift+click is still seen as a link gesture by libghostty.
    ...
    override func mouseUp(with event: NSEvent) {
        ...
        // Stash modifiers so synchronous ghostty action callbacks
        // (e.g. OPEN_URL) can branch on shift/option/etc. Cleared immediately
        // after the call returns to avoid leaking state to later events.
        let callbackCtx = GhosttyApp.callbackContext(from: ghostty_surface_userdata(surface))
        callbackCtx?.pendingClickModifiers = event.modifierFlags
        let consumed = ghostty_surface_mouse_button(surface, GHOSTTY_MOUSE_RELEASE, GHOSTTY_MOUSE_LEFT, mouseModsFromEvent(event))
        callbackCtx?.pendingClickModifiers = []
