...
enum SessionRestorePolicy {
    static func isRunningUnderAutomatedTests(
        ...
        if environment["NORI_UI_TEST_MODE"] == "1" {
            return true
        }
        if environment.keys.contains(where: { $0.hasPrefix("NORI_UI_TEST_") }) {
            return true
        }
    ...
    static func shouldAttemptRestore(
        ...
        if environment["NORI_DISABLE_SESSION_RESTORE"] == "1" {
            return false
        }
...
enum SessionScrollbackReplayStore {
    static let environmentKey = "NORI_RESTORE_SCROLLBACK_FILE"
    private static let directoryName = "nori-session-scrollback"
