...
struct NoriConfigFile: Codable, Sendable {
    var commands: [NoriCommandDefinition]
}

struct NoriCommandDefinition: Codable, Sendable, Identifiable {
    var name: String
    var description: String?
    var keywords: [String]?
    var restart: NoriRestartBehavior?
    var workspace: NoriWorkspaceDefinition?
    var command: String?
    var confirm: Bool?

    var id: String {
        "nori.config.command." + (name.addingPercentEncoding(withAllowedCharacters: .alphanumerics) ?? name)
    }
    ...
    init(
        ...
        keywords: [String]? = nil,
        restart: NoriRestartBehavior? = nil,
        workspace: NoriWorkspaceDefinition? = nil,
        command: String? = nil,
        confirm: Bool? = nil
    ) {
        self.name = name
    ...
    init(from decoder: Decoder) throws {
        ...
        restart = try container.decodeIfPresent(NoriRestartBehavior.self, forKey: .restart)
        workspace = try container.decodeIfPresent(NoriWorkspaceDefinition.self, forKey: .workspace)
...
enum NoriRestartBehavior: String, Codable, Sendable {
    case recreate
    case ignore
    case confirm
}

struct NoriWorkspaceDefinition: Codable, Sendable {
    var name: String?
    var cwd: String?
    var color: String?
    var layout: NoriLayoutNode?

    init(name: String? = nil, cwd: String? = nil, color: String? = nil, layout: NoriLayoutNode? = nil) {
        self.name = name
        self.cwd = cwd
        self.color = color
        self.layout = layout
    ...
    init(from decoder: Decoder) throws {
        ...
        layout = try container.decodeIfPresent(NoriLayoutNode.self, forKey: .layout)
...
indirect enum NoriLayoutNode: Codable, Sendable {
    case pane(NoriPaneDefinition)
    case split(NoriSplitDefinition)
    ...
    init(from decoder: Decoder) throws {
        ...
        if hasPane && hasDirection {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
                    debugDescription: "NoriLayoutNode must not contain both 'pane' and 'direction' keys"
                )
            )
        }

        if hasPane {
            let pane = try container.decode(NoriPaneDefinition.self, forKey: .pane)
            self = .pane(pane)
        } else if hasDirection {
            let splitDef = try NoriSplitDefinition(from: decoder)
            self = .split(splitDef)
        } else {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
                    debugDescription: "NoriLayoutNode must contain either a 'pane' key or a 'direction' key"
                )
            )
        }
    }
...
struct NoriSplitDefinition: Codable, Sendable {
    var direction: NoriSplitDirection
    var split: Double?
    var children: [NoriLayoutNode]

    init(direction: NoriSplitDirection, split: Double? = nil, children: [NoriLayoutNode]) {
    ...
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        direction = try container.decode(NoriSplitDirection.self, forKey: .direction)
        split = try container.decodeIfPresent(Double.self, forKey: .split)
        children = try container.decode([NoriLayoutNode].self, forKey: .children)
        if children.count != 2 {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
...
enum NoriSplitDirection: String, Codable, Sendable {
    case horizontal
    case vertical
}

struct NoriPaneDefinition: Codable, Sendable {
    var surfaces: [NoriSurfaceDefinition]

    init(surfaces: [NoriSurfaceDefinition]) {
        self.surfaces = surfaces
    }

    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        surfaces = try container.decode([NoriSurfaceDefinition].self, forKey: .surfaces)
        if surfaces.isEmpty {
            throw DecodingError.dataCorrupted(
                DecodingError.Context(
                    codingPath: decoder.codingPath,
...
struct NoriSurfaceDefinition: Codable, Sendable {
    var type: NoriSurfaceType
    var name: String?
    var command: String?
    var cwd: String?
...
enum NoriSurfaceType: String, Codable, Sendable {
    case terminal
    case browser
}

@MainActor
final class NoriConfigStore: ObservableObject {
    @Published private(set) var loadedCommands: [NoriCommandDefinition] = []
    ...
    let globalConfigPath: String = {
        ...
        return (home as NSString).appendingPathComponent(".config/nori/nori.json")
    }()
    ...
    private let watchQueue = DispatchQueue(label: "com.nori.config-file-watch")
    ...
    private func updateLocalConfigPath(_ directory: String?) {
        ...
        if let directory, !directory.isEmpty {
            newPath = findNoriConfig(startingFrom: directory)
                ?? (directory as NSString).appendingPathComponent("nori.json")
        } else {
            newPath = nil
        }
    ...
    private func findNoriConfig(startingFrom directory: String) -> String? {
        var current = directory
        let fs = FileManager.default
        while true {
            let candidate = (current as NSString).appendingPathComponent("nori.json")
    ...
    private func parseConfig(at path: String) -> NoriConfigFile? {
        ...
        do {
            return try JSONDecoder().decode(NoriConfigFile.self, from: data)
        } catch {
            NSLog("[NoriConfig] parse error at %@: %@", path, String(describing: error))
            return nil
        }
    }
