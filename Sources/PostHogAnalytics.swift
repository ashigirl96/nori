...
final class PostHogAnalytics {
    ...
    // PostHog Cloud US default (matches other nori properties).
    private let host = "https://us.i.posthog.com"

    private let dailyActiveEvent = "nori_daily_active"
    private let hourlyActiveEvent = "nori_hourly_active"
    ...
    private init() {
        workQueue = DispatchQueue(label: "com.nori.posthog.analytics", qos: .utility)
    ...
    private var isEnabled: Bool {
        ...
        return ProcessInfo.processInfo.environment["NORI_POSTHOG_ENABLE"] == "1"
#else
...
#if DEBUG
        config.debug = ProcessInfo.processInfo.environment["NORI_POSTHOG_DEBUG"] == "1"
#endif
    ...
    nonisolated static func superProperties(infoDictionary: [String: Any]) -> [String: Any] {
        var properties: [String: Any] = ["platform": "noriterm"]
    ...
    nonisolated static func shouldFlushAfterCapture(event: String) -> Bool {
        switch event {
        case "nori_daily_active", "nori_hourly_active":
            return true
        default:
            return false
        }
